...
public class PictureServiceImpl extends ServiceImpl<PictureMapper, Picture> implements IPictureService {
    ...
    public Picture uploadPicture(String imgsrc, String destinationPath, String pictureid) throws IOException {
        ...
        if (destinationPath.indexOf((storageService.getBucketName() + "/")) == 0) {
            ftppath = destinationPath.substring((storageService.getBucketName() + "/").length(),
        ...
        if (imgsrc.contains("data:image/") && imgsrc.contains("base64,")) {
            String ext = imgsrc.substring(imgsrc.indexOf("data:image/") + "data:image/".length(),
                    imgsrc.indexOf(";"));// 图片后缀
            ...
            try {
                ...
                storageService.putObject(storageService.getBucketName(), ftppath + "/" + imageName,
    ...
    public Picture uploadPicture(InputStream dataInputStream, String sourcePath, String destinationPath,
            String imageName, String pictureid) throws IOException {
        Picture picture = null;
    ...
    public Picture downloadPicturelocal(String pictureid, String sourcePath, String destinationPath) throws Exception {
        ...
        try {
            storageService.putObject(storageService.getBucketName(), destinationPath + "/" + imageName,
                    new URL(sourcePath).openStream());
        } catch (Exception e) {
            return null;
        }
        if (GeneralUtil.isEmpty(pictureid)) {
    ...
    public String buildDestinationPath(String destinationPath) {
        if (destinationPath.indexOf((storageService.getBucketName() + "/")) == 0) {
            destinationPath = destinationPath.substring((storageService.getBucketName() + "/").length(),
                    destinationPath.length());
            if (destinationPath.charAt(destinationPath.length() - 1) == '/') {
    ...
    public Picture downloadPictureMaterial(String pictureid, String sourcePath, String destinationPath)
            throws Exception {
        Picture picture = null;
        String uri = null;
        String imageName = null;
        ...
        try {
            ...
            storageService.putObject(storageService.getBucketName(), destinationPath + "/" + imageName,
                    new URL("file:\\" + sourcePath).openStream());
        } catch (Exception e) {
            e.printStackTrace();
            return null;
        }
