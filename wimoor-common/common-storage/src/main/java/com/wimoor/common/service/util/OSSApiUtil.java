...
public class OSSApiUtil {
    ...
    public boolean isRun() {
        if (StrUtil.isNotBlank(oss_endpoint) && StrUtil.isNotBlank(accessKeyId) && StrUtil.isNotBlank(
                accessKeySecret)) {
            return true;
        } else {
            return false;
     ...
     *
     * @param objectName
     * @param stream     可以是已下方式 InputStream inputStream = new URL(url).openStream(); ByteArrayInputStream
     *                   inputStream=new ByteArrayInputStream(content.getBytes()) InputStream inputStream = new
