        ...
        try {
            while ((line = br.readLine()) != null) {
                ...
                if (lineNumber == 0) {
                    ...
                    record.setQuantity(
                            ReportFieldUtil.getIntegerValue(info, titleList, "avaliable-quantity(sellable)"));
                    record.setQtyWithRemovalsInProgress(
                            ReportFieldUtil.getIntegerValue(info, titleList, "qty-with-removals-in-progress"));
                    record.setInvAge0To90Days(ReportFieldUtil.getIntegerValue(info, titleList, "inv-age-0-to-90-days"));
                    record.setInvAge91To180Days(
                            ReportFieldUtil.getIntegerValue(info, titleList, "inv-age-91-to-180-days"));
                    record.setInvAge181To270Days(
                            ReportFieldUtil.getIntegerValue(info, titleList, "inv-age-181-to-270-days"));
                    record.setInvAge271To365Days(
                            ReportFieldUtil.getIntegerValue(info, titleList, "inv-age-271-to-365-days"));
                    record.setInvAge365PlusDays(
                            ReportFieldUtil.getIntegerValue(info, titleList, "inv-age-365-plus-days"));
                    record.setCurrency(GeneralUtil.getStrValue(info, titleList, "currency"));
                    String currency = record.getCurrency();
                    record.setQtyToBeChargedLtsf6Mo(
                            ReportFieldUtil.getIntegerValue(info, titleList, "qty-to-be-charged-ltsf-6-mo"));
                    record.setQtyToBeChargedLtsf12Mo(
                            ReportFieldUtil.getIntegerValue(info, titleList, "qty-to-be-charged-ltsf-12-mo"));
                    record.setProjectedLtsf6Mo(
                            ReportFieldUtil.getBigDecimalValue(info, titleList, "projected-ltsf-6-mo"));
                    record.setProjectedLtsf12Mo(
                            ReportFieldUtil.getBigDecimalValue(info, titleList, "projected-ltsf-12-mo"));
                    record.setUnitsShippedLast7Days(
                            ReportFieldUtil.getIntegerValue(info, titleList, "units-shipped-last-7-days"));
                    record.setUnitsShippedLast30Days(
                            ReportFieldUtil.getIntegerValue(info, titleList, "units-shipped-last-30-days"));
                    record.setUnitsShippedLast30Days(
                            ReportFieldUtil.getIntegerValue(info, titleList, "units-shipped-last-60-days"));
                    record.setUnitsShippedLast30Days(
                            ReportFieldUtil.getIntegerValue(info, titleList, "units-shipped-last-90-days"));
                    ...
                    record.setDays(ReportFieldUtil.getIntegerValue(info, titleList,
                            "Healthy Inventory Level	Recommended sales price	Recommended sale duration (days)"));
                    record.setRecommendedAction(
                            ReportFieldUtil.getStrValue(info, titleList, "Recommended Removal Quantity"));
                    record.setEstimatedCostSavingsOfRecommendedActions(
                            ReportFieldUtil.getBigDecimalValue(info, titleList,
