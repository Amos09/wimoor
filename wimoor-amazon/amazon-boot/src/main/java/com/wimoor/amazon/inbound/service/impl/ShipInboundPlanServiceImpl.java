...
public class ShipInboundPlanServiceImpl extends ServiceImpl<ShipInboundPlanMapper, ShipInboundPlan> implements
        IShipInboundPlanService {
    ...
    public void saveShipInboundPlan(ShipInboundPlan inplan) {
        ...
        List<ShipInboundShipment> shipmentList = iFulfillmentInboundService.createInboundShipmentPlan(auth, market,
                inplan);
    ...
    private ShipInboundPlan createPlan(AmazonAuthority auth, Marketplace market, InboundShipmentInfo item,
            String warehouseid, Integer skunum) {
        Date nowdate = new Date();
        ...
        Map<String, Object> tocountry = shipInboundShipmentService.findToCountry(
                item.getDestinationFulfillmentCenterId(), null);
    ...
    @Override
    public ShipInboundShipment createShipment(AmazonAuthority auth, Marketplace market, InboundShipmentInfo item,
            String warehouseid, Integer skunum) {
        // TODO Auto-generated method stub
    ...
    @Override
    public ShipInboundShipment convertShipmentPojo(AmazonAuthority auth, Marketplace market, InboundShipmentInfo item,
            String warehouseid, Integer skunum) {
        // TODO Auto-generated method stub
        Date nowdate = new Date();
        ShipInboundPlan inplan = new ShipInboundPlan();
    ...
    public ShipPlanVo getPlanBaseInfo(String inboundplanid, UserInfo user) {
        ...
        if (list != null && list.getTotal() > 0) {
            ...
            for (SummaryShipmentVo item : listshipment) {
                ...
                AmzShipFulfillmentCenter center = iAmzShipFulfillmentCenterService.getByCode(
                        item.getDestinationFulfillmentCenterId());
                if (center != null) {
    ...
    public Map<String, Object> uploadShipListByExcel(Sheet sheet, Map<String, Object> map) {
        ...
        for (int i = 1; i <= sheet.getLastRowNum(); i++) {
            ...
            if (StrUtil.isNotEmpty(sku)) {
                List<ProductInfo> templist = productInfoMapper.selectBySku(sku.trim(),
                        map.get("marketplaceid").toString(),
                ...
                if (templist == null || templist.size() == 0) {
                    ...
                    if (map.get("warehouseid") == null) {//不用本地库存
                        ...
                        if (qty > 0) {
                            //拿msku
                            String msku = productInfoMapper.findMSKUBySKUMarket(sku,
                                    map.get("marketplaceid").toString(),
                                    map.get("amazonauthid").toString());
                            if (msku == null) {
                                isok = false;
    ...
    public ChartLine shipArrivalTimeChart(String groupid, String amazonAuthId, String sku, String marketplaceid,
            int daysize, UserInfo userinfo) {
        SimpleDateFormat FMT_YMD = new SimpleDateFormat("MM-dd");
        String shopid = userinfo.getCompanyid();
        List<Map<String, Object>> resultList = new ArrayList<Map<String, Object>>();
        List<Map<String, Object>> result = iShipInboundItemService.getshotTime(userinfo.getCompanyid(), groupid,
                marketplaceid, sku);
        List<Map<String, Object>> shipRecord = this.baseMapper.getShipArrivalTimeRecord(userinfo.getCompanyid(),
                marketplaceid, sku);
        ...
        try {
            ...
            Result<List<Map<String, Object>>> oversea = erpClientOneFeign.getShipArrivalTimeRecord(
                    userinfo.getCompanyid(), country, groupid, sku);
        ...
        if (shipRecord != null && shipRecord.size() > 0) {
            ...
            for (int k = 0; k < shipRecord.size(); k++) {
                ...
                if (arrivalTime != null) {
                    LocalDate DateNow = ZonedDateTime.ofInstant(arrivalTime.toInstant(), ZoneId.systemDefault())
                            .toLocalDate();
                    ...
                    String tooltipname =
                            "<table style='color:white;margin:8px'><tr><td>货物编码：" + shipid + "</td></tr><tr><td>"
                                    + "创建日期：" + GeneralUtil.formatDate(createdate) + "</td></tr><tr><td>" + "货物渠道："
                                    + channame + "</td></tr><tr><td>"
                                    + "预计到货日期：" + GeneralUtil.formatDate(arrivalTime, FMT_YMD)
                                    + "</td></tr><tr><td>"
                    ...
                    if (prevmaps == null) {
                        ...
                        LocalDate prevDateNow = ZonedDateTime.ofInstant(prevArrivalTime.toInstant(),
        ...
        if (result != null && result.size() > 0) {
            for (Map<String, Object> skuMap : result) {
                if (skuMap.get("marketplaceId") != null && skuMap.get("marketplaceId").toString().equals("EU")) {
                    int summonth = skuMap.get("salesmonth") == null ? 0
                            : Integer.parseInt(skuMap.get("salesmonth").toString());
                    int sumseven =
                            skuMap.get("salesweek") == null ? 0 : Integer.parseInt(skuMap.get("salesweek").toString());
                    int sum15 = skuMap.get("salesfifteen") == null ? 0
                            : Integer.parseInt(skuMap.get("salesfifteen").toString());
                    ...
                    int qty = DaysalesFormulaServiceImpl.getAvgSales(formula, summonth, sumseven, sum15,
                            GeneralUtil.parseDate(openDate));
                ...
                //每日预估销量
                Map<String, Integer> presaleMap = productInPresaleService.selectBySKUMarket(sku, marketplaceid,
                        groupid);
                Date today = new Date();
                ...
                BigDecimal sales = skuMap.get("sales") == null ? new BigDecimal("0")
                        : new BigDecimal(skuMap.get("sales").toString());
                BigDecimal quantity = skuMap.get("quantity") == null ? new BigDecimal("0")
                        : new BigDecimal(skuMap.get("quantity").toString());
                //安全库存周期
                BigDecimal stocking_cycle = skuMap.get("stocking_cycle") == null ? new BigDecimal("0")
                        : new BigDecimal(skuMap.get("stocking_cycle").toString());
                    ...
                    for (int i = 0; i < shipRecord.size(); i++) {
                        ...
                        if (days < 0) {// 预计到达日期在今天之前,且收货数量与发货数量相差5%
                            if (arrivalquantity.floatValue() <= 5 || (inbondquantity.floatValue() != 0
                                    && arrivalquantity.divide(inbondquantity, 4, RoundingMode.HALF_EVEN).floatValue()
                                    < 0.06)) {
                                continue;
                            }
                            ...
                            tooltipName +=
                                    "<tr><td class='text-red'>比预计少收数量：" + arrivalquantity + "</td></tr></table>";
                ...
                while (day > 0) {
                    ...
                    if (shipRecord != null && shipRecord.size() > 0) {
                        for (int i = 0; i < shipRecord.size(); i++) {
                            Map<String, Object> map = shipRecord.get(i);
                            if (map.get("tooltipName") == null || map.get("arrivalTime") == null
                                    || map.get("sku") == null) {
                                continue;
                            }
                            Date arrivalTime = AmzDateUtils.getDate(map.get("arrivalTime"));
                            ...
                            LocalDate Date1 = ZonedDateTime.ofInstant(today.toInstant(), ZoneId.systemDefault())
                                    .toLocalDate();
                            LocalDate Date2 = ZonedDateTime.ofInstant(arrivalTime.toInstant(), ZoneId.systemDefault())
                                    .toLocalDate();
