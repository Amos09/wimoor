...
public class SummaryOrderReportServiceImpl implements ISummaryOrderReportService {
    ...
    public void addOrdersSummary(OrdersReport oldone, OrdersReport newone) {
        ...
        if (oldNeed == false && newNeed == false) {
            ...
            if (itemPirce != null && quantity == 0 && itemPirce.intValue() == 0) {
                return;
            }
        }
    ...
    public void summaryOrderReport() {
        ...
        if (refreshdate == null || !GeneralUtil.formatDate(refreshdate).equals(GeneralUtil.formatDate(new Date()))) {
            List<ReportRequestRecord> list = reportRequestRecordMapper.selectNoTreatReport(
                    ReportType.OrdersByOrderDateReport, null);
    ...
    public void productDateUpdate(List<Map<String, Object>> list) {
        ...
        for (Map<String, Object> map : list) {
            ...
            try {
                ...
                BigDecimal buyprice =
                        map.get("landed_amount") == null ? null : new BigDecimal(map.get("landed_amount").toString());
    ...
    public BigDecimal calculate_Units(Map<String, Object> map, ProductInOrder productInOrder) {
        ...
        int sumweek =
                map.get("salesWeek") == null ? 0 : Integer.parseInt(map.get("salesWeek").toString());// 往前推2天之后的7日销量
        ...
        BigDecimal price7 = map.get("orderprice7") == null ? new BigDecimal("0")
                : new BigDecimal(map.get("orderprice7").toString());
    ...
    void getProfit(ProductInProfit sys, BigDecimal price, Map<String, Object> map, BigDecimal buyprice,
            Map<String, Object> materialMap) throws BizException {
        if (price == null || price.floatValue() == 0.0) {
            return;
        }
        String msku = map.get("msku").toString();
        ...
        if (materialMap.get("height") != null) {
            ...
            if (materialMap.get("height_unit") != null) {
                dim_local.setHeightUnits(materialMap.get("height_unit").toString());
            }
            dim_local.setWeight(new BigDecimal(materialMap.get("weight").toString()));
            if (materialMap.get("weight_unit") != null) {
                dim_local.setWeightUnits(materialMap.get("weight_unit").toString());
            }
            dim_local.setLength(new BigDecimal(materialMap.get("length").toString()));
            if (materialMap.get("length_unit") != null) {
                dim_local.setLengthUnits(materialMap.get("length_unit").toString());
            }
            dim_local.setWidth(new BigDecimal(materialMap.get("width").toString()));
            if (materialMap.get("width_unit") != null) {
                dim_local.setWidthUnits(materialMap.get("width_unit").toString());
            }
        }
        ...
        if ("EU".equals(map.get("region").toString())) {
            stockCycle = fBAShipCycleMapper.findShipCycleBySKU(map.get("sku").toString(), "EU",
                    map.get("groupid").toString());
        } else {
            stockCycle = fBAShipCycleMapper.findShipCycleBySKU(map.get("sku").toString(),
                    map.get("marketplaceid").toString(), map.get("groupid").toString());
        }
        ...
        try {
            ...
            if (fbaFee == null) {
                ...
                deatail2 = this.profitService.getProfitByLocalData(country, profitcfg, inputDimension_amz,
                        inputDimension_local, isMedia, type, typeid, cost, "RMB", buyprice, "local", inSnl,
                        shipmentfee);
            } else {
                ref = referralFeeService.findByPgroup(fbaFee.getProductGroup(), country);
                if (ref == null || ref.getId() == null) {
        ...
        if (margin.length() >= 2) {
            ...
            if (map.get("sales7") != null) {
                sys.setProfitWeek(deatail.getProfit().multiply(new BigDecimal(map.get("sales7").toString()))
    ...
    public InputDimensions getInputDimensions(Map<String, Object> map) {
        ...
        BigDecimal height =
                map.get("height") == null ? new BigDecimal("0") : new BigDecimal(map.get("height").toString());
        BigDecimal length =
                map.get("length") == null ? new BigDecimal("0") : new BigDecimal(map.get("length").toString());
        BigDecimal weight =
                map.get("weight") == null ? new BigDecimal("0") : new BigDecimal(map.get("weight").toString());
