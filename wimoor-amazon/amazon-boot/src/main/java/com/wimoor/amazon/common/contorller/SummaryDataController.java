...
public class SummaryDataController {
    ...
    public Result<Map<String, Object>> selectByMutilParameterAction(
            @ApiParam("查询DTO") @RequestBody SummaryMutilParameterQueryDTO dto) {
        UserInfo userinfo = UserInfoContext.get();
    ...
    public Result<Map<String, Object>> queryChartReturnAction(
            @ApiParam("查询DTO") @RequestBody SummaryMutilParameterQueryDTO dto) {
        UserInfo userinfo = UserInfoContext.get();
        dto.setShopid(userinfo.getCompanyid());
        if (StrUtil.isEmpty(dto.getGroupid())) {
    ...
    public Result<Map<String, Object>> queryChartReturnSummaryAction(
            @ApiParam("查询DTO") @RequestBody SummaryMutilParameterQueryDTO dto) {
        UserInfo userinfo = UserInfoContext.get();
        dto.setShopid(userinfo.getCompanyid());
        if (StrUtil.isEmpty(dto.getGroupid())) {
    ...
    public Result<Map<String, Object>> selectSumByMutilParameterAction(
            @ApiParam("查询DTO") @RequestBody SummaryMutilParameterQueryDTO dto) {
        UserInfo userinfo = UserInfoContext.get();
        dto.setShopid(userinfo.getCompanyid());
        if (StrUtil.isEmpty(dto.getGroupid())) {
    ...
    public Result<List<Map<String, Object>>> queryChartMarketAction(
            @ApiParam("查询DTO") @RequestBody SummaryMutilParameterQueryDTO dto) {
        UserInfo userinfo = UserInfoContext.get();
        dto.setShopid(userinfo.getCompanyid());
        if (StrUtil.isEmpty(dto.getGroupid())) {
    ...
    public Result<IPage<Map<String, Object>>> findRankByShopAction(
            @ApiParam("查询DTO") @RequestBody UserSalesRankQueryDTO dto) {
        UserInfo userinfo = UserInfoContext.get();
        return Result.success(
                iUserSalesRankService.findRankByShop(dto.getPage(), userinfo.getCompanyid(), dto.getDaytype()));
    }


    @ApiOperation(value = "获取销量统计数据")
    @PostMapping("/top5")
    public Result<List<ProductSalesRankVo>> top5Action(
            @ApiParam("查询DTO") @RequestBody SummaryMutilParameterQueryDTO dto) {
        UserInfo userinfo = UserInfoContext.get();
        dto.setShopid(userinfo.getCompanyid());
        if (StrUtil.isEmpty(dto.getGroupid())) {
