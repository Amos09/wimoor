...
/**
 * <p>
 * 服务实现类 计算 第一步：遍历站点与店铺 第二步：准备销售数据，准备库存数据，准备周期 第三步：准备海外仓数据 第四步：计算
 *
 * </p>
 *
 * @author wimoor team
...
public class AmzProductSalesPlanServiceImpl extends
        ServiceImpl<AmzProductSalesPlanMapper, AmzProductSalesPlan> implements IAmzProductSalesPlanService {
     ...
     * @return
     */
    public Map<String, Integer> getSkuInventry(AmazonAuthority auth, String marketplaceid,
            Map<String, Integer> fulfillMap, String sku) {
        List<Map<String, Object>> listqty = inventoryReportMapper.findFBAWithoutUnsellable(auth.getId(), marketplaceid,
                sku);
    ...
    public Map<String, Integer> getShipRecord(AmazonAuthority auth, String marketplaceid, String sku) {
        List<Map<String, Object>> shipRecord = shipInboundPlanMapper.getShipArrivalTimeRecord(auth.getShopId(),
                marketplaceid, sku);
    ...
    public void handlePresaleMarket(AmazonAuthority auth, Marketplace market, String sku) {
        ...
        for (Map<String, Object> item : list) {
            ...
            if (marketplaceid.equals("EU")) {
                int sumseven =
                        item.get("sumseven") == null ? 0 : Integer.parseInt(item.get("sumseven").toString());// 七日销量
            ...
            try {
                Result<Map<String, Object>> materialResult = erpClientOneFeign.getMSkuDeliveryAndInv(auth.getShopId(),
                        auth.getGroupid(), msku, market.getMarket(), "true");
    ...
    public void handleTags(PlanDTO dto, IPage<Map<String, Object>> page) {
        if (page != null && page.getRecords() != null && page.getRecords().size() > 0) {
            ...
            try {
                Result<Map<String, String>> skutaglist = erpClientOneFeign.getTagsIdsListByMsku(dto.getShopid(),
                        skulist);
        ...
        for (Map<String, Object> item : list) {
            ...
            if (msku != null) {
                ...
                if (dto.getSelected() == true && dto.getPlantype().equals("ship")) {
                    if (planitem != null && planitem.get("amount") != null
                            && Integer.parseInt(planitem.get("amount").toString()) > 0) {
...
//					 }
    ...
    public void setShipRecord(Map<String, Object> item, String shopid, String groupid, String marketplaceid, String sku,
            int puOnDay) {
        List<Map<String, Object>> shipRecord = shipInboundPlanService.getShipRecord(shopid, groupid, marketplaceid,
                sku);
    ...
    public void handItem(Map<String, Object> item, Map<String, Map<String, Object>> planmap, PlanDetailDTO dto) {
        ...
        List<AmzInboundFbaCycle> cycleList = iAmzInboundFbaCycleService.getInboundFbaCycle(dto.getShopid(),
                marketplaceid);
        ...
        if (dto.getPlantype().equals("ship") && dto.getIseu()) {
            ...
            if (inbound != null) {
                if (item.get("quantity") == null) {
                    ...
                    Integer myqty =
                            item.get("localquantity") != null ? Integer.parseInt(item.get("localquantity").toString())
                                    : 0;
        ...
        Integer sysavgsales =
                item.get("sysavgsales") == null ? 0 : Integer.parseInt(item.get("sysavgsales").toString());
        Integer fbaquantity = item.get("quantity") == null ? 0 : Integer.parseInt(item.get("quantity").toString());
        Integer reallyamount =
                item.get("reallyamount") == null ? 0 : Integer.parseInt(item.get("reallyamount").toString());
        if (marketplaceid.equals("EU")) {
        ...
        if (!marketplaceid.equals("EU")) {
            ...
            if (market != null && item.get("asin") != null) {
                item.put("url",
                        "https://www." + market.getPointName() + "/dp/" + item.get("asin").toString() + "?th=1&psc=1");
            }
        }
        Integer overseaqty = 0;
        try {
            Result<Map<String, Object>> materialResult = erpClientOneFeign.getMSkuDeliveryAndInv(dto.getShopid(),
                    groupid, msku, country, "false");
            if (Result.isSuccess(materialResult) && materialResult.getData() != null) {
                if (materialResult.getData().get("overseaqty") != null) {
                    overseaqty = Integer.valueOf(materialResult.getData().get("overseaqty").toString());
                }
        ...
        if (dto.getPlantype().equals("purchase")) {
            Integer needpurchase =
                    item.get("needpurchase") == null ? 0 : Integer.parseInt(item.get("needpurchase").toString());
        ...
        if (dto.getPlantype().equals("purchase")) {
            item.putAll(this.getAfterSales(psku, marketplaceid, groupid, sysavgsales, fbaquantity, overseaqty,
                    reallyamount));
        } else {
            int aftersalesday =
                    item.get("aftersalesday") == null ? 0 : Integer.parseInt(item.get("aftersalesday").toString());
    ...
    public List<Map<String, Object>> ExpandCountryDataByGroup(PlanDetailDTO dto) {
        ...
        if (dto.getIseu() == null || dto.getIseu() == false) {
            ...
            for (Map<String, Object> item : planitem) {
                String key =
                        item.get("groupid").toString() + item.get("amazonauthid").toString() + item.get("marketplaceid")
                                .toString() + item.get("sku").toString() + item.get("msku").toString();
    ...
    public Integer getAfterSales(AmzProductSalesPlanShipItem item) {
        ...
        if (list != null && list.size() > 0) {
            ...
            if (item.getMarketplaceid().equals("EU")) {
                int sumseven = salesitem.get("sumseven") == null ? 0
                        : Integer.parseInt(salesitem.get("sumseven").toString());// 七日销量
                int summonth =
                        salesitem.get("summonth") == null ? 0 : Integer.parseInt(salesitem.get("summonth").toString());
                int sum15 = salesitem.get("sum15") == null ? 0 : Integer.parseInt(salesitem.get("sum15").toString());
                Date openDate =
                        salesitem.get("openDate") == null ? null : AmzDateUtils.getDate(salesitem.get("openDate"));
        ...
        List<AmzInboundFbaCycle> cycleList = iAmzInboundFbaCycleService.getInboundFbaCycle(auth.getShopId(),
                item.getMarketplaceid());
        FBAShipCycle cycle = iFBAShipCycleService.getFbaShipCycle(item.getGroupid().toString(), item.getMarketplaceid(),
                item.getSku());
        AmzInboundFbaCycle myfbacycle = iAmzInboundFbaCycleService.getDefaultInboundFbaCycle(cycleList);
        if (cycle != null && cycle.getTranstype() != null) {
            myfbacycle = iAmzInboundFbaCycleService.getTransInboundFbaCycle(cycleList, cycle.getTranstype());
        ...
        Integer shipcycle = getShipCycle(cycle, myfbacycle);
        Integer mincycle = getMinShipCycle(cycle, myfbacycle);
        Map<String, ProductInPresale> prelist = iProductInPresaleService.getPresale(item.getSku(),
                item.getMarketplaceid(), auth);
        int afterSales = 0;
        ...
        try {
            ...
            Result<Map<String, Object>> materialResult = erpClientOneFeign.getMSkuDeliveryAndInv(auth.getShopId(),
                    auth.getGroupid(), item.getMsku(), country, "false");
            if (Result.isSuccess(materialResult) && materialResult.getData() != null) {
                if (materialResult.getData().get("overseaqty") != null) {
                    overseaqty = Integer.valueOf(materialResult.getData().get("overseaqty").toString());
                }
    ...
    public Map<String, Object> getAfterSales(String sku, String marketplaceid, String groupid, int avgsales,
            int fbaInventoryQty, Integer overseaqty, int amount) {
        // TODO Auto-generated method stub
