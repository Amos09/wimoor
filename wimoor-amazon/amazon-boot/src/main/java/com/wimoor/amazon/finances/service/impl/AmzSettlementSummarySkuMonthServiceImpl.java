...
public class AmzSettlementSummarySkuMonthServiceImpl extends
        ServiceImpl<AmzSettlementSummarySkuMonthMapper, AmzSettlementSummarySkuMonth> implements
        IAmzSettlementSummarySkuMonthService {
    ...
    private Map<String, BigDecimal> findSku_LongtermStorage(Map<String, Object> map) {
        ...
        for (Map<String, Object> item : list) {
            if (item.get("sku") != null) {
                result.put(item.get("sku").toString(),
                        new BigDecimal(item.get("longTermFee") == null ? "0" : item.get("longTermFee").toString()));
            }
        }
        return result;
    ...
    private Map<String, BigDecimal> findSku_StorageFee(Map<String, Object> map) {
        ...
        for (Map<String, Object> item : list) {
            if (item.get("asin") != null) {
                result.put(item.get("asin").toString(),
                        new BigDecimal(item.get("storagefee") == null ? "0" : item.get("storagefee").toString()));
            }
        }
        return result;
    }
    ...
    private Map<String, BigDecimal> findSku_Reimbursements(Map<String, Object> param) {
        ...
        for (Map<String, Object> item : list) {
            if (item.get("sku") != null) {
                result.put(item.get("sku").toString(),
                        new BigDecimal(item.get("amount") == null ? "0" : item.get("amount").toString()));
            }
        }
        return result;
    }
    ...
    private Map<String, BigDecimal> findSku_FirstShipment(Map<String, Object> map) {
        ...
        for (Map<String, Object> item : list) {
            if (item.get("sellersku") != null) {
                result.put(item.get("sellersku").toString(),
                        new BigDecimal(item.get("skufee") == null ? "0" : item.get("skufee").toString()));
            }
        }
        return result;
    ...
    private Map<String, List<Map<String, Object>>> findSku_finItemData(Map<String, Object> map,
    ...
    private AmzSettlementSummarySkuMonth currencyExchange(
        ...
        one.setPrincipal(
                exchangeRateHandlerService.changeCurrencyByLocal(shopid, fromcurrency, tocurrency, item.getPrincipal(),
                        2));
        one.setCommission(
                exchangeRateHandlerService.changeCurrencyByLocal(shopid, fromcurrency, tocurrency, item.getCommission(),
                        2));
        one.setFbafee(
                exchangeRateHandlerService.changeCurrencyByLocal(shopid, fromcurrency, tocurrency, item.getFbafee(),
                        2));
        one.setPromotion(
                exchangeRateHandlerService.changeCurrencyByLocal(shopid, fromcurrency, tocurrency, item.getPromotion(),
                        2));
        one.setRefund(
                exchangeRateHandlerService.changeCurrencyByLocal(shopid, fromcurrency, tocurrency, item.getRefund(),
                        2));
        one.setShipping(
                exchangeRateHandlerService.changeCurrencyByLocal(shopid, fromcurrency, tocurrency, item.getShipping(),
                        2));
        one.setTax(
                exchangeRateHandlerService.changeCurrencyByLocal(shopid, fromcurrency, tocurrency, item.getTax(), 2));
        one.setOtherfee(
                exchangeRateHandlerService.changeCurrencyByLocal(shopid, fromcurrency, tocurrency, item.getOtherfee(),
                        2));
        ...
        setincome = setincome.add(one.getPrincipal())
                ...
                .add(one.getOtherfee());
        //============================================分摊店铺数据=====================================
        one.setShareStorageFee(exchangeRateHandlerService.changeCurrencyByLocal(shopid, fromcurrency, tocurrency,
                item.getShareStorageFee(), 2));
        one.setShareLongStorageFee(exchangeRateHandlerService.changeCurrencyByLocal(shopid, fromcurrency, tocurrency,
                item.getShareLongStorageFee(), 2));
        one.setShareAdvSpendFee(exchangeRateHandlerService.changeCurrencyByLocal(shopid, fromcurrency, tocurrency,
                item.getShareAdvSpendFee(), 2));
        one.setShareCouponRedemptionFee(
                exchangeRateHandlerService.changeCurrencyByLocal(shopid, fromcurrency, tocurrency,
                        item.getShareCouponRedemptionFee(), 2));
        one.setShareReserveFee(exchangeRateHandlerService.changeCurrencyByLocal(shopid, fromcurrency, tocurrency,
                item.getShareReserveFee(), 2));
        one.setShareReimbursementFee(exchangeRateHandlerService.changeCurrencyByLocal(shopid, fromcurrency, tocurrency,
                item.getShareReimbursementFee(), 2));
        one.setShareDisposalFee(exchangeRateHandlerService.changeCurrencyByLocal(shopid, fromcurrency, tocurrency,
                item.getShareDisposalFee(), 2));
        one.setShareShopOtherFee(exchangeRateHandlerService.changeCurrencyByLocal(shopid, fromcurrency, tocurrency,
                item.getShareShopOtherFee(), 2));

        BigDecimal avgprice = one.getSales() != null && one.getSales() > 0 ? one.getPrincipal()
                .divide(new BigDecimal(one.getSales()), 2, RoundingMode.HALF_UP) : new BigDecimal("0");

        //============================================报表数据=====================================
        one.setRptAdvSales(exchangeRateHandlerService.changeCurrencyByLocal(shopid, fromcurrency, tocurrency,
                item.getRptAdvSales(), 2));
        one.setRptAdvSpendFee(exchangeRateHandlerService.changeCurrencyByLocal(shopid, fromcurrency, tocurrency,
                item.getRptAdvSpendFee(), 2));
        one.setRptLongStorageFee(exchangeRateHandlerService.changeCurrencyByLocal(shopid, fromcurrency, tocurrency,
                item.getRptLongStorageFee(), 2));
        one.setRptStorageFee(exchangeRateHandlerService.changeCurrencyByLocal(shopid, fromcurrency, tocurrency,
                item.getRptStorageFee(), 2));
        one.setRptReimbursementsFee(exchangeRateHandlerService.changeCurrencyByLocal(shopid, fromcurrency, tocurrency,
                item.getRptReimbursementsFee(), 2));

        one.setRptDisposalFee(exchangeRateHandlerService.changeCurrencyByLocal(shopid, fromcurrency, tocurrency,
                item.getRptDisposalFee(), 2));
        one.setRptDisposalUnits(item.getRptDisposalUnits());

        one.setRptAdvUnits(item.getRptAdvUnits());
        //============================================本地利润计算数据=====================================
        one.setProfitCompanytax(exchangeRateHandlerService.changeCurrencyByLocal(shopid, fromcurrency, tocurrency,
                item.getProfitCompanytax(), 2));
        one.setProfitCustomstax(exchangeRateHandlerService.changeCurrencyByLocal(shopid, fromcurrency, tocurrency,
                item.getProfitCustomstax(), 2));
        one.setProfitExchangelost(exchangeRateHandlerService.changeCurrencyByLocal(shopid, fromcurrency, tocurrency,
                item.getProfitExchangelost(), 2));
        one.setProfitLostrate(exchangeRateHandlerService.changeCurrencyByLocal(shopid, fromcurrency, tocurrency,
                item.getProfitLostrate(), 2));
        one.setProfitMarketfee(exchangeRateHandlerService.changeCurrencyByLocal(shopid, fromcurrency, tocurrency,
                item.getProfitMarketfee(), 2));
        one.setProfitOtherfee(exchangeRateHandlerService.changeCurrencyByLocal(shopid, fromcurrency, tocurrency,
                item.getProfitOtherfee(), 2));
        one.setProfitVat(
                exchangeRateHandlerService.changeCurrencyByLocal(shopid, fromcurrency, tocurrency, item.getProfitVat(),
                        2));

        //============================================本地成本数据=====================================
        one.setProfitLocalShipmentfee(exchangeRateHandlerService.changeCurrencyByLocal(shopid, "CNY", tocurrency,
                item.getProfitLocalShipmentfee(), 2));
        one.setLocalPrice(
                exchangeRateHandlerService.changeCurrencyByLocal(shopid, "CNY", tocurrency, item.getLocalPrice(), 2));
        one.setLocalUnitPrice(
                exchangeRateHandlerService.changeCurrencyByLocal(shopid, "CNY", tocurrency, item.getLocalUnitPrice(),
                        2));
        one.setLocalOtherCost(
                exchangeRateHandlerService.changeCurrencyByLocal(shopid, "CNY", tocurrency, item.getLocalOtherCost(),
                        2));
        one.setLocalReturnTax(
                exchangeRateHandlerService.changeCurrencyByLocal(shopid, "CNY", tocurrency, item.getLocalReturnTax(),
                        2));
        one.setLocalShipmentItemFee(exchangeRateHandlerService.changeCurrencyByLocal(shopid, "CNY", tocurrency,
                item.getLocalShipmentItemFee(), 2));
        one.setLocalFifoShipmentFee(exchangeRateHandlerService.changeCurrencyByLocal(shopid, "CNY", tocurrency,
                item.getLocalFifoShipmentFee(), 2));
        one.setLocalFifoCost(
                exchangeRateHandlerService.changeCurrencyByLocal(shopid, "CNY", tocurrency, item.getLocalFifoCost(),
                        2));
        one.setLocalUnitPrice(
                exchangeRateHandlerService.changeCurrencyByLocal(shopid, "CNY", tocurrency, item.getLocalUnitPrice(),
                        2));
            ...
            for (Map<String, Object> fin : finlist) {//当前SKU下面各个币种的动态字段进行汇率转换和汇总到finlist中
                ...
                for (Map<String, String> itemfield : fieldlist) {
                    if (fin.get("field" + itemfield.get("value")) != null) {
                        ...
                        itemvalue = exchangeRateHandlerService.changeCurrencyByLocal(shopid, myfromcurrency, tocurrency,
                                itemvalue, 2);
                        BigDecimal value = finitem.get("field" + itemfield.get("value")) != null ? finitem.get(
                                "field" + itemfield.get("value")) : new BigDecimal("0");
    ...
    public void summaryMonthly(String amazonauthid, String marketplaceName, Date settlementStartDate) {
        ...
        for (AmzSettlementSummarySkuMonth item : list) {
            ...
            AmzSettlementSummarySkuMonth countrycurrency = currencyExchange(item, fieldlist, shopid, fromcurrency,
                    compiledExp);
    ...
    private void setExtendsData(AmzSettlementSummarySkuMonth item, Map<String, BigDecimal> storagefee,
        ...
        if (disposalFeeitem != null) {
            item.setRptDisposalFee(
                    disposalFeeitem.get("disposal") != null ? new BigDecimal(disposalFeeitem.get("disposal").toString())
                            : new BigDecimal("0"));
            item.setRptDisposalUnits(
                    disposalFeeitem.get("units") != null ? Integer.parseInt(disposalFeeitem.get("units").toString())
                            : Integer.parseInt("0"));
        } else {
        ...
        if (advfeeitem != null) {
            item.setRptAdvSpendFee(advfeeitem.get("spend") != null ? new BigDecimal(advfeeitem.get("spend").toString())
                    : new BigDecimal("0"));
            item.setRptAdvSales(
                    advfeeitem.get("totalsales") != null ? new BigDecimal(advfeeitem.get("totalsales").toString())
                            : new BigDecimal("0"));
            item.setRptAdvUnits(advfeeitem.get("units") != null ? Integer.parseInt(advfeeitem.get("units").toString())
                    : Integer.parseInt("0"));
        } else {
        ...
        if (inoutfee != null) {
            item.setLocalFifoShipmentFee(
                    inoutfee.get("transfee") != null ? new BigDecimal(inoutfee.get("transfee").toString())
                            : new BigDecimal("0"));
            item.setLocalFifoCost(inoutfee.get("cost") != null ? new BigDecimal(inoutfee.get("cost").toString())
                    : new BigDecimal("0"));
        } else {
    ...
    public void calcuteRefundRate(Map<String, Object> item) {
        if (item.get("order_amount") != null && item.get("refundorder") != null) {
            ...
            if (order_amount.floatValue() >= 0.001) {
                BigDecimal refundrate = refundorder.multiply(new BigDecimal("100"))
                        .divide(order_amount, 2, RoundingMode.HALF_UP);
                    ...
                    if (value != null) {
                        if ("avgprice".equals(titlearray[j].toString())) {
                            if (map.get("principal") != null && map.get("sales") != null) {
                                cell.setCellValue(Float.parseFloat(map.get("principal").toString()) / Float.parseFloat(
                                        map.get("sales").toString()));
                            } else {
                                cell.setCellValue("0");
                            }
                        } else if ("profitrate".equals(titlearray[j].toString())) {
                            if (map.get("profit") != null && map.get("principal") != null) {
                                cell.setCellValue(Float.parseFloat(map.get("profit").toString()) / Float.parseFloat(
                                        map.get("principal").toString()));
                            } else {
                                cell.setCellValue("0");
                            }
                        } else {
