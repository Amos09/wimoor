...
//	    		 }
        ...
        for (int step = 0; step < amazonAuthorityList.size(); step++) {
            ...
            if (list != null && list.size() > 0) {
                ...
                for (int i = 0; i < list.size(); i++) {
                    ...
                    String monthkey = item.getMarketplaceName() + "#" + item.getSettlementStartDate()
                ...
                for (String monthkey : month) {
                    ...
                    try {
                        iAmzSettlementSummarySkuMonthService.summaryMonthly(amazonAuthority.getId(), marketname,
                                GeneralUtil.getDatez(monthitme + "-01"));
                    } catch (Exception e) {
                        e.printStackTrace();
                    }
                }
    ...
    public void refreshAllByManual() {
        ...
        for (int step = 0; step < amazonAuthorityList.size(); step++) {
            ...
            if (list != null && list.size() > 0) {
                System.out.println(
                        "sellerid:" + amazonAuthority.getSellerid() + "     size:" + list.size() + "    time:"
                                + new Date());
                Set<String> month = new HashSet<String>();
                for (int i = 0; i < list.size(); i++) {
                    AmzSettlementAccReport item = list.get(i);
                ...
                for (String monthkey : month) {
                    ...
                    String monthitme = keys[1];
                    System.out.println(
                            "marketname:" + marketname + "   monthitme:" + monthitme + "    time:" + new Date());
                    try {
                        iAmzSettlementSummarySkuMonthService.summaryMonthly(amazonAuthority.getId(), marketname,
                                GeneralUtil.getDatez(monthitme + "-01"));
                    } catch (Exception e) {
    ...
    public void refreshData(ProgressHelper progressHelper, FinDataMonthDTO dto) {
        ...
        for (AmazonAuthority auth : listauth) {
            ...
            if (marketname != null) {
                ...
                List<Marketplace> markets = iMarketplaceService.selectMarketBySellerid(auth.getSellerid(),
                        auth.getShopId());
            ...
            for (String mktname : marketnames) {
                ...
                for (String monthitme : month) {
                    ...
                    progressHelper.setProgress(progress.intValue());
                    iAmzSettlementSummarySkuMonthService.summaryMonthly(auth.getId(), mktname,
                            GeneralUtil.getDatez(monthitme + "-01"));
                }
            }
        }
