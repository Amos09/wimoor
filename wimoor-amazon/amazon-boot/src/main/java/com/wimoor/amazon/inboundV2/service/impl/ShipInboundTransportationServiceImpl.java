...
//			config.setPallets(pallets);
        ...
        GenerateTransportationOptionsResponse response = iInboundApiHandlerService.generateTransportationOptions(auth,
                plan.getInboundPlanId(), request);
        if (response != null) {
    ...
    public ListTransportationOptionsResponse listTransportationOptions(TransportationDTO dto) {
        ...
        AmazonAuthority auth = amazonAuthorityService.selectByGroupAndMarket(plan.getGroupid(),
                plan.getMarketplaceid());
        ListTransportationOptionsResponse response = iInboundApiHandlerService.listTransportationOptions(auth,
    ...
    @Override
    public ShipInboundOperation confirmTransportationOptions(String formid,
        ...
        ConfirmTransportationOptionsResponse response = iInboundApiHandlerService.confirmTransportationOptions(auth,
    ...
    public void saveShipmentsFee(ShipInboundShipment shipment, ShipInboundTrans transfee, UserInfo user) {
        //获取分摊费用 存起来
        List<Map<String, Object>> itemlist = shipInboundShipmentItemV2Mapper.selectByShipmentid(
                shipment.getShipmentid());
        BigDecimal totalWeight = new BigDecimal("0");
        BigDecimal totalFee = new BigDecimal("0");
        for (Map<String, Object> item : itemlist) {
            BigDecimal weight =
                    item.get("weight") != null ? new BigDecimal(item.get("weight").toString()) : new BigDecimal("0");
            BigDecimal volume =
                    item.get("volume") != null ? new BigDecimal(item.get("volume").toString()) : new BigDecimal("0");
        ...
        for (Map<String, Object> feeitem : itemlist) {
            BigDecimal maxWeight =
                    feeitem.get("maxWeight") != null ? new BigDecimal(feeitem.get("maxWeight").toString())
                            : new BigDecimal("0");
            BigDecimal price = feeitem.get("price") != null ? new BigDecimal(feeitem.get("price").toString())
                    : new BigDecimal("0");
            BigDecimal priceWavg =
                    feeitem.get("price_wavg") != null ? new BigDecimal(feeitem.get("price_wavg").toString()) : price;
