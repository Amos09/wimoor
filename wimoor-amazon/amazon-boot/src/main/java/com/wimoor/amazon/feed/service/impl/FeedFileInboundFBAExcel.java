...
public class FeedFileInboundFBAExcel {
    ...
    public static ByteArrayOutputStream getFile(ShipInboundPlan plan, ShipInboundShipment shipment,
            List<ShipInboundItemVo> itemlist, List<ShipInboundBox> boxlist, List<ShipInboundCase> boxcase,
            String sellerid) {
                ...
                if (i == 0) {
                    ...
                    for (int j = 0; j < boxlist.size(); j++) {
                        ...
                        for (int z = 0; z < boxcase.size(); z++) {
                            if (boxcase.get(z).getMerchantsku().equals(skuinfo.getSellersku())
                                    && boxlist.get(j).getBoxnum() == boxcase.get(z).getNumberofcase()) {
                                if (plan.getArecasesrequired() == false) {
                                    if (boxcase.get(z).getQuantity() == null || boxcase.get(z).getQuantity() == 0) {
                                        ...
                                        cell.setCellType(CellType.NUMERIC);
                                        cell.setCellValue(boxcase.get(z).getQuantity());
                                        hasvalue = true;
                                        break;
                                    }
                                } else {
                                    if (boxcase.get(z).getNumberofcase() == null
                                            || boxcase.get(z).getNumberofcase() == 0) {
                                        cell.setCellType(CellType.NUMERIC);
                                        cell.setCellValue(0);
                                        hasvalue = true;
                                        break;
                ...
                if (i > (3 + itemlist.size() + 2)) {
                    ...
                    if (StrUtil.isNotEmpty(cellValue)) {
                        if (cellValue.contains("${center}")) {
                            cell.setCellValue(
                                    cellValue.replace("${center}", shipment.getDestinationfulfillmentcenterid()));
                        }
