 ...
 */
@Service
public class AmzNotificationsDestinationServiceImpl extends
        ServiceImpl<AmzNotificationsDestinationMapper, AmzNotificationsDestination> implements
        IAmzNotificationsDestinationService {
                ...
                if (old != null) {
                    ...
                    if (resource.getEventBridge() != null) {
                        if (resource.getEventBridge().getAccountId() != null) {
                            if (old.getResourceEventBridgeAccountid() == null || !old.getResourceEventBridgeAccountid()
                                    .equals(resource.getEventBridge().getAccountId())) {
                                old.setResourceEventBridgeAccountid(resource.getEventBridge().getAccountId());
                                needupdate = true;
                            }
                        }
                        if (resource.getEventBridge().getRegion() != null) {
                            if (old.getResourceEventBridgeRegion() == null || !old.getResourceEventBridgeRegion()
                                    .equals(resource.getEventBridge().getRegion())) {
                                old.setResourceEventBridgeRegion(resource.getEventBridge().getRegion());
                                needupdate = true;
                            }
                        }
                        if (resource.getEventBridge().getName() != null) {
                            if (old.getResourceEventBridgeName() == null || old.getResourceEventBridgeName()
                                    .equals(resource.getEventBridge().getName())) {
