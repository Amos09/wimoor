...
public class SalesSummaryController {
    ...
    public Result<List<Map<String, String>>> getOrderSumFieldAction(@RequestBody SalesDTO dto) {
        ...
        parameter.put("search",
                dto.getSearch() != null && !dto.getSearch().isEmpty() ? "%" + dto.getSearch().trim() + "%" : null);
        parameter.put("marketplaceid",
                dto.getMarketplace() != null && !dto.getMarketplace().isEmpty() ? dto.getMarketplace().trim() : null);
        ...
        if ("day".equals(dto.getSummaryType())) {
            if (days > 90) {
                if (StrUtil.isEmpty(search)) {
                    ...
                    parameter.put("search",
                            search != null && !search.isEmpty() ? search.trim().replaceAll("%", "") : null);
                }
            } else {
