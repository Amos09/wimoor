...
public class ProductListingsItemServiceImpl implements IProductListingsItemService {
    ...
    public Item captureListMatchingProduct(AmazonAuthority amazonAuthority, String sku, List<String> marketplaces,
            String issueLocale, List<String> includedData) {
        // TODO Auto-generated method stub
        ListingsApi api = apiBuildService.getProductApi(amazonAuthority);
        try {
            Item item = api.getListingsItem(amazonAuthority.getSellerid(), sku, marketplaces, issueLocale,
                    includedData);
            return item;
    ...
    @Override
    public Call captureListMatchingProductSync(AmazonAuthority amazonAuthority, AmzProductRefresh amzProductRefresh,
        ...
        try {
            if (amazonAuthority.apiNotRateLimit()) {
                ...
                Call item = api.getListingsItemAsync(amazonAuthority.getSellerid(), amzProductRefresh.getSku(),
                        marketplaces, null, includedData, callback);
                return item;
            }
        } catch (ApiException e) {
            // TODO Auto-generated catch block
    ...
    public void handlerResult(Item result, AmazonAuthority amazonAuthority, AmzProductRefresh mrefresh) {
        ...
        if (result != null && result.getSummaries() != null && result.getSummaries().size() > 0) {
            ...
            for (ItemSummaryByMarketplace summary : result.getSummaries()) {
                ProductInfo product = getProductInfo(summary.getMarketplaceId(), result.getSku(),
                        amazonAuthority.getId(), summary.getAsin());
                    ...
                    if (summary.getMainImage() != null && summary.getMainImage().getLink() != null) {
                        if (product.getImage() != null) {
                            ...
                            if (picture == null
                                    ...
                                    || (product.getLastupdate() != null && !(product.getLastupdate()
                                    .compareTo(AmzDateUtils.getDate(summary.getLastUpdatedDate())) == 0))
                            ) {
                                if (picture != null && picture.getUrl() != null && !picture.getUrl()
                                        .equals(summary.getMainImage().getLink())) {
                                    String path = PictureServiceImpl.productImgPath + amazonAuthority.getShopId() + "/"
                                            + amazonAuthority.getId() + "/" + summary.getMarketplaceId() + "/";
                                    picture = pictureService.downloadPicture(summary.getMainImage().getLink(), path,
                                            product.getImage().toString());
                                    String manufacturer =
                                            product.getManufacturer() != null ? product.getManufacturer() : "";
                                    product.setManufacturer(manufacturer + "-i");
                                }
                                if (picture != null && picture.getOpttime() != null
                                        && GeneralUtil.distanceOfDay(picture.getOpttime(), new Date()) < 3) {
                                    //do nothing
                                } else {
                                    String path = PictureServiceImpl.productImgPath + amazonAuthority.getShopId() + "/"
                                            + amazonAuthority.getId() + "/" + summary.getMarketplaceId() + "/";
                ...
                if ((product.getAsin() == null && summary.getAsin() != null)
                        ...
                        || (product.getPcondition() == null && summary.getConditionType() != null
                        && summary.getConditionType().getValue() != null)
                        ...
                        .equals(EmojiFilterUtils.filterEmoji(summary.getItemName())))
    ...
    public ProductInfo pushAsin(ProductListingItemDTO dto) {
        ...
        List<ProductInfo> info = iProductInfoService.selectBySku(dto.getSku(), market.getMarketplaceid(),
                amazonAuthority.getId());
        ...
        try {
            resp = api.putListingsItem(amazonAuthority.getSellerid(), dto.getSku(), dto.getMarketplaceids(), body,
                    null);
        } catch (ApiException e) {
            // TODO Auto-generated catch block
            e.printStackTrace();
    ...
    public Object deleteAsin(ProductListingItemDTO dto) {
        // TODO Auto-generated method stub
        AmazonAuthority amazonAuthority = amazonAuthorityService.getById(dto.getAmazonauthid());
        List<ProductInfo> oldinfo = iProductInfoService.selectBySku(dto.getSku(), dto.getMarketplaceids().get(0),
                amazonAuthority.getId());
        ProductInfo infoone = null;
        ...
        try {
            response = api.deleteListingsItem(amazonAuthority.getSellerid(), dto.getSku(), dto.getMarketplaceids(),
                    null);
        } catch (ApiException e) {
            // TODO Auto-generated catch block
            e.printStackTrace();
    ...
    public ProductInfo saveAsin(ProductListingItemDTO dto) {
        ...
        List<ProductInfo> oldinfo = iProductInfoService.selectBySku(dto.getSku(), market.getMarketplaceid(),
                amazonAuthority.getId());
        if (oldinfo != null && oldinfo.size() > 0) {
            info = oldinfo.get(0);
            ...
            if (productInOpt != null) {
                ...
                if (dto.getImage() != null) {
                    String path = PictureServiceImpl.productImgPath + amazonAuthority.getShopId() + "/"
                            + amazonAuthority.getId() + "/" + info.getMarketplaceid() + "/";
                    try {
            ...
            if (dto.getImage() != null) {
                String path =
                        PictureServiceImpl.productImgPath + amazonAuthority.getShopId() + "/" + amazonAuthority.getId()
                                + "/" + info.getMarketplaceid() + "/";
                try {
                    picture = pictureService.downloadPicture(dto.getImage(), path, null);
                } catch (Exception e) {
                    // TODO Auto-generated catch block
            ...
            new Thread(new Runnable() {
                ...
                public void run() {
                    // TODO Auto-generated method stub
                    iProductCatalogItemService.captureCatalogProductSync(amazonAuthority, refresh,
                            dto.getMarketplaceids());
                }
    ...
    public ListingsItemSubmissionResponse changePriceSku(ProductListingItemDTO dto) {
        ...
        ListingsItemSubmissionResponse resp = changePriceSku(dto.getUserid(), amazonAuthority, market, info_one, opt,
                dto.getPrice(), dto.getFromDate(), dto.getToDate());
        boolean haserror = false;
        if (resp != null && !resp.getIssues().isEmpty() && resp.getIssues().size() > 0) {
            for (Issue issues : resp.getIssues()) {
                if (issues.getSeverity().equals(SeverityEnum.ERROR)) {
    ...
    @CacheEvict(value = {"ProductInfoCache"}, allEntries = true)
    public ListingsItemSubmissionResponse changePriceSku(String userid, AmazonAuthority amazonAuthority,
            Marketplace market, ProductInfo info, ProductInOpt opt, String newprice, String fromDate, String toDate) {
        // TODO Auto-generated method stub
        ListingsApi api = apiBuildService.getProductApi(amazonAuthority);
        ListingsItemSubmissionResponse resp = null;
        try {
            ...
            resp = api.patchListingsItem(amazonAuthority.getSellerid(), info.getSku(),
                    Arrays.asList(market.getMarketplaceid()), body, null);
    ...
    @CacheEvict(value = {"ProductInfoCache"}, allEntries = true)
    public ListingsItemSubmissionResponse changeShipTemplate(String userid, AmazonAuthority amazonAuthority,
            Marketplace market, ProductInfo info, ProductInOpt opt) {
        // TODO Auto-generated method stub
        ListingsApi api = apiBuildService.getProductApi(amazonAuthority);
        ListingsItemSubmissionResponse resp = null;
        try {
    ...
    public Result<?> updateShipGroupName(AmazonAuthority amazonAuthority, ProductInfo info, Marketplace marketplace,
        ...
        if (productXml != null && amazonAuthority != null && user != null) {
            response = submitfeedService.SubmitFeedQueue(productXml, "Product" + info.getSku(), amazonAuthority,
                    FeedFileInventoryXML.type, user, null);
        } else {
