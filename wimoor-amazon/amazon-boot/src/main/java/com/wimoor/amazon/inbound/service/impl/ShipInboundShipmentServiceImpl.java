...
public class ShipInboundShipmentServiceImpl extends
        ServiceImpl<ShipInboundShipmentMapper, ShipInboundShipment> implements IShipInboundShipmentService {
    ...
    public IPage<ShipInboundShipmenSummarytVo> findByTraceCondition(ShipInboundShipmenSummaryDTO dto) {
        ...
        if (dto.getAuditstatus() != null && dto.getAuditstatus().equals("7")) {
            ...
            for (Map<String, Object> cent : centlist) {
                centnumber.put(cent.get("centerId").toString(),
                        cent.get("num") != null ? cent.get("num").toString() : "0");
            }
        }
        ...
        for (ShipInboundShipmenSummarytVo item : list.getRecords()) {
            ...
            if (item.getArrivalTime() != null) {
                if (item.getArrivalTime().after(new Date())) {
                    Integer delayDays = (
                            Double.valueOf(GeneralUtil.distanceOfDay(item.getArrivalTime(), new Date())).intValue()
                                    * -1);
                    item.setDelayDays(delayDays);
                } else {
                    Integer delayDays = Double.valueOf(GeneralUtil.distanceOfDay(item.getArrivalTime(), new Date()))
                            .intValue();
                    item.setDelayDays(delayDays);
                }
            }
    ...
    public String createInboundShipment(ShipInboundShipment shipment) {
        ...
        AmazonAuthority auth = amazonAuthorityService.selectByGroupAndMarket(inplan.getAmazongroupid(),
                inplan.getMarketplaceid());
    ...
    private void updateItemList(AmazonAuthority auth, Marketplace market, ShipInboundPlan inplan,
            List<ShipInboundItem> itemlist) {
        ...
        if (oldshipment.getStatus() != shipment.getStatus()) {
            ...
            switch (shipment.getStatus()) {
                case 0:
                    ...
                    break;
                case 1:
                    shipment.setStatus1date(new Date());
                    break;
                case 2:
    ...
    public ShipInboundShipment saveCartShipment(UserInfo user, ShipCartShipmentDTO dto) {
        ...
        if (dto.getOpttype() != null && "submit".equals(dto.getOpttype())) {
            ...
            ByteArrayOutputStream stream = FeedFileInboundFBAExcel.getFile(inplan, shipment, itemlist,
                    dto.getBoxListDetail(), dto.getCaseListDetail(), auth.getSellerid());
            //判断状态，上一条请求正在处理中，请稍后再提交;如果请求已经完成，可再次提交。
            AmzSubmitFeedQueue submitfeed = submitfeedService.HandlerFeedQueue(stream, shipment.getShipmentid(), auth,
                    FeedFileInboundFBAExcel.type, user);
            if (submitfeed != null) {
                shipment.setSubmissionidExcel(submitfeed.getId());
                shipment.setFeedstatus("waiting");
            }
            ByteArrayOutputStream stream2 = FeedFileInboundFBAXML.getFile(shipment, auth.getSellerid(),
                    dto.getCaseListDetail());
            AmzSubmitFeedQueue submitfeedXML = submitfeedService.HandlerFeedQueue(stream2, shipment.getShipmentid(),
                    auth, FeedFileInboundFBAXML.type, user);
    ...
    public String getLabelUrl(String shipmentid, String pagetype, String labelType, String pannum) {
        ...
        return iFulfillmentInboundService.getLabelDownloadUR(auth, shipment.getShipmentid(), shipment.getBoxnum(),
                pagetype, labelType, pannum);
    }
    ...
    @Override
    public ShipInboundShipmentDTO syncShipment(String groupid, String marketplaceid, String shipmentid,
            String warehouseid) {
        // TODO Auto-generated method stub
        Marketplace marketPlace = marketplaceService.findMapByMarketplaceId().get(marketplaceid);
        AmazonAuthority auth = amazonAuthorityService.selectByGroupAndMarket(groupid, marketplaceid);
        InboundShipmentList inboundShipmentList = iFulfillmentInboundService.listShipment(auth, marketPlace,
                Arrays.asList(shipmentid));
        if (inboundShipmentList.size() > 0) {
            InboundShipmentInfo ship = inboundShipmentList.get(0);
            ShipInboundShipment shipment = shipInboundPlanService.createShipment(auth, marketPlace, ship, warehouseid,
                    null);
            List<ShipInboundItem> itemlist = iFulfillmentInboundService.syncItemsByShipmentId(auth, marketPlace,
                    shipment);
    ...
    @Override
    public ShipInboundShipmenSummarytVo getUnSyncShipment(String groupid, String marketplaceid, String shipmentid,
            String warehouseid) {
        // TODO Auto-generated method stub
        Marketplace marketPlace = marketplaceService.findMapByMarketplaceId().get(marketplaceid);
        AmazonAuthority auth = amazonAuthorityService.selectByGroupAndMarket(groupid, marketplaceid);
        ...
        if (inboundShipmentList.size() > 0) {
            InboundShipmentInfo ship = inboundShipmentList.get(0);
            ShipInboundShipment shipment = shipInboundPlanService.convertShipmentPojo(auth, marketPlace, ship,
                    warehouseid, null);
            ShipInboundShipment oldshipment = this.getById(shipment.getShipmentid());
            ShipInboundPlan plan = shipment.getInboundplan();
            if (oldshipment == null) {
            ...
            shipInboundPlanService.updateById(plan);
            List<ShipInboundItem> itemlist = iFulfillmentInboundService.getUnSyncItemsByShipmentId(auth, marketPlace,
                    shipment);
            plan.setSkunum(itemlist.size());
            ...
            itemlist.stream().forEach(item -> {
                ...
                Map<String, Object> map = iProductInfoService.findNameAndPicture(item.getSellersku(),
                        market.getMarketplaceid(), groupid);
    ...
    public void setExcelBookShipmentLabel(String shopid, SXSSFWorkbook workbook, String shipmentid,
        ...
        try {
            ...
            for (int i = 0; i < skulist.size(); i++) {
                if (skulist.get(i).get("fnsku") == null || skulist.get(i).get("quantity") == null) {
                    ...
                    String quantity =
                            skulist.get(i).get("quantity") == null ? "0" : skulist.get(i).get("quantity").toString();
    ...
    public void validateShipment(String shipmentid) {
        ...
        AmazonAuthority auth = amazonAuthorityService.selectByGroupAndMarket(plan.getAmazongroupid(),
                plan.getMarketplaceid());
        if (auth != null) {
            auth.setMarketPlace(marketPlace);
            List<InboundShipmentItem> itemlist = iFulfillmentInboundService.requestInboundShipmentsItem(auth,
                    shipmentid, marketPlace);
            InboundShipmentInfo shipmentReponse = iFulfillmentInboundService.requestInboundShipments(auth, shipmentid,
                    marketPlace);
            ...
            if (shipmentReponse != null && shipmentReponse.getShipmentStatus() != null) {
                if (!shipmentReponse.getShipmentStatus().getValue()
                    ...
                    throw new BizException("状态：" + shipment.getShipmentstatus() + "与亚马逊货件状态："
                            + shipmentReponse.getShipmentStatus() + "不匹配！");
                }
                if (!shipmentReponse.getShipmentStatus().equals(ShipmentStatus.CANCELLED)
                        && (shipment.getStatus() <= 0)) {
                    throw new BizException(
                            "本地货件取消货驳回与亚马逊货件状态：" + shipmentReponse.getShipmentStatus() + "不匹配！");
                }
            }
            if (amzaddress.getAddressLine1() != null
                ...
                throw new BizException(
                        "地址：" + sysaddress.getAddressline1() + "与亚马逊货件地址：" + amzaddress.getAddressLine1()
                                + "不匹配！");
            }
            if (amzaddress.getAddressLine2() != null
                    && StrUtil.isNotBlank(amzaddress.getAddressLine2().trim())
                    && !amzaddress.getAddressLine2()
                    .equals(sysaddress.getAddressline2().replace("\r", "").replace("\n", ""))) {
                throw new BizException(
                        "地址：" + sysaddress.getAddressline2() + "与亚马逊货件地址：" + amzaddress.getAddressLine2()
                                + "不匹配！");
            }
            if (amzaddress.getCity() != null
                ...
                throw new BizException(
                        "地址：" + sysaddress.getCity() + "与亚马逊货件地址：" + amzaddress.getCity() + "不匹配！");
            }
            if (amzaddress.getCountryCode() != null
                ...
                throw new BizException(
                        "地址：" + sysaddress.getCountrycode() + "与亚马逊货件地址：" + amzaddress.getCountryCode()
                                + "不匹配！");
            }
            if (amzaddress.getDistrictOrCounty() != null
                ...
                throw new BizException("地址：" + sysaddress.getDistrictorcounty() + "与亚马逊货件地址："
                        + amzaddress.getDistrictOrCounty() + "不匹配！");
            }
            if (amzaddress.getPostalCode() != null
                ...
                throw new BizException(
                        "地址：" + sysaddress.getPostalcode() + "与亚马逊货件地址：" + amzaddress.getPostalCode()
                                + "不匹配！");
            }
            if (amzaddress.getStateOrProvinceCode() != null
                ...
                throw new BizException("地址：" + sysaddress.getStateorprovincecode() + "与亚马逊货件地址："
                        + amzaddress.getStateOrProvinceCode() + "不匹配！");
            }
            ...
            if (itemlist != null && itemlist.size() > 0) {
                ...
                for (int i = 0; i < sysitemlist.size(); i++) {
                    ...
                    if (item != null && item.getQuantityshipped() > 0) {
                        ...
                        if (sysitem != null && sysitem.getSellerSKU().equals(sku) && sysitem.getQuantityShipped() > 0
                                && num > 0) {
                            isshow = true;
    ...
    public Integer refreshShipmentRec(String shipmentid) {
        ...
        ShipInboundShipmentDTO ship = this.syncShipment(plan.getAmazongroupid(), plan.getMarketplaceid(), shipmentid,
                null);
        Integer result = 0;
    ...
    @Transactional
    public void saveTransTrance(UserInfo user, ShipInboundShipment shipment, String carrier, String transtyle,
        ...
        try {
            ...
            if (StrUtil.isNotBlank(qid)) {
                ...
                ByteArrayOutputStream stream = FeedFileInboundFBAExcel.getFile(inplan, shipment, volist, boxlist,
                        caselist, auth.getSellerid());
                AmzSubmitFeedQueue submitfeed = submitfeedService.HandlerFeedQueue(stream, shipment.getShipmentid(),
                        auth, FeedFileInboundFBAExcel.type, user);
                if (submitfeed != null) {
                    shipment.setSubmissionidExcel(submitfeed.getId());
                    shipment.setFeedstatus("waiting");
                    this.updateById(shipment);
    ...
    public String saveMarkshiped(UserInfo user, String shipmentid, String operateType) {
        ...
        try {
            InboundShipmentList shipmentlist = this.iFulfillmentInboundService.listShipment(auth, market,
                    Arrays.asList(shipmentid));
    ...
    public void refreshTransportDetails(String shipmentid) {
        // TODO Auto-generated method stub
        ShipInboundShipment shipment = this.baseMapper.selectById(shipmentid);
        if (shipment == null || shipment.getTranstyle() == null) {
            return;
        }
        ShipInboundPlan inplan = shipInboundPlanService.getById(shipment.getInboundplanid());
        AmazonAuthority auth = amazonAuthorityService.selectByGroupAndMarket(inplan.getAmazongroupid(),
        ...
        if (shipment.getTranstyle().equals("SP")) {
            if (trans.getTransportDetails() != null
                    && trans.getTransportDetails().getNonPartneredSmallParcelData() != null) {
                ...
                if (splist != null) {
                    for (int i = 0; i < boxlist.size(); i++) {
                        ...
                        if (spboxlist != null && spboxlist.size() > i) {
                            ...
                            if (spstatus.getTrackingId() != null && spstatus.getTrackingId().indexOf("FBA") != 0
                                    && !spstatus.getTrackingId().equals("123456")) {
