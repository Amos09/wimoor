...
public class ShipInboundBoxServiceImpl extends ServiceImpl<ShipInboundBoxV2Mapper, ShipInboundBox> implements
        IShipInboundBoxService {
    ...
    public Boolean hasSubmitPackage(ShipInboundPlan inplan) {
        ...
        ShipInboundOperation operation = iShipInboundOperationService.getOperation(auth, inplan.getId(),
    ...
    public ShipInboundOperation generatePackingOptions(String formid) {
        ...
        if (plan != null) {
            ...
            GeneratePackingOptionsResponse response = iInboundApiHandlerService.generatePackingOptions(auth,
    ...
    public void savePackingInformation(ShipCartDTO dto, UserInfo user) {
        ...
        for (ShipInboundBox box : list) {
            shipInboundCaseV2Service.remove(
                    new LambdaQueryWrapper<ShipInboundCase>().eq(ShipInboundCase::getBoxid, box.getId()));
    ...
    public Map<String, Object> listPackingOptions(PackingDTO dto) {
        ...
        if (plan != null && StrUtil.isNotBlank(plan.getInboundPlanId())) {
            ...
            AmazonAuthority auth = amazonAuthorityService.selectByGroupAndMarket(plan.getGroupid(),
                    plan.getMarketplaceid());
            ListPackingOptionsResponse response = iInboundApiHandlerService.listPackingOptions(auth,
                    plan.getInboundPlanId(), dto.getPageSize(), dto.getPaginationToken());
            if (response != null) {
                Map<String, Object> map = new HashMap<String, Object>();
    ...
    public ShipInboundOperation setPackingInformation(ShipInboundPlan plan, UserInfo user,
        ...
        if (isshipment) {
            List<ShipInboundShipment> shipments = shipInboundShipmentService.lambdaQuery()
                    .eq(ShipInboundShipment::getFormid, plan.getId()).list();
        ...
        for (Entry<String, List<ShipInboundBox>> entry : map.entrySet()) {
            ...
            for (ShipInboundBox box : boxlist) {
                ...
                List<ShipInboundCase> caselist = iShipInboundCaseService.lambdaQuery()
                        .eq(ShipInboundCase::getBoxid, box.getId()).list();
        ...
        SetPackingInformationResponse response = iInboundApiHandlerService.setPackingInformation(auth,
    ...
    public ShipInboundOperation confirmPackingOption(PackingDTO dto) {
        ...
        ConfirmPackingOptionResponse response = iInboundApiHandlerService.confirmPackingOption(auth,
                plan.getInboundPlanId(), dto.getPackingOptionId());
        if (response != null) {
