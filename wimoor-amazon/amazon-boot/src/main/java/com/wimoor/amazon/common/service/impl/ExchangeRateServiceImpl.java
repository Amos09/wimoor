...
public class ExchangeRateServiceImpl extends ServiceImpl<ExchangeRateMapper, ExchangeRate> implements
    ...
    public void saveMyCurrencyRate(UserInfo user, ExchangeRateCustomer rate) {
        ...
        if (!rate.idIsNULL()) {
            oldrate = exchangeRateCustomerMapper.selectById(rate.getId());
        }
        if (oldrate == null) {
