    ...
    public String determineProductTier(String country, InputDimensions dimensions, String isMedia) throws BizException {
        ...
        for (int i = 0; i < tierList.size(); i++) {
            ...
            if (result) {
                ...
                outboundWeight = outboundWeight.divide(new BigDecimal("0.5")).setScale(0, RoundingMode.CEILING)
                        .multiply(new BigDecimal("0.5"));//进位取整数，模拟不足500g按500g算；
                break;
            }
        }
        // 第二次用带包装重量重新计算product tier
        dimensions = new InputDimensions(length, width, height, getDimUnit(country), outboundWeight,
