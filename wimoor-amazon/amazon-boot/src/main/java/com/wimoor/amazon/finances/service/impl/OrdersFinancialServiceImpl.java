 ...
 */
@Service
public class OrdersFinancialServiceImpl extends ServiceImpl<OrdersFinancialMapper, OrdersFinancial> implements
        IOrdersFinancialService {
    ...
    public List<OrdersFinancial> getOrdersFinancialList(AmazonAuthority amazonAuthority, String orderid) {
        ...
        LambdaQueryWrapper<OrdersFinancial> query = new LambdaQueryWrapper<OrdersFinancial>().eq(
                OrdersFinancial::getAmazonOrderId, orderid);
    ...
    public List<OrdersFinancial> saveFinancialData(List<?> chargeList, Date postdate, ShipmentItem item,
        ...
        if (chargeList != null) {
            for (int k = 0; k < chargeList.size(); k++) {
                ...
                if (amzOrderFinancial.getAmount() == null || amzOrderFinancial.getAmount()
    ...
    public List<OrdersFinancial> setFinancialByOrderId(AmazonAuthority amazonAuthority, String orderid,
            String nexttoken) {
                ...
                for (ShipmentItem shipmentItem : itemlist) {
                    ...
                    for (Object key : map.keySet()) {
                        if (key.toString().contains("List")) {
                            ...
                            List<OrdersFinancial> msublist = saveFinancialData(chargeList, postedDate, shipmentItem,
                                    orderid);
