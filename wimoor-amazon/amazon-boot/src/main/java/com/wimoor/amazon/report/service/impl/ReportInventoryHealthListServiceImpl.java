        ...
        try {
            while ((line = br.readLine()) != null) {
                ...
                if (lineNumber != 0) {
                    ...
                    snapshot_date = info[0];
                    fnsku = info[2];
                    name = info[4];
                    condition = info[5];
                    if (condition.contains("No Listing") || condition.contains("Unknown")) {
                        continue;
                    }
                    sales_rank = info[6];
                    product_group = info[7];
                    total_quantity = info[8];
                    sellable_quantity = info[9];
                    unsellable_quantity = info[10];
                    inv_age_0_to_90_days = info[11];
                    inv_age_91_to_180_days = info[12];
                    inv_age_181_to_270_days = info[13];
                    inv_age_271_to_365_days = info[14];
                    inv_age_365_plus_days = info[15];
                    units_shipped_last_24_hrs = info[16];
                    units_shipped_last_7_days = info[17];
                    units_shipped_last_30_days = info[18];
                    units_shipped_last_90_days = info[19];
                    units_shipped_last_180_days = info[20];
                    units_shipped_last_365_days = info[21];
                    weeks_of_cover_t7 = info[22];
                    weeks_of_cover_t30 = info[23];
                    weeks_of_cover_t90 = info[24];
                    weeks_of_cover_t180 = info[25];
                    weeks_of_cover_t365 = info[26];
                    num_afn_new_sellers = info[27];
                    num_afn_used_sellers = info[28];
                    currency = info[29];
                    your_price = info[30];
                    sales_price = info[31];
                    lowest_afn_new_price = info[32];
                    lowest_afn_used_price = info[33];
                    lowest_mfn_new_price = info[34];
                    lowest_mfn_used_price = info[35];
                    qty_to_be_charged_ltsf_12_mo = info[36];
                    qty_in_long_term_storage_program = info[37];
                    qty_with_removals_in_progress = info[38];
                    projected_ltsf_12_mo = info[39];
                    per_unit_volume = info[40];
                    is_hazmat = info[41];
                    in_bound_quantity = info[42];
                    asin_limit = info[43];
                    ...
                    newhealth.setAsin(asin);
                    newhealth.setAsinLimit(asin_limit);
                    newhealth.setFcondition(condition);
                    newhealth.setShopid(amazonAuthority.getShopId());
                    newhealth.setCurrency(currency);
                    newhealth.setFnsku(fnsku);
                    newhealth.setInBoundQuantity(in_bound_quantity);
                    newhealth.setInboundRecommendQuantity(inbound_recommend_quantity);
                    newhealth.setInvAge0to90days(Integer.parseInt(inv_age_0_to_90_days));
                    newhealth.setInvAge91to180days(Integer.parseInt(inv_age_91_to_180_days));
                    newhealth.setInvAge181to270days(Integer.parseInt(inv_age_181_to_270_days));
                    newhealth.setInvAge271to365days(Integer.parseInt(inv_age_271_to_365_days));
                    newhealth.setInvAge365plusdays(Integer.parseInt(inv_age_365_plus_days));
                    newhealth.setIsHazmat(is_hazmat);
                    newhealth.setLowestAfnNewPrice(lowest_afn_new_price);
                    newhealth.setLowestAfnUsedPrice(lowest_afn_used_price);
                    newhealth.setLowestMfnNewPrice(lowest_mfn_new_price);
                    newhealth.setLowestMfnUsedPrice(lowest_mfn_used_price);
                    newhealth.setName(name);
                    newhealth.setNumAfnNewSellers(num_afn_new_sellers);
                    newhealth.setNumAfnUsedSellers(num_afn_used_sellers);
                    newhealth.setPerUnitVolume(per_unit_volume);
                    newhealth.setProductGroup(product_group);
                    newhealth.setProjectedLtsf12Mo(projected_ltsf_12_mo);
                    newhealth.setProjectedLtsf6mo(projected_ltsf_6_mo);
                    newhealth.setQtyInLongTermStorageProgram(qty_in_long_term_storage_program);
                    newhealth.setQtyToBeChargedLtsf12mo(qty_to_be_charged_ltsf_12_mo);
                    newhealth.setQtyToBeChargedLtsf6mo(qty_to_be_charged_ltsf_6_mo);
                    newhealth.setQtyWithRemovalsInProgress(qty_with_removals_in_progress);
                    newhealth.setUnitsShippedLast180days(units_shipped_last_180_days);
                    newhealth.setUnitsShippedLast24hrs(units_shipped_last_24_hrs);
                    newhealth.setUnitsShippedLast30days(units_shipped_last_30_days);
                    newhealth.setUnitsShippedLast365days(units_shipped_last_365_days);
                    newhealth.setUnitsShippedLast7days(units_shipped_last_7_days);
                    newhealth.setUnitsShippedLast90days(units_shipped_last_90_days);
                    newhealth.setUnsellableQuantity(unsellable_quantity);
                    newhealth.setWeeksOfCoverT180(weeks_of_cover_t180);
                    newhealth.setWeeksOfCoverT30(weeks_of_cover_t30);
                    newhealth.setWeeksOfCoverT365(weeks_of_cover_t365);
                    newhealth.setWeeksOfCoverT7(weeks_of_cover_t7);
                    newhealth.setWeeksOfCoverT90(weeks_of_cover_t90);
                    newhealth.setYourPrice(your_price);
                    newhealth.setSalesRank(sales_rank);
                    newhealth.setTotalQuantity(total_quantity);
                    newhealth.setSku(sku);
                    newhealth.setSalesPrice(sales_price);
                    newhealth.setSellableQuantity(sellable_quantity);
                    newhealth.setAsinLimit(asin_limit);
                    newhealth.setSnapshotDate(DateUtil.parse(snapshot_date));
