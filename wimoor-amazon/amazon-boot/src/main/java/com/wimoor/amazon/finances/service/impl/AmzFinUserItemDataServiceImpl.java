 ...
 */
@Service
public class AmzFinUserItemDataServiceImpl extends ServiceImpl<AmzFinUserItemDataMapper, AmzFinUserItemData> implements
        IAmzFinUserItemDataService {
    ...
    public int saveFinItemData(AmzFinUserItemData amzFinUserItemData) {
        ...
        if (!isnew) {
            if (oldAmzFinUserItemDataList == null || oldAmzFinUserItemDataList.size() == 0
                    || oldAmzFinUserItemDataList.get(0).getId().equals(amzFinUserItemData.getId())) {
                return this.baseMapper.updateById(amzFinUserItemData);
            } else {
                throw new BizException(
                        amzFinUserItemData.getSku() + "在" + GeneralUtil.formatDate(amzFinUserItemData.getByday())
                                + "已经有了一笔记录了！");
            }
        } else {
            if (oldAmzFinUserItemDataList != null && oldAmzFinUserItemDataList.size() > 0) {
