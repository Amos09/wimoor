...
public class ShipInboundPlanQuotaV2Controller {
    ...
    public Result<Integer> checkinvAction(@PathVariable("checkid") String checkid,
            @RequestBody List<String> inboundformids) {
        Integer i = 0;
    ...
    public void downExcelLabelAction(
        ...
        try {
            ...
            // 将数据写入Excel
            shipInboundPlanQuotaV2Service.setExcelBookShipmentLabel(user.getCompanyid(), workbook,
                    shipInboundPlanQuotaV2Service.getLabelValue(dto));
            workbook.write(fOut);
            workbook.close();
            fOut.flush();
            fOut.close();
    ...
    public void downPDFLabelAction(
        ...
        try {
            PdfWriter writer = PdfWriter.getInstance(document, response.getOutputStream());
            shipInboundPlanQuotaV2Service.setPDFDocLabel(document, shipInboundPlanQuotaV2Service.getLabelValue(dto),
                    writer);
        } catch (FileNotFoundException e) {
            e.printStackTrace();
        } catch (DocumentException e) {
