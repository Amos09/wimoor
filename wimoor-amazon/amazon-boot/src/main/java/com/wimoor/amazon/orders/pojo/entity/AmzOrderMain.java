...
public class AmzOrderMain implements Serializable {
    ...
    public AmzOrderMain(Order order, String market) {
        ...
        if (order.getPaymentExecutionDetail() != null && order.getPaymentExecutionDetail().size() > 0 &&
                order.getPaymentExecutionDetail().get(0).getPayment() != null
                && order.getPaymentExecutionDetail().get(0).getPayment().getAmount() != null) {
            this.setPaymentExecutionDetailItem(
                    new BigDecimal(order.getPaymentExecutionDetail().get(0).getPayment().getAmount()));
        }
