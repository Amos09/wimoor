...
public class ShipInboundPlanController {
    ...
    public Result<String> saveInboundPlanAction(@ApiParam("发货计划") @RequestBody ShipInboundPlanDTO inplan) {
        ...
        try {
            ...
            if (myinplan.getId() != null && myinplan.getPlanitemlist() != null
                    && myinplan.getPlanitemlist().size() > 0) {
    ...
    public Result<IPage<Map<String, Object>>> getShipRecordAction(String groupid, String marketplaceid, String sku) {
        UserInfo user = UserInfoContext.get();
        List<Map<String, Object>> shipRecord = shipInboundPlanService.getShipRecord(user.getCompanyid(), groupid,
                marketplaceid, sku);
        List<Map<String, Object>> shipBadRecord = shipInboundPlanService.getShipBadRecord(user.getCompanyid(),
                marketplaceid, sku);
