...
public class ProductFollowHandlerServiceImpl extends ServiceImpl<ProductFollowMapper, ProductFollow> implements
        IProductFollowHandlerService, IAwsSQSMessageHandlerService {
    ...
    public void recordProductFollow(String authid, String asin, String marketplaceid, Date timeofChange,
            Integer offernumber) {
    ...
    public void recordFollowOfferSellerName() {//一小时执行一次
        ...
        if (list != null && list.size() > 0) {
            for (int i = 0; i < list.size(); i++) {
                ...
                String refreshtime =
                        followOffer.get("refreshtime") == null ? null : followOffer.get("refreshtime").toString();
                int refreshnum = followOffer.get("refreshnum") == null ? 0
                        : Integer.parseInt(followOffer.get("refreshnum").toString());
                //更新时间大于24h的，重置更新次数
                if (refreshtime != null) {
