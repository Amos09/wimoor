...
public class AmzFinEmailServiceImpl implements IAmzFinEmailService {
    ...
    public Map<String, Map<String, Object>> getTotalFinAcc(List<Map<String, Object>> FBAInvCostList,
        ...
        for (Map<String, Object> map : allFinAccountList) {
            ...
            BigDecimal newamount = exchangeRateHandlerService.changeCurrencyByLocal(shopid, mycurrency, currency,
                    amount, 3);
            if ("convert".equals(map.get("type"))) {
                BigDecimal oldamount = item.get("transfer_total") == null ? new BigDecimal("0")
                        : (BigDecimal) item.get("transfer_total");
                item.put("transfer_total", oldamount.add(newamount));
                transfer_all = transfer_all.add(newamount);
            } else {
                BigDecimal oldamount = item.get("untransfer_total") == null ? new BigDecimal("0")
                        : (BigDecimal) item.get("untransfer_total");
    ...
    public void getFinEmailContent(StringBuffer buf, String shopid, String beginDate, String endDate) {
        ...
        if ((localInvCostList == null || localInvCostList.size() == 0) && (FBAInvCostList == null
                || FBAInvCostList.size() == 0)) {
            return;
        }
        List<AmazonGroup> grouplist = iAmazonGroupService.selectByShopId(shopid);
        ...
        buf.append(
                "<thead><tr style='height:30px;border:1px solid #eee; background:#fafafa;color:#666'><th>店铺</th><th style='text-align:right;'>已出账金额</th><th style='text-align:right;'>未出账金额</th><th style='text-align:right;'>FBA库存货值</th></tr></thead>");
        buf.append("<tbody>");
        int j = 0;
        ...
        buf.append("<tr style='height:30px;'><td>合计</td><td style='text-align:right;color: #00a65a !important;'>￥"
                ...
                + "</td><td style='text-align:right;color: #dd4b39 !important;'>￥"
                + GeneralUtil.formatterQuantity(new BigDecimal(map.get("untransfer_all").toString()))
                + "</td><td style='text-align:right;'>￥"
                + GeneralUtil.formatterQuantity(new BigDecimal(map.get("totalprice_all").toString())) + "</td></tr>");
        buf.append("</table>");
        buf.append(
                "<div style='margin:15px 0px'>为了您更清楚的了解到数据的来源，下面提供了各个店铺在每个站点下的汇总金额。其中汇率转换是按照wimoor系统主页展示的每日汇率计算的。</div>");
        buf.append("<table style='width:700px;'>");
        buf.append(
                "<thead><tr style='height:30px;border:1px solid #eee; background:#fafafa;color:#666'><th>店铺</th><th>站点</th><th style='text-align:right;'>已出账金额</th><th style='text-align:right;'>FBA库存货值</th></tr></thead>");
        buf.append("<tbody>");
        j++;
        int index = 0;
        String mycurrency = "";
        for (AmazonGroup group : grouplist) {
            ...
            for (Map<String, Object> market : marketmap) {
                ...
                for (Map<String, Object> mmap : allFinAccountList) {
                    ...
                    if (groupid.equals(group.getId()) && mmap.get("currency").toString().equals(mycurrency)
                            && "convert".equals(mmap.get("type"))) {
                        BigDecimal amount =
                                mmap.get("amount") == null ? new BigDecimal("0") : (BigDecimal) mmap.get("amount");
                        BigDecimal newamount = exchangeRateHandlerService.changeCurrencyByLocal(shopid, mycurrency,
                                "RMB", amount, 3);
                ...
                for (Map<String, Object> mmap : FBAInvCostList) {
                    ...
                    if (groupid.equals(group.getId()) && (
                            market.get("marketplaceid").toString().equals(mmap.get("marketplaceid").toString())
                                    || mmap.get("marketplaceid").toString().equals(market.get("region").toString()))) {
                ...
                String str = "<td style='text-align:right;background-color:#f2f2f2'>￥" + GeneralUtil.formatterQuantity(
                        totalprice) + "</td>";
                buf.append("<tr style='height:50px;background-color: " + color + ";'><td>" + group.getName()
                        + "</td><td>" + marketname
                        + "</td><td style='text-align:right'><span style='color: #00a65a !important;'>"
        ...
        buf.append("</tbody>");
        buf.append(
                "<tr style='height:30px;'><td>合计</td><td> </td><td style='text-align:right;color: #00a65a !important;'>￥"
                        + GeneralUtil.formatterQuantity(new BigDecimal(map.get("transfer_all").toString()))
                        + "</td><td style='text-align:right;'>￥"
                        + GeneralUtil.formatterQuantity(new BigDecimal(map.get("totalprice_all").toString()))
                        + "</td></tr>");
        buf.append("</table>");
        if (localInvCostList != null && localInvCostList.size() > 0) {
            ...
            buf.append("<table style='width:700px;'>");
            buf.append(
                    "<thead><tr style='height:30px;border:1px solid #eee; background:#fafafa;color:#666'><th>本地仓库</th><th style='text-align:right;'>库存货值</th></tr></thead>");
            buf.append("<tbody>");
        ...
        buf.append("<div>");
        buf.append(
                "<span style='color: rgb(192, 192, 192); font-size: x-small;'> 如果您有任何关于用户体验方面的问题或建议，都可联系我们：service@wimoor.com </span>");
        buf.append("</div>");
        buf.append("<div>");
        buf.append(
                "<span style='color: rgb(192, 192, 192); font-size: x-small;'> 如有其它疑问，请拨打wimoor官方热线 86-13554833402获得更多帮助,Copyright © 2017 深圳市万墨信息科技有限公司版权所有   </span>");
        buf.append("</div>");
        buf.append("</div>");
        buf.append("</div>");
    }
    ...
    public void sendWeekEmailDetail(String shopid) {
        ...
        buf.append(
                "<div style='width:800px;max-device-width: 800px;margin:0px auto;background:#fff;border-top-radius:4px;'>");
...
//					+ "2020年02月01日-2020年02月29日"
        ...
        buf.append("<div style='text-align: center;line-height: 0;'><img src='" + photoPath
                + "/temp/footer.png'><br><br></div> ");
    ...
    public void sendMonthEmailDetail(String shopid) {
        ...
        buf.append("<h2 style='margin:0px'>财务一览</h2>");
        buf.append("<div style='color: #999;font-size:12px;'>时间："
                + GeneralUtil.formatDate(GeneralUtil.getFirstdayOfMonth(GeneralUtil.getLastMonthday(new Date())),
                "yyyy年MM月dd日")
                + "-" + GeneralUtil.formatDate(GeneralUtil.getLastdayOfMonth(GeneralUtil.getLastMonthday(new Date())),
                "yyyy年MM月dd日") + "</div>");
        buf.append("<br>");

        String beginDate = GeneralUtil.formatDate(
                GeneralUtil.getFirstdayOfMonth(GeneralUtil.getLastMonthday(new Date())), "yyyy-MM-dd");
