...
public class FulfillmentInboundServiceImpl implements IFulfillmentInboundService {
    ...
    public GetInboundGuidanceResult getInboundGuidance(AmazonAuthority auth, String marketplaceId,
    ...
    public Map<String, SKUPrepInstructions> getPrepInstructions(AmazonAuthority auth, Marketplace market,
            List<String> mlist) {
    ...
    public List<ShipInboundShipment> createInboundShipmentPlan(AmazonAuthority auth, Marketplace market,
            ShipInboundPlan inplan) {
        ...
        for (int i = 0; i < itemlist.size(); i++) {
            ...
            ProductInfo product = iProductInfoService.productOnlyone(auth.getId(), itemsku.getSellersku(),
        ...
        try {
            ...
            for (InboundShipmentPlan shipment : plan) {
                ...
                String newship = "FBA" + "(" + (calender.get(Calendar.MONTH) + 1) + "/" + (calender.get(Calendar.DATE))
                        + "/" + calender.get(Calendar.YEAR) + " " + calender.get(Calendar.HOUR) + ":" + calender.get(
                        Calendar.MINUTE) + ")-" + (i + 1);
                ...
                if (oldinfo != null && StrUtil.isNotBlank(oldinfo.getCompany()) && StrUtil.isNotBlank(
                        oldinfo.getChannel())) {
    ...
    public String createInboundShipment(AmazonAuthority amazonAuthority, Marketplace marketplace, ShipInboundPlan plan,
            ShipInboundShipment shipment) {// 点击审核通过
        ...
        if (shipment.getIntendedBoxContentsSource() != null) {
            header.setIntendedBoxContentsSource(
                    IntendedBoxContentsSource.fromValue(shipment.getIntendedBoxContentsSource()));
        } else {
    ...
    public String updateInboundShipment(AmazonAuthority amazonAuthority, Marketplace marketplace,
    ...
    public InboundShipmentList listShipment(AmazonAuthority auth, Marketplace market, String startdate,
            String enddate) {
        FbaInboundApi api = apiBuildService.getInboundApi(auth);
        try {
            ...
            GetShipmentsResponse response = api.getShipments("DATE_RANGE", market.getMarketplaceid(),
            ...
            while (result.getNextToken() != null) {
                response = api.getShipments("NEXT_TOKEN", market.getMarketplaceid(), null, null, null, null,
                        result.getNextToken());
    ...
    public InboundShipmentList listShipment(AmazonAuthority auth, Marketplace market, List<String> shipmentIdsList) {
        FbaInboundApi api = apiBuildService.getInboundApi(auth);
        try {
            GetShipmentsResponse response = api.getShipments("SHIPMENT", market.getMarketplaceid(), null,
                    shipmentIdsList,
                    null, null, null);
            if (response == null) {
                return null;
    ...
    @Override
    public String putTransportDetailsRequest(AmazonAuthority amazonAuthority, Marketplace marketplace,
    ...
    public String getLabelDownloadUR(AmazonAuthority amazonAuthority, String shipmentid, Integer boxnum,
            String pagetype, String labelType, String pannum) {
        FbaInboundApi api = apiBuildService.getInboundApi(amazonAuthority);
        try {
    ...
    public void runApi(AmazonAuthority amazonAuthority) {
        ...
        for (Marketplace market : awsregion) {
            try {
                List<Map<String, Object>> plist = shipInboundShipmentMapper.findProblemShip(
                        amazonAuthority.getGroupid(), market.getMarketplaceid());
                if (plist != null && plist.size() > 1) {
                    ...
                    List<Map<String, Object>> list = shipInboundShipmentMapper.findAllShiped(
                            amazonAuthority.getGroupid(), market.getMarketplaceid());
    ...
    public List<ShipInboundItem> syncItemsByShipmentId(AmazonAuthority amazonAuthority, Marketplace market,
            ShipInboundShipment shipment) {
        // TODO Auto-generated method stub
        amazonAuthority.setUseApi("getShipmentItemsByShipmentId");
        FbaInboundApi api = apiBuildService.getInboundApi(amazonAuthority);
        try {
            GetShipmentItemsResponse response = api.getShipmentItemsByShipmentId(shipment.getShipmentid(),
                    market.getMarketplaceid());
            if (response != null && response.getPayload() != null) {
    ...
    public List<ShipInboundItem> getUnSyncItemsByShipmentId(AmazonAuthority amazonAuthority, Marketplace market,
            ShipInboundShipment shipment) {
        // TODO Auto-generated method stub
        amazonAuthority.setUseApi("getShipmentItemsByShipmentId");
        FbaInboundApi api = apiBuildService.getInboundApi(amazonAuthority);
        try {
            GetShipmentItemsResponse response = api.getShipmentItemsByShipmentId(shipment.getShipmentid(),
                    shipment.getInboundplan().getMarketplaceid());
            if (response != null && response.getPayload() != null) {
                GetShipmentItemsResult shipmentItemsResult = response.getPayload();
    ...
    private void listShipmentItemsAsync(AmazonAuthority amazonAuthority, Marketplace market) {
        ...
        try {
            ...
            cstart.add(Calendar.DATE, -3);
            Calendar cend = Calendar.getInstance();
            ApiCallback<GetShipmentItemsResponse> callback = new ApiCallbackGetShipmentItems(this, amazonAuthority,
                    market, null);
    ...
    private void getItemsByShipmentIdAsync(AmazonAuthority amazonAuthority, Marketplace market,
            ShipInboundShipment shipment) {
        // TODO Auto-generated method stub
        amazonAuthority.setUseApi("getShipmentItems");
        FbaInboundApi api = apiBuildService.getInboundApi(amazonAuthority);
    ...
    public void handlerResult(AmazonAuthority auth, Marketplace market, GetShipmentsResponse result) {
            ...
            if (shipment != null) {
                ...
                if (item.getShipmentStatus() != null && (shipment.getStatus() >= 5 || shipment.getSyncInv() == 1
                    ...
                    if (shipment.getSyncInv() == 1 || shipment.getSyncInv() == 2) {
                        ...
                        } else if ("IN_TRANSIT".equals(item.getShipmentStatus().getValue())
                                && shipment.getStatus() != 5) {
                            if (shipment.getStatus() == null) {
                        ...
                        } else if ("RECEIVING".equals(item.getShipmentStatus().getValue())
                                && shipment.getStatus() != 5) {
                            if (shipment.getStatus() == null) {
                                shipment.setStatus(5);
                            }
        ...
        if (shipmentsResult.getNextToken() != null) {
            ...
            try {
                ...
                api.getShipmentsAsync("NEXT_TOKEN", market.getMarketplaceid(), null, null, null, null,
                        shipmentsResult.getNextToken(), callback);
            } catch (ApiException e) {
                // TODO Auto-generated catch block
                e.printStackTrace();
            } catch (LWAException e) {
    ...
    private List<ShipInboundItem> getItemResultData(AmazonAuthority auth, GetShipmentItemsResult itemResult,
            ShipInboundShipment param_shipment) {
        ...
        for (InboundShipmentItem item : itemResult.getItemData()) {
            ...
            if (inbounditem != null) {
                String msku = iProductInfoService.getMSKU(auth.getId(),
                        param_shipment.getInboundplan().getMarketplaceid(), item.getSellerSKU());
                try {
                    if (msku == null) {
    ...
    private List<ShipInboundItem> handlerItemResultData(AmazonAuthority auth, GetShipmentItemsResult itemResult,
            ShipInboundShipment param_shipment, boolean needshipqty) {
    ...
    @Override
    public void handlerItemResult(AmazonAuthority auth, Marketplace market, GetShipmentItemsResponse result,
            ShipInboundShipment param_shipment, boolean needshipqty) {
        // TODO Auto-generated method stub
        ...
        if (itemResult.getNextToken() != null) {
            FbaInboundApi api = apiBuildService.getInboundApi(auth);
            ApiCallback<GetShipmentItemsResponse> callback = new ApiCallbackGetShipmentItems(this, auth, market,
                    param_shipment);
            try {
                api.getShipmentItemsAsync("NEXT_TOKEN", market.getMarketplaceid(), null, null,
                        itemResult.getNextToken(), callback);
            } catch (ApiException e) {
                // TODO Auto-generated catch block
                e.printStackTrace();
            } catch (LWAException e) {
    ...
    @Override
    public List<InboundShipmentItem> requestInboundShipmentsItem(AmazonAuthority auth, String shipmentid,
            Marketplace marketPlace) {
        FbaInboundApi api = apiBuildService.getInboundApi(auth);
        List<InboundShipmentItem> list = null;
        try {
            GetShipmentItemsResponse response = api.getShipmentItemsByShipmentId(shipmentid,
    ...
    @Override
    public InboundShipmentInfo requestInboundShipments(AmazonAuthority auth, String shipmentid,
            Marketplace marketPlace) {
        FbaInboundApi api = apiBuildService.getInboundApi(auth);
        ...
        try {
            ...
            response = api.getShipments(querytype, marketPlace.getMarketplaceid(), null, shipmentIdList, null, null,
                    null);
        } catch (ApiException e) {
            e.printStackTrace();
            throw AmzUtil.getException(e);
