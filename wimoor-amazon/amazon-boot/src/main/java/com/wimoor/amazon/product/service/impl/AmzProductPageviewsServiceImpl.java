...
public class AmzProductPageviewsServiceImpl extends
        ServiceImpl<AmzProductPageviewsMapper, AmzProductPageviews> implements IAmzProductPageviewsService {
    ...
    public void refreshDownload() {
        ...
        for (AmzProductPageviewsConditionVo item : list) {
            if (GeneralUtil.distanceOfMinutes(item.getOpttime(), new Date()) > 5) {
                ...
                this.baseMapper.deleteByMarketplaceid(item.getMarketplaceid(), item.getAmazonAuthid().toString(),
                        GeneralUtil.getStrDate(item.getByday()));
            }
        }
    }

    public String uploadSessionFile(UserInfo user, String data, String marketplaceid, String sellerid, Date day,
            String type) {
        final Map<String, Integer> titleMap = new HashMap<String, Integer>();
        final String[] FILE_HEADER = {"﻿parent", "child", "title", "sku", "sessions - total",
                "session percentage - total",
                "page views - total", "page views percentage - total", "buy box percentage", "units ordered",
                "units ordered - b2b",
        ...
        if (type != null) {
            if (type.equals("week")) {
                ...
                c.setTime(day);
                for (int i = 1; i <= 7; i++) {
                    this.baseMapper.deleteByMarketplaceid(marketplaceid, auth.getId(),
                            GeneralUtil.getStrDate(c.getTime()));
    ...
    public String uploadSessionFile(String marketplaceid, String sellerid, String day,
            List<AmzProductPageviews> pagelist) {
        AmazonAuthority auth = amazonAuthorityService.selectBySellerId(sellerid);
        if (auth == null) {
            return null;
        }
