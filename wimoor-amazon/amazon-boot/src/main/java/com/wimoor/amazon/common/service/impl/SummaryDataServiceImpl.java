...
public class SummaryDataServiceImpl extends ServiceImpl<SummaryDataMapper, SummaryData> implements ISummaryDataService {
    ...
    public Map<String, Object> getReturnOrder(SummaryMutilParameterQueryDTO parameter) {
        ...
        List<Object> quantityList = ChartPoint.getChartData(ChartPoint.SumType.Daily, listMap, "return_date",
    ...
    public Map<String, Object> selectByMutilParameter(SummaryMutilParameterQueryDTO parameter) {
        ...
        Date endDate = GeneralUtil.getDatez(parameter.getEndDate());
        List<String> labels = ChartPoint.getLabels(ChartPoint.SumType.Daily, beginDate, endDate);
        List<Object> quantityList = ChartPoint.getChartData(ChartPoint.SumType.Daily, listMap, "purchase_date",
                "quantity", beginDate, endDate);
        List<Object> orderNumberList = ChartPoint.getChartData(ChartPoint.SumType.Daily, listMap, "purchase_date",
                "ordernumber", beginDate, endDate);
        Map<String, Object> result = new HashMap<String, Object>();
        result.put("labels", labels);
        result.put("quantityList", quantityList);
    ...
    public ChartLine findOrderSummaryBySku(String groupid, String amazonAuthId, String sku, String parentAsin,
            String marketplaceid, Integer daysize, String linetype, UserInfo user) {
        ...
        if (list != null && list.size() > 0) {
            for (int i = 0; i < list.size(); i++) {
                if (linetype != null && linetype.equals("order")) {
                    tempmap.put(list.get(i).getSku() + sdf.format(list.get(i).getPurchaseDate()),
                            list.get(i).getOrdersum());
                } else {
                    tempmap.put(list.get(i).getSku() + sdf.format(list.get(i).getPurchaseDate()),
        ...
        for (int i = 0; i <= daysize; i++, c.add(Calendar.DATE, 1)) {
            ...
            Integer value = tempmap.get(mysku + tempkey) == null ? Integer.valueOf("0")
                    : Integer.valueOf(tempmap.get(mysku + tempkey).toString());
