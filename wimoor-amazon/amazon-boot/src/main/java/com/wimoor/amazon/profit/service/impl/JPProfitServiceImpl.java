...
public class JPProfitServiceImpl extends ProfitServiceImpl {
    ...
    /**
     * 判断 Product Tier high_value:对于samll和standard size,当售价大于等于45000日元时，产品为high_value
     *
     * @throws BizException
     */
    @Override
    ...
    public BigDecimal calculateFBA(String country, String productTierId, InputDimensions inputDimension, String isMedia,
            String type, BigDecimal outboundWeight, ProfitConfigCountry profitConfigX, String shipmentTypes)
            throws BizException {
        BigDecimal FBA = new BigDecimal("0");
        FBAFormat fbaFormat = fbaFormatService.findByProductTierIdAndWeight(productTierId, outboundWeight, country);
        String format = fbaFormat.getFbaFormat();
    ...
    public BigDecimal calculateStorageFee(String country, ProfitConfigCountry profitConfigX,
        ...
        BigDecimal storageFee = monthlyFee.multiply(volume)
                .multiply(month);//8.126 日元 × {[商品尺寸(cm 3)] / (10cm × 10cm × 10cm)} × [保管天数/当月的天数]
        return storageFee;
    }
