...
public class ShipInboundPlanServiceImpl extends ServiceImpl<ShipInboundPlanV2Mapper, ShipInboundPlan> implements
    ...
    public void saveShipInboundPlan(ShipInboundPlan inplan) {
        ...
        AmazonAuthority auth = this.amazonAuthorityService.selectByGroupAndMarket(inplan.getGroupid(),
    ...
    public ShipInboundOperation cancelInboundPlan(ShipInboundPlan inplan) {
        AmazonAuthority auth = amazonAuthorityService.selectByGroupAndMarket(inplan.getGroupid(),
                inplan.getMarketplaceid());
        CancelInboundPlanResponse response = iInboundApiHandlerService.cancelInboundPlan(auth,
    ...
    public Map<String, Object> listInboundPlanPallets(String planid) {
        ...
        if (plan != null) {
            ...
            ListInboundPlanPalletsResponse response = iInboundApiHandlerService.listInboundPlanPallets(auth,
    ...
    public void getDeliveryChallanDocument(String planid, String shipmentId) {
        ...
        if (plan != null) {
            AmazonAuthority auth = amazonAuthorityService.selectByGroupAndMarket(plan.getGroupid(),
                    plan.getMarketplaceid());
            GetDeliveryChallanDocumentResponse response = iInboundApiHandlerService.getDeliveryChallanDocument(auth,
                    plan.getInboundPlanId(), shipmentId);
            if (response != null) {
    ...
    public String generateSelfShipAppointmentSlots(String planid, String shipmentId, String startDate, String endDate) {
        ...
        if (plan != null) {
            ...
            GenerateSelfShipAppointmentSlotsResponse response = iInboundApiHandlerService.generateSelfShipAppointmentSlots(
                    auth, plan.getInboundPlanId(), shipmentId, dateitem);
            if (response != null) {
    ...
    public ShipInboundOperation confirmInboundPlan(ShipInboundPlan plan) {
        ...
        List<ShipInboundItem> itemlist = iShipInboundItemService.lambdaQuery()
                .eq(ShipInboundItem::getFormid, plan.getId()).list();
        ShipAddress address = shipAddressService.getById(plan.getSourceAddress());
        CreateInboundPlanResponse response = iInboundApiHandlerService.createInboundPlan(auth, market, plan, itemlist,
                address);
        if (response != null) {
    ...
    public ShipInboundPlan changeAddress(UserInfo user, String formid, String addressid) {
        ...
        if (plan.getAuditstatus() >= 3) {
            ShipAddress address = shipAddressService.getById(plan.getSourceAddress());
            List<ShipInboundShipment> shipments = shipInboundShipmentService.lambdaQuery()
                    .eq(ShipInboundShipment::getInboundplanid, plan.getInboundPlanId()).list();
            for (ShipInboundShipment item : shipments) {
                UpdateShipmentSourceAddressResponse response = iInboundApiHandlerService.updateShipmentSourceAddress(
                        auth, plan.getInboundPlanId(), item.getShipmentid(), address);
    ...
    public ShipInboundPlan changeShipmentAddress(UserInfo user, String shipmentid, String addressid) {
        ...
        ShipInboundPlan plan = this.lambdaQuery().eq(ShipInboundPlan::getInboundPlanId, shipment.getInboundplanid())
                .one();
        AmazonAuthority auth = amazonAuthorityService.getById(plan.getAmazonauthid());
        plan.setOperator(user.getId());
        plan.setOpttime(new Date());
        ...
        this.updateById(plan);
        ShipAddress address = shipAddressService.getById(plan.getSourceAddress());
        UpdateShipmentSourceAddressResponse response = iInboundApiHandlerService.updateShipmentSourceAddress(auth,
                plan.getInboundPlanId(), shipmentid, address);
        iShipInboundOperationService.setOperationID(auth, plan.getId(), response.getOperationId());
        return plan;
    }
