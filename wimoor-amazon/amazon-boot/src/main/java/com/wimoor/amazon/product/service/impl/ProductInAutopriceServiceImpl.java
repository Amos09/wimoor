 ...
 */
@Service
public class ProductInAutopriceServiceImpl extends ServiceImpl<ProductInAutopriceMapper, ProductInAutoprice> implements
        IProductInAutopriceService {
    ...
    public void changeTaskPriceSku(String asin, String marketplaceid, Map<String, FollowOfferChange> mapseller) {
        ...
        for (ProductInfo info : list) {
            ...
            if ((info.getInvalid() != null && info.getInvalid() == true) || (opt != null && opt.getDisable() != null
                    && opt.getDisable() == true)) {
                continue;
            }
            ProductInAutoprice auto = this.getById(info.getId());
            boolean disable = ((auto == null || auto.getDisable() == null) ? false : auto.getDisable());
            if (auto == null || disable == true || auto.getLowestprice() == null
                    || auto.getLowestprice().floatValue() < 0.00001) {
                continue;
            }
            ...
            if (lowestprice == null) {
                for (Entry<String, FollowOfferChange> entry : mapseller.entrySet()) {
                    ...
                    BigDecimal price =
                            value.getListingPriceAmount() != null ? value.getListingPriceAmount() : new BigDecimal("0");
                    price = price.add(
                            value.getShipingAmount() != null ? value.getShipingAmount() : new BigDecimal("0"));
            ...
            if (type == 2) {
                if (buyboxseller != null) {
                    BigDecimal price =
                            buyboxseller.getListingPriceAmount() != null ? buyboxseller.getListingPriceAmount()
                                    : new BigDecimal("0");
                    price = price.add(buyboxseller.getShipingAmount() != null ? buyboxseller.getShipingAmount()
                            : new BigDecimal("0"));
                    lowestprice = price;
                }
            }
            ...
            if (myseller != null) {
                ...
                BigDecimal lprice = asinseller.getListingPriceAmount() != null ? asinseller.getListingPriceAmount()
                        : new BigDecimal("0");
                BigDecimal ship =
                        asinseller.getShipingAmount() != null ? asinseller.getShipingAmount() : new BigDecimal("0");
                    ...
                    if (myseller.getIsBuyBoxWinner()) {
                        ...
                        for (Entry<String, FollowOfferChange> entry : mapseller.entrySet()) {
                            ...
                            if (!value.getSellerid().equals(amazonAuthority.getSellerid())) {
                                BigDecimal price = value.getListingPriceAmount() != null ? value.getListingPriceAmount()
                                        : new BigDecimal("0");
                                price = price.add(value.getShipingAmount() != null ? value.getShipingAmount()
                                        : new BigDecimal("0"));
