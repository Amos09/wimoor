...
public class AmzOrderMainServiceImpl extends ServiceImpl<AmzOrderMainMapper, AmzOrderMain> implements
        IAmzOrderMainService, IAwsSQSMessageHandlerService {
    ...
    public void handlerOrderResponse(AmazonAuthority auth, AmzAuthApiTimelimit apilimit, GetOrdersResponse response) {
        ...
        if (orders != null && orders.size() > 0) {
            for (int i = 0; i < orders.size(); i++) {
                ...
                String channel =
                        order.getFulfillmentChannel() != null ? order.getFulfillmentChannel().getValue() : null;
                String tructionid = order.getFulfillmentInstruction() != null ? order.getFulfillmentInstruction()
                        .getFulfillmentSupplySourceId() : null;
                ...
                BigDecimal ordertotal =
                        order.getOrderTotal() == null ? null : new BigDecimal(order.getOrderTotal().getAmount());
                    ...
                    if (apilimit.apiNotRateLimit()) {
                        if (StrUtil.isBlank(token)) {
                            ...
                            api.getOrdersAsync(marketplaceIds, null, null, null,
                                    null, null, fulfillmentChannels, paymentMethods,
                                    null, null, 100,
                                    null, null, token, null, null, null, null, null, null, null, null,
                                    orderReponseHandler);
                        }
                    }
