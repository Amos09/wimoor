...
public class HandlerReportServiceImpl implements IHandlerReportService, IAwsSQSMessageHandlerService {
    ...
    public void refreshTreatReport(List<ReportRequestRecord> reportRequestRecordList, Marketplace marketplace)
            throws Exception {
    ...
    public int hasNeedProcessReportList(String reporttype) {
        List<ReportRequestRecord> reportRequestRecordList = reportRequestRecordMapper.selectNeedProcessRequest(
                reporttype, null);
    ...
    public void refreshProcessReportList(String reporttype) {
        List<Runnable> runnables = new ArrayList<Runnable>();
        if (reporttype == null) {
            List<ReportRequestRecord> reportRequestRecordList = reportRequestRecordMapper.selectNeedProcessRequest(null,
                    null);
            ...
            if (reportRequestRecordList == null || reportRequestRecordList.size() < 100) {
                List<ReportRequestRecord> reportSettRequestRecordList = reportRequestRecordMapper.selectNeedProcessRequest(
                        ReportType.SettlementReport, null);
        ...
        Runnable runnable = new Runnable() {
            public void run() {
                for (int i = 0; i < region.size(); i++) {
                    ...
                    List<ReportRequestRecord> reportRequestRecordList = reportRequestRecordMapper.selectNeedRefreshRequest(
                            reporttype, marketplace.getMarketplaceid());
                    if (reportRequestRecordList != null && reportRequestRecordList.size() > 0) {
    ...
    public void requestReport(List<AmazonAuthority> amazonAuthorityList, IReportService service, Calendar cstart,
            Calendar cend, Boolean ignore) {
        List<Runnable> runnables = new ArrayList<Runnable>();
    ...
    public void requestReport(List<AmazonAuthority> amazonAuthorityList, String reportType, Date start, Date end,
            Boolean ignore) {
        ReportRequestType type = iReportRequestTypeService.findByTypeCode(reportType);
        if (type == null) {
            return;
    ...
    public boolean handlerMessage(JSONObject body) {
        ...
        if (reportProcessingFinishedNotification != null) {
            ...
            if (record != null) {
                if (record.getReportProcessingStatus() == null || (!record.getReportProcessingStatus().equals("success")
                        && !record.getReportProcessingStatus().equals("treat"))) {
    ...
    @Override
    public GetReportsResponse listReportList(AmazonAuthority amazonAuthority, String reporttype, Calendar cstart,
            Calendar cend, String nextToken) {
        // TODO Auto-generated method stub
