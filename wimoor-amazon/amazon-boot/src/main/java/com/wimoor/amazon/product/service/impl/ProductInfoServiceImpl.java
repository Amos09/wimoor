...
public class ProductInfoServiceImpl extends ServiceImpl<ProductInfoMapper, ProductInfo> implements IProductInfoService {
    ...
    @Override
    public IPage<AmzProductListVo> getListByUser(UserInfo userinfo, ProductListQuery query,
    ...
    @Override
    public List<AmzProductListVo> getListDetailByUser(UserInfo userinfo, ProductListQuery query,
            Map<String, Object> parameter) {
        // TODO Auto-generated method stub
        parameter.put("shopid", userinfo.getCompanyid());
        List<String> taglist = query.getTaglist();//123,456,122,
    ...
    public void handleProductListVo(UserInfo userinfo, Map<String, Object> parameter, List<AmzProductListVo> list) {
        ...
        try {
            if (mskuList.size() > 0) {
                Result<Map<String, String>> materialTagsIdsListResult = erpClientOneFeign.getTagsIdsListByMsku(
                        userinfo.getCompanyid(), mskuList);
    ...
    public IPage<AmzProductListVo> getListParentByUser(UserInfo userinfo, ProductListQuery query,
            Map<String, Object> parameter) {
        // TODO Auto-generated method stub
        parameter.put("shopid", userinfo.getCompanyid());
        List<String> taglist = query.getTaglist();//123,456,122,
        ...
        Set<String> set = new HashSet<String>(
                Arrays.asList("amazonAuthId", "marketplaceid", "parentAsin", "name", "image", "landed_amount",
                        "landed_currency", "changeprice",
                        "averageSalesDay", "averageSalesWeek", "averageSalesMonth", "listing_currency", "sumweek",
                        "summonth", "orderweek", "sales15", "ordermonth", "sessions",
                        "sessionrate", "unitsordered7", "advimpr", "advsales", "advcpc", "advclick", "advspc",
                        "advspend", "advctr", "advacos", "afn_fulfillable_quantity",
                        "buybox", "prorate", "profitrate", "profitall", "proprate", "marketplacename", "pointName",
                        "groupname", "newprorate", "country", "marketindex"
                ));
            ...
            for (ProductInfoListVo item : list.getRecords()) {
                if (item.getId() != null) {
                    if ("1".equals(item.getIssfg())) {
                        ...
                        Result<Integer> data = erpClientOneFeign.getAssemblyCanAction(materialid, warehouseid,
                                dto.getShopid());
    ...
    public void getWeekOtherCostForSKU(UserInfo user, List<AmzProductListVo> productList,
            List<Map<String, Object>> skulist, List<String> mskuList) {
        if (productList != null) {
            ...
            for (AmzProductListVo item : productList) {
                ...
                if (itemList != null) {
                    for (Map<String, Object> itemFin : itemList) {
                        ...
                        otherCost = otherCost.add(
                                exchangeRateHandlerService.changeCurrencyByLocal(currency, item.getLandedCurrency(),
                                        new BigDecimal(amount), 2));
                    }
    ...
    public Map<String, Object> updateProductPrice(UserInfo user, Map<String, Object> map, String ftype) {
        ...
        if (priceXml != null && auth != null && user != null && StrUtil.isNotEmpty(marketplaceid)) {
            ...
            response = submitfeedService.SubmitFeedQueue(priceXml, "productPrice" + sku, auth,
                    FeedFileProductPriceXML.type, user, null);
        } else {
        ...
        if (StrUtil.isNotEmpty(sku) && StrUtil.isNotEmpty(marketplaceid) && auth != null && standardPrice != null) {
            ...
            List<ProductPrice> propriceList = productPriceMapper.selectBySkuAndSeller(sku, marketplaceid,
                    auth.getSellerid());
    ...
    public void showProfitDetial(Map<String, Object> map) {
        ...
        ProductInProfit productInProfit =
                map.get("productInProfit") != null ? (ProductInProfit) map.get("productInProfit") : null;
        ...
        if (productInProfit != null && map.get("productpricetype") == null) {
            ...
            if (map.get("onlymargin") == null) {
                ...
                if (dim_local != null) {
                    String lengthValue =
                            dim_local.getInputDimensions().getLength().value.toString() + dim_local.getInputDimensions()
                                    .getLength().units;
                    String widthValue =
                            dim_local.getInputDimensions().getWidth().value.toString() + dim_local.getInputDimensions()
                                    .getWidth().units;
                    String heightValue =
                            dim_local.getInputDimensions().getHeight().value.toString() + dim_local.getInputDimensions()
                                    .getHeight().units;
                    String weightValue =
                            dim_local.getInputDimensions().getWeight().value.toString() + dim_local.getInputDimensions()
                                    .getWeight().units;
                    ...
                    if (dim_amz != null) {
                        String lengthValue =
                                dim_amz.getInputDimensions().getLength().value.toString() + dim_amz.getInputDimensions()
                                        .getLength().units;
                        String widthValue =
                                dim_amz.getInputDimensions().getWidth().value.toString() + dim_amz.getInputDimensions()
                                        .getWidth().units;
                        String heightValue =
                                dim_amz.getInputDimensions().getHeight().value.toString() + dim_amz.getInputDimensions()
                                        .getHeight().units;
                        String weightValue =
                                dim_amz.getInputDimensions().getWeight().value.toString() + dim_amz.getInputDimensions()
                                        .getWeight().units;
                        map.put("dimensionsInfo", lengthValue + "-" + widthValue + "-" + heightValue);
                        map.put("dimensionsWeight", weightValue);
                    }
    ...
    public void setLocalValue(Map<String, Object> map, String pid, String msku) {
        ...
        if (Result.isSuccess(result)) {
            ...
            if (mpriceMap != null) {
                if (mpriceMap.get("totalamount") != null) {
                    ...
                    currPrice = exchangeRateHandlerService.changeCurrencyByLocal("CNY", market.getCurrency(),
                            currPrice);
                    ...
                    map.put("repricetime", "实时计算,最近10个采购单的均价,第一笔订单的时间:"
                            + GeneralUtil.formatDate(AmzDateUtils.getDate(mpriceMap.get("audittime"))) + "，对应金额RMB："
                            + repriceRMB);
                }
            }
        }

        Map<String, Object> transFeeMap = shipInboundTransService.shipmentTransFee(auth.getShopId(),
                info.getMarketplaceid(), auth.getGroupid(), info.getSku());
        if (transFeeMap != null && transFeeMap.get("skufeeavg") != null) {
            ...
            if (mcurrShipmentFee != null) {
                ...
                currShipmentFee = exchangeRateHandlerService.changeCurrencyByLocal("CNY", market.getCurrency(),
                        currShipmentFeeRMB);
    ...
    public IPage<Map<String, Object>> getProductInfoWithFnSKU(Page<Object> page, Map<String, Object> map) {
        // TODO Auto-generated method stub
        AmazonAuthority auth = amazonAuthorityService.selectByGroupAndMarket(map.get("groupid").toString(),
