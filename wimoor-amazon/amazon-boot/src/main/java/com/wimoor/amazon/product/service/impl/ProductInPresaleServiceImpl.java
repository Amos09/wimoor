...
public class ProductInPresaleServiceImpl extends ServiceImpl<ProductInPresaleMapper, ProductInPresale> implements
        IProductInPresaleService {
    ...
    public int getLocalSalesday(Map<String, Map<String, Integer>> sumpresaleMap, int quantity,
            List<Map<String, Object>> fbasale) {
        int salesday = 0;
        Calendar now = Calendar.getInstance();
        while (quantity > 0 && salesday < 365) {
    ...
    public void calculationPresales(UserInfo user, Map<String, Object> item, List<Map<String, Object>> fbasale,
            Map<String, Map<String, Integer>> sumpresaleMap, Map<String, Object> param) {
        //对于欧洲站点，重新计算日均销量
        ...
        int stocking_cycle =
                item.get("stocking_cycle") == null ? 0 : Integer.parseInt(item.get("stocking_cycle").toString());
        int quantity = 0;
        ...
        if (presaleMap != null) {
            ...
            if (sumpresales_stocking - quantity > 0) {
                planquantity = sumpresales_stocking - quantity > sumpresales_mincycle ? sumpresales_stocking - quantity
                        : sumpresales_mincycle;
            }
            salesday = getSalesday(presaleMap, quantity, presale);// 根据销量分摊库存，求可售天数
        } else {
            if (stocking_cycle * presale - quantity > 0) {
                planquantity =
                        stocking_cycle * presale - quantity > mincycle * presale ? stocking_cycle * presale - quantity
                                : mincycle * presale;
            }
            if (presale != 0) {
    ...
    public int calculationSuggessPlan(Map<String, Object> map, Map<String, Object> param, int cycle,
            List<Map<String, Object>> fbasale, Map<String, Map<String, Integer>> sumpresaleMap) {
        Integer salesummary = param.get("salesummary") == null ? 0 : (Integer) param.get("salesummary");
        Integer fbaplannumber = param.get("fbaplannumber") == null ? 0 : (Integer) param.get("fbaplannumber");
        Integer selfsale_quantity_sum =
                param.get("selfsale_quantity_sum") == null ? 0 : (Integer) param.get("selfsale_quantity_sum");
        int plan = 0;
    ...
    public String uploadPreSalesByExcel(Workbook workbook, UserInfo user) {
        ...
        for (int i = 1; i <= sheet.getLastRowNum(); i++) {
            ...
            if (group == null) {
                throw new BizException("无法正常获取店铺，请确认店铺是否存在或已经改名");
            }
            groupid = group.getId();
    ...
    public List<Map<String, Object>> getProductPreSalesByMonth(String sku, String marketplaceid, String groupid) {
        ...
        List<Map<String, Object>> monthsale = this.baseMapper.selectMonthDateSales(sku, marketplaceid, groupid,
                GeneralUtil.formatDate(begin), GeneralUtil.formatDate(end));
    ...
    public List<Map<String, Object>> getProductPreSales(String sku, String marketplaceid, String groupid,
        ...
        Map<String, Object> qtymap = new HashMap<String, Object>();
        List<Map<String, Object>> list = this.baseMapper.selectDateSales(sku, marketplaceid, groupid,
                GeneralUtil.formatDate(beginDate), GeneralUtil.formatDate(endDate));

        cold.setTime(c.getTime());
        cold.add(Calendar.MONTH, 1);

        List<ProductInPresale> prelist = this.baseMapper.selectAllDayPresale(sku, marketplaceid, groupid,
                GeneralUtil.formatDate(c.getTime()), GeneralUtil.formatDate(cold.getTime()));
