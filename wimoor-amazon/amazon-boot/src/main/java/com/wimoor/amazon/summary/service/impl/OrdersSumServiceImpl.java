...
public class OrdersSumServiceImpl implements IOrdersSumService {
    ...
    public static Boolean checkTimeType(Map<String, Object> map, Calendar c, Date beginDate, Date endDate) {
        ...
        if ("Daily".equals(map.get("bytime"))) {
            ...
            return c.getTime().equals(endDate)
                    || GeneralUtil.getSundayOfThisWeek(c.getTime()).compareTo(GeneralUtil.getSundayOfThisWeek(endDate))
                    <= 0;
        } else if ("Monthly".equals(map.get("bytime"))) {
    ...
    public List<Map<String, Object>> findordershopReport(Map<String, Object> parameter) {
        ...
        for (Date step = getStepStart(calendar, summaryType); step.before(end) || step.equals(end);
                step = getStep(calendar, summaryType)) {
                ...
                for (String datekey : map2.keySet()) {
                    ...
                    if (datekey.contains("v")) {
                        ...
                        if (sumDate.get(newkey) == null) {
                            ...
                            int num2 =
                                    sumDate.get(newkey) == null ? 0 : Integer.parseInt(sumDate.get(newkey).toString());
    ...
    public Map<String, Object> getSumOrdersPrice(List<Map<String, Object>> list) {
        ...
        for (Map<String, Object> item : list) {
            allpricermb = allpricermb.add(item.get("orderpricermb") == null ? new BigDecimal("0")
                    : new BigDecimal(item.get("orderpricermb").toString()));
            allprice = allprice.add(item.get("orderprice") == null ? new BigDecimal("0")
                    : new BigDecimal(item.get("orderprice").toString()));
            allqty = allqty.add(item.get("quantity") == null ? new BigDecimal("0")
                    : new BigDecimal(item.get("quantity").toString()));
            allorder = allorder.add(item.get("ordersum") == null ? new BigDecimal("0")
                    : new BigDecimal(item.get("ordersum").toString()));
        }
    ...
    public Map<String, Object> searchOrderSummary(UserInfo userinfo, AmazonOrderSummaryDTO model) {
        ...
        for (c.setTime(beginDate); ChartPoint.checkTimeType(map.get("bytime").toString(), c, beginDate, endDate);
                ChartPoint.step(map.get("bytime").toString(), c, beginDate, endDate)) {
        ...
        if (last != null) {
            lastorder =
                    last.get("ordernumber") != null ? new BigDecimal(last.get("ordernumber").toString()) : lastorder;
