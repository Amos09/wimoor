...
public class ShipInboundOperationServiceImpl extends
        ServiceImpl<ShipInboundOperationMapper, ShipInboundOperation> implements IShipInboundOperationService {
        ...
        if (response != null) {
            if (old == null) {
                ...
                if (response.getOperationStatus() != null) {
                    ...
                    if (response.getOperationStatus().getValue().equals(OperationStatus.FAILED.getValue())
                            && response.getOperationProblems() != null) {
                ...
                if (response.getOperationStatus() != null && !old.getOperationStatus()
                        .equals(response.getOperationStatus().getValue())) {
    ...
    public ShipInboundOperation getOperation(AmazonAuthority auth, String operationid) {
        ...
        InboundOperationStatus response = iInboundApiHandlerService.getInboundOperationStatus(auth, operationid);
        one.setOpttime(new Date());
        if (response.getOperationStatus() != null && !one.getOperationStatus()
                .equals(response.getOperationStatus().getValue())) {
            one.setOperationStatus(response.getOperationStatus().getValue());
            if (response.getOperationStatus().getValue().equals(OperationStatus.FAILED.getValue())
                    && response.getOperationProblems() != null) {
    ...
    public ShipInboundOperation getOperation(AmazonAuthority auth, String fromid, String operation) {
        ...
        if (list != null && list.getRecords().size() > 0) {
            ...
            if (one.getOperationStatus() != null && !one.getOperationStatus()
                    .equals(OperationStatus.IN_PROGRESS.getValue())) {
                return one;
            }
            InboundOperationStatus response = iInboundApiHandlerService.getInboundOperationStatus(auth,
                    one.getOperationid());
            one.setOpttime(new Date());
            if (response.getOperationStatus() != null && !one.getOperationStatus()
                    .equals(response.getOperationStatus().getValue())) {
