...
public class FBAShipCycleController {
    ...
    public Result<?> updateStockCycleSku(@RequestBody FBAShipCycle cycle) {
        ...
        if (result > 0) {
            iAmzProductSalesPlanService.refreshData(cycle.getGroupid().toString(), cycle.getMarketplaceid(),
                    cycle.getSku());
    ...
    public Result<Map<String, Object>> updateStockAction(String marketplaceid, String groupid, String sku, String cycle,
            String mincycle, String fee) {
        ...
        int result = iFBAShipCycleService.updateStockCycle(groupid, marketplaceid, sku, stockcycle, mincycles, fees,
                userinfo);
        if (result > 0) {
