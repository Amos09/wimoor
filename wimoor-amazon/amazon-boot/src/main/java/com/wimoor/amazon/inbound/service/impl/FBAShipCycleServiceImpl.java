...
public class FBAShipCycleServiceImpl extends ServiceImpl<FBAShipCycleMapper, FBAShipCycle> implements
        IFBAShipCycleService {

    @CacheEvict(value = {"FbaCycle"}, allEntries = true)
    public int updateStockCycleTransType(FBAShipCycle cycle, UserInfo user) {
        int result = 0;
        FBAShipCycle stockcy = this.baseMapper.findShipCycleBySKU(cycle.getSku(), cycle.getMarketplaceid(),
                cycle.getGroupid().toString());
        if (stockcy == null) {
            stockcy = cycle;
    ...
    @CacheEvict(value = {"FbaCycle"}, allEntries = true)
    public int updateStockCycle(String groupid, String marketplaceid, String sku, Integer stockcycle, Integer mincycle,
            BigDecimal fee, UserInfo user) {
        int result = 0;
    ...
    @CacheEvict(value = {"FbaCycle"}, allEntries = true)
    public boolean updateMinCycle(String groupid, String marketplaceid, String type, String sku, Integer num,
            UserInfo user) {
        FBAShipCycle stockcy = this.baseMapper.findShipCycleBySKU(sku, marketplaceid, groupid);
        if (stockcy == null) {
            stockcy = new FBAShipCycle();
    ...
    @CacheEvict(value = {"FbaCycle"}, allEntries = true)
    public boolean updateFirstLegCharges(String groupid, String marketplaceid, String type, String sku, Integer num,
            UserInfo user) {
        FBAShipCycle stockcy = this.baseMapper.findShipCycleBySKU(sku, marketplaceid, groupid);
        if (stockcy == null) {
            stockcy = new FBAShipCycle();
