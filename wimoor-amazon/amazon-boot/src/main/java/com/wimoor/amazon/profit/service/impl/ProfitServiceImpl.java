...
public class ProfitServiceImpl implements IProfitService {
    ...
    /**
     * 按固定利率估算售价 成本=purchase+shipMent+others+FBA（所有亚马逊直接收费）+FBA的GST税+佣金+rate比率*售价+汇率损耗费+关税+进口GST税+销售GST税+VAT税+企业所得税;
     * 汇率损耗费=[售价-FBA（所有亚马逊直接收费）-佣金]*lostrate; 佣金=售价*R>minimum?售价*R:minimum; 售价 = 成本+售价*margin;售价=成本/(1-margin);
     * vat税费=售价*(vatRate/(1+vatRate)); 销售GST税费=售价*(selling_GSTRate/(1+selling_GSTRate)); 企业所得税 = 税率C * (销售价 -
     * 以上所有成本(除了进口GST税, FBA的GST税));
     ...
     * 2.price=[(1-C)*(purchase+shipMent+others+FBA（所有亚马逊直接收费）+
     * minimum+关税-FBA（所有亚马逊直接收费+fbaTax）*lostrate-minimum*lostrate)+fbaTax+进口GST税]/(1-C)*[1/(1+vatRate)-selling_GSTRate/(1+selling_GSTRate)-rateFee-lostrate]-margin
     *
     * @param referralrate
     */
    public List<Map<String, String>> calculateSellingPrice(CostDetail costDetail, int typeId, String country,
            BigDecimal referralrate) {
        List<Map<String, String>> resultList = new ArrayList<Map<String, String>>();
        List<BigDecimal> marginList = findMarginList();//固定参考利率列表
        ReferralFee resultfee = referralFeeService.getReferralFeeByTypeCountry(typeId,
                country);//根据国家和id或parent_id查询计算标准
        ...
        BigDecimal temp = new BigDecimal("1").divide(new BigDecimal("1").add(costDetail.getVatRate()), 4,
                RoundingMode.HALF_UP);
        BigDecimal temp2 = costDetail.getSelling_GSTRate()
                .divide(new BigDecimal("1").add(costDetail.getSelling_GSTRate()), 4, RoundingMode.HALF_UP);
        for (int i = 0; i < marginList.size(); i++) {
            Map<String, String> map = new HashMap<String, String>();
            Map<String, BigDecimal> priceResult = estimateSellingPrice(costDetail, referralR, minimum, FBA, temp, temp2,
                    marginList.get(i));
            if (priceResult != null) {
                ...
                if ("IN".equals(country)) {
                    ...
                    BigDecimal fbaTaxFee = costDetail.getFBA().add(closingFee).add(referralFee)
                            .multiply(costDetail.getFbaTax())
                            .setScale(4, RoundingMode.HALF_UP);
                    costDetail.setFbaTaxFee(fbaTaxFee);
                    FBA = getAmazonFeeButRef(costDetail);
                    priceResult = estimateSellingPrice(costDetail, referralR, minimum, FBA, temp, temp2,
                            marginList.get(i));
                    ...
                    if (referralFee.floatValue() > minimum.floatValue() && resultfee.getPercent2() != null
                            && resultfee.getPercent2().floatValue() > 0) {
     ...
     */
    public Map<String, BigDecimal> estimateSellingPrice(CostDetail costDetail, BigDecimal referralR, BigDecimal minimum,
            BigDecimal FBA,
        ...
        if (sellingPrice.multiply(referralR).doubleValue() < minimum.doubleValue()) {
            BigDecimal m2 = costDetail.getPurchase().add(costDetail.getShipment()).add(costDetail.getOthers())
                    .add(FBA).add(minimum).add(costDetail.getTax())
                    .subtract(FBA.add(costDetail.getFbaTaxFee()).multiply(costDetail.getCurrencyTransportRate()))
...
//		currencyUnitList.add("€");
    ...
    public BigDecimal calculateStorageFee(String country, ProfitConfigCountry profitConfigX,
            InputDimensions inputDimension, boolean isStandard)
    ...
    public BigDecimal determineOutboundWeight(String country, ProductTier productTier, InputDimensions inputDimension,
        ...
        outboundWeight = weight.add(boxweight).multiply(new BigDecimal("1000"))
                .setScale(2, RoundingMode.CEILING);// 进位取整数，模拟不足一克按一克算,单位：g
        ...
        if (profitServiceX != null) {
            ...
            if (isSmlAndLight && Arrays.asList(smlAndLightCountry).contains(country)) {
                ...
                if (productTierId != null) {
                    ...
                    productTierName =
                            productTier.getAmzName() == null ? productTier.getName() : productTier.getAmzName();
                    ...
                    FBA = profitServiceX.calculateSmlFBA(country, productTierId, outboundWeight, profitConfigCountry,
                            type);//各个国家重写的方法
                    BigDecimal sippfee = profitServiceX.calculateFBASIPP(country, productTierId, inputDimension,
                            isMedia, type, outboundWeight, profitConfigCountry, sipp);
            ...
            if (productTierName != null) {
                if (country.equals("US")) {
                    ...
                    BigDecimal weight = profitServiceX.getFBAFormatWeight(country, productTierId, inputDimension,
                            outboundWeight, profitConfigCountry);
    ...
    public CostDetail initCostDetail(String country, ProfitConfig profitCfgAll, InputDimensions inputDimension,
            ...
            boolean isSmlAndLight, Boolean sipp)
            throws BizException {
        // 对用户输入的长宽高重新排序
        ...
        if (channel.contains("fba")) {// 如果利润计算方案是FBA，印度目前只支持FBA
            if (isSmlAndLight && Arrays.asList(smlAndLightCountry).contains(country)) {
                ...
                if (sipp != null && sipp == false) {
                    sippfee = profitServiceX.calculateFBASIPP(country, productTierId, inputDimension, isMedia, type,
                            outboundWeight, profitConfigX, sipp);
                }
                FBA = FBA.subtract(sippfee);
                ...
                BigDecimal sippfee = profitServiceX.calculateFBASIPP(country, productTierId, inputDimension, isMedia,
                        type, outboundWeight, profitConfigX, sipp);
                FBA = FBA.subtract(sippfee);
                costDetail.setFBA(FBA);
                costDetail.setSipp(sippfee);
            }
        ...
        BigDecimal lostrate = profitConfigX.getLostrate()
                .divide(new BigDecimal("100"), 4, RoundingMode.HALF_UP);//Currency Transport fee
        BigDecimal sellerrate = profitConfigX.getSellerrate()
                .divide(new BigDecimal("100"), 4, RoundingMode.HALF_UP);//Marketing
        BigDecimal costrate = profitConfigX.getCostrate()
                .divide(new BigDecimal("100"), 4, RoundingMode.HALF_UP);//其它每单销售固定成本比率
//		BigDecimal promotion = profitConfigX.getPromotion().divide(new BigDecimal("100"),4,RoundingMode.HALF_UP);//all store promotion
        ...
        //企业所得税率
        BigDecimal corporateInRate = profitConfigX.getCorporateInRate()
...
//		costDetail.setPromotionRate(promotion);
        ...
        if ("IN".equals(country) && profitConfigX.isHasDeclaredValue()) {
            ...
            declaredValue = this.exchangeRateHandlerService.changeCurrencyByLocal(declaredValueCur, to,
                    declaredValue);// 单位转换
        } else {
        ...
        List<Map<String, String>> resultList = calculateSellingPrice(costDetail, typeId, country,
                referralrate);// 按固定利率估算售价和利润
        costDetail.setResultList(resultList);
        return costDetail;
    }


    BigDecimal calculateShipment(ProfitConfig profitcfg, ProfitConfigCountry profitConfigX, String currency, String to,
            InputDimensions inputDimension, BigDecimal shipment) {
        if (inputDimension == null || profitConfigX == null) {
            return new BigDecimal("0");
        }
        if (profitcfg.getShipmentstyle() != null && profitcfg.getShipmentstyle().equals("manually")
                && shipment == null) {
            return new BigDecimal("0");
        }
        if (shipment != null && shipment.floatValue()
                > 0.0001) {//profitcfg.getShipmentstyle().equals("manually")如果手动输入了运费，shipment用户页面输入的运费
            ...
            shipment = inputDimension.getWeight(InputDimensions.unit_kg).getValue()
                    .multiply(profitConfigX.getConstantw());
    ...
    public CostDetail getProfitByLocalData(String country, ProfitConfig profitcfg, InputDimensions inputDimension_amz,
            InputDimensions inputDimension_local,
            String isMedia, String type, int typeid, BigDecimal cost, String currency, BigDecimal price,
            String shipmentType,
        ...
        ////本地运费
        BigDecimal shipment = calculateShipment(profitcfg, profitConfigX, currency, to, inputDimension_local,
                shipmentfee);
        if (shipment == null) {
            return null;
        }
        ...
        if (channel.contains("fba")) {// 如果利润计算方案是FBA，印度目前只支持FBA
            Map<String, String> result = this.getProductTier(profitConfigX, inputDimension_amz, country, isMedia, price,
                    isSmlAndLight, type, null);
            ...
            if ("IN".equals(country)) {
                ...
                costDetail.setClosingFee(closingFee);
                //亚马逊收费GST税率默认18%, 应用于亚马逊销售佣金, Fixed Closing Fee, FBA fee等
                BigDecimal fbaTaxFee = costDetail.getFBA().add(closingFee).add(referralFee).multiply(fbaGSTTax)
                        .setScale(4, RoundingMode.HALF_UP);
                costDetail.setFbaTaxFee(fbaTaxFee);
            }
...
//		BigDecimal promotion = profitConfigX.getPromotion().divide(new BigDecimal("100"),4,RoundingMode.HALF_UP);//all store promotion
        ...
        //销售GST税率
        BigDecimal sellingGSTRate = profitConfigX.getSellingGSTRate()
                .divide(new BigDecimal("100"), 4, RoundingMode.HALF_UP);
        //企业所得税率
        BigDecimal corporateInRate = profitConfigX.getCorporateInRate()
                .divide(new BigDecimal("100"), 4, RoundingMode.HALF_UP);
...
//		costDetail.setPromotionRate(promotion);
    ...
    @SuppressWarnings("unused")
    public CostDetail getProfitByAmazonData(String country, ProfitConfig profitCfgAll,
            InputDimensions inputDimension_local, String isMedia,
            BigDecimal cost, String currency, BigDecimal price, FBAEstimatedFee fbaFee, ReferralFee ref,
            boolean isSmlAndLight, BigDecimal shipmentfee) throws BizException {
        // 使用报表的长宽高和重量
        ...
        if ("grams".equalsIgnoreCase(fbaFee.getUnitOfWeight())) {
            if (fbaFee.getItemPackageWeight() != null) {
                BigDecimal weight = fbaFee.getItemPackageWeight()
        ...
        ProfitConfigCountry profitConfigX = profitCfgAll.getProfitConfigCountry(country);//得到各个国家的利润方案
        ////本地运费
        BigDecimal shipment = calculateShipment(profitCfgAll, profitConfigX, currency, to, inputDimension_local,
                shipmentfee);
        if (shipment == null) {
            return null;
        }
...
//		referralFeeService.calculateReferralFee(typeid, price, country);
        if (price.setScale(2, RoundingMode.HALF_EVEN).floatValue() == fbaFee.getYourPrice().floatValue()) {
            ...
            BigDecimal referralRate = fbaFee.getEstimatedReferralFeePerUnit()
                    .divide(fbaFee.getYourPrice(), 2, RoundingMode.HALF_EVEN);//比率=佣金/售价；
            referralFee = price.multiply(referralRate).setScale(2, RoundingMode.HALF_EVEN);
            if (ref != null && ref.getLoweast() != null && referralFee != null
                    && ref.getLoweast().floatValue() > referralFee.floatValue()) {
...
//			}
            ...
            if ("IN".equals(country)) {
                ...
                //亚马逊收费GST税率默认18%, 应用于亚马逊销售佣金, Fixed Closing Fee, FBA fee等
                BigDecimal fbaTaxFee = fba.add(referralFee).add(fixclosingFee).multiply(profitConfigX.getFbaTaxes())
                        .divide(new BigDecimal("100"), 4, RoundingMode.HALF_UP);
                costDetail.setFbaTaxFee(fbaTaxFee);
            }
            if ("CA".equals(country)) {
                BigDecimal fbaTaxFee = fba.multiply(profitConfigX.getFbaTaxes())
                        .divide(new BigDecimal("100"), 4, RoundingMode.HALF_UP);
                costDetail.setFbaTaxFee(fbaTaxFee);
            }
            ...
            BigDecimal storageFee = profitServiceX.calculateStorageFee(country, profitConfigX, inputDimensionNew,
                    isStandard);
            costDetail.setStorageFee(storageFee);
        ...
        BigDecimal GSTRate = profitConfigX.getGstrate().divide(new BigDecimal("100"), 4, RoundingMode.HALF_UP);
        //销售GST税率
        BigDecimal selling_GSTRate = profitConfigX.getSellingGSTRate()
                .divide(new BigDecimal("100"), 4, RoundingMode.HALF_UP);
        //企业所得税率
        BigDecimal corporateInRate = profitConfigX.getCorporateInRate()
                .divide(new BigDecimal("100"), 4, RoundingMode.HALF_UP);
        ...
        costDetail.setImport_GST(import_gst);
//		汇率损耗费=[售价-FBA（所有亚马逊直接收费）-佣金]*lostrate;
        BigDecimal currencyLostFee = price.subtract(FBA.add(costDetail.getFbaTaxFee()))
                .multiply(costDetail.getCurrencyTransportRate());
        currencyLostFee = currencyLostFee.floatValue() > 0 ? currencyLostFee : new BigDecimal("0");
//		vat增值税费=售价*vat/(1+vat)
        BigDecimal vatFee = price.multiply(costDetail.getVatRate()
...
//		成本=purchase+shipMengt+others+FBA（所有亚马逊直接收费,含佣金）+比率*售价+汇率损耗费+vat+销售GST税+进口关税+进口GST税费;
    ...
    //根据售价计算产品成本详情及利润
    public CostDetail getCostDetail(ProfitQuery query, CostDetail costDetail, int typeId, BigDecimal referralrate,
            boolean isSmlAndLight, ProfitConfig profitcfg, Boolean sipp) {
        ...
        if (price != null && price.doubleValue() > 0) {
            ...
            if (profitcfg != null) {
                ...
                if (isSmlAndLight && Arrays.asList(smlAndLightCountry).contains(costDetail.getCountry())) {
                    ...
                    BigDecimal FBA = profitServiceX.calculateSmlFBA(costDetail.getCountry(),
            ...
            BigDecimal totalCost = costDetail.getPurchase().add(costDetail.getShipment()).add(costDetail.getOthers())
                    .add(FBA).add(costDetail.getFbaTaxFee()).add(referralFee)
                    .add(price.multiply(costDetail.getRateFee())).add(currencyLostFee).add(vatFee)
                    .add(costDetail.getTax()).add(costDetail.getImport_GST()).add(selling_GST)
                    .add(corporateInFee);// 计算所有成本
    ...
    public String determineSmlProductTier(String country, InputDimensions inputDimension, String isMedia) {
        ...
        InputDimensions dimensions = new InputDimensions(length, width, height, getDimUnit(country), weight,
        ...
        if (productTier != null && boxweight.compareTo(new BigDecimal("0")) > 0) {
            ...
            dimensions = new InputDimensions(length, width, height, getDimUnit(country), weight,
                    getWeightUnit(country));
            for (int i = 0; i < tierList.size(); i++) {
                Map<String, Object> tierMap = tierList.get(i);
                String format = tierMap.get("format").toString();
    ...
    public BigDecimal calculateSmlFBA(String country, String productTierId, BigDecimal outboundWeight,
            ProfitConfigCountry profitConfigCountry, String type) {
        BigDecimal value = new BigDecimal("0");
        return value;
    }
            ...
            for (int i = 1; i < 4; i++) {
                if (resultfee.getTop(i) != null && sellingPrice.subtract(resultfee.getTop(i)).doubleValue() > 0) {
                    ...
                    referralFee = referralFee
                            .add((resultfee.getTop(i)
                                    .subtract((i == 1 ? new BigDecimal("0.0") : resultfee.getTop(i - 1))))
