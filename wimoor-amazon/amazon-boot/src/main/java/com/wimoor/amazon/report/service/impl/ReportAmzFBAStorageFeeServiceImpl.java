        ...
        try {
            while ((line = br.readLine()) != null) {
                ...
                if (lineNumber > 0) {
                    ...
                    fBAStorageFee.setAverageQuantityOnHand(
                            getBigDecimalValue(info, titleList, "average_quantity_on_hand"));
                    fBAStorageFee.setAverageQuantityPendingRemoval(
                            getBigDecimalValue(info, titleList, "average_quantity_pending_removal"));
                    fBAStorageFee.setEstimatedTotalItemVolume(
                            getBigDecimalValue(info, titleList, "estimated_total_item_volume"));
                    ...
                    fBAStorageFee.setStorageUtilizationRatioUnits(
                            getStrValue(info, titleList, "storage_utilization_ratio_units"));
                    fBAStorageFee.setCurrency(getStrValue(info, titleList, "currency"));
                    fBAStorageFee.setQualifiesForInventoryDiscount(
                            getStrValue(info, titleList, "qualifies_for_inventory_discount"));
                    fBAStorageFee.setEligibleForInventoryDiscount(
                            getStrValue(info, titleList, "eligible_for_inventory_discount"));
                    fBAStorageFee.setBaseRate(getBigDecimalValue(info, titleList, "base_rate"));
                    fBAStorageFee.setUtilizationSurchargeRate(
                            getBigDecimalValue(info, titleList, "utilization_surcharge_rate"));
                    fBAStorageFee.setEstimatedMonthlyStorageFee(
                            getBigDecimalValue(info, titleList, "estimated_monthly_storage_fee"));
                    fBAStorageFee.setDangerousGoodsStorageType(
                            getStrValue(info, titleList, "dangerous_goods_storage_type"));
                    fBAStorageFee.setTotalIncentiveFeeAmount(
                            getBigDecimalValue(info, titleList, "total_incentive_fee_amount"));
                    fBAStorageFee.setBreakdownIncentiveFeeAmount(
                            getStrValue(info, titleList, "breakdown_incentive_fee_amount"));
                    fBAStorageFee.setAverageQuantityCustomerOrders(
                            getBigDecimalValue(info, titleList, "average_quantity_customer_orders"));
