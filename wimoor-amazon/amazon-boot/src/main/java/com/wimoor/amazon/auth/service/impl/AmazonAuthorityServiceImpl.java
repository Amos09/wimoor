...
public class AmazonAuthorityServiceImpl extends ServiceImpl<AmazonAuthorityMapper, AmazonAuthority> implements
        IAmazonAuthorityService {
        ...
        Runnable runnable = new Runnable() {
            public void run() {
                ...
                try {
                    ...
                    developerId = developerId.replace("\n", "");
                    GetAuthorizationCodeResponse codeResult = api.getAuthorizationCode(sellingPartnerId, developerId,
                            mwsAuthToken);
    ...
    public AmazonAuthority authSeller(String state, String selling_partner_id, String mws_auth_token,
        ...
        try {
            ...
            if (oldAuth != null) {
                ...
                Calendar cend = Calendar.getInstance();
                handlerReportService.requestAuthReport("id", auth.getId(), ReportType.ProductListings, false);
                handlerReportService.requestReport(auth.getId(), ReportType.OrdersByOrderDateReport, cstart, cend,
                        false);
                handlerReportService.requestAuthReport("id", auth.getId(), ReportType.InventoryReport, false);
                return auth;
            }
    ...
    public String getAuthUrl(String groupid, String marketplaceid) {
        ...
        result += "/apps/authorize/consent?application_id=" + apiBuildService.getAppid() + "&state=" + groupid + "@"
                + market.getAwsRegion();
        ...
        if (grouplist != null && grouplist.size() > 0) {
            for (AmazonGroup groupitem : grouplist) {
                ...
                for (Map<String, Object> authAdvItem : authAdvlist) {
                    ...
                    List<Map<String, Object>> advmarketList = this.baseMapper.selectAdvMarketByAdvAuthId(
                            authAdvItem.get("id").toString());
                    for (Map<String, Object> advcountry : advmarketList) {
                        String marketplaceid = advcountry.get("marketplaceId").toString();
                        String countrykey =
                                advcountry.get("marketplaceId").toString() + advcountry.get("sellerId").toString();
