    ...
    private ShopFee getShopFee(String settlementid) {
        ...
        for (Map<String, Object> item : list) {
            String transaction_type =
                    item.get("transaction_type") != null ? item.get("transaction_type").toString() : "#";
            if (transaction_type.equals("sku")) {
                shopfee.shop_in_principal =
                        item.get("principal") != null ? new BigDecimal(item.get("principal").toString())
                                : shopfee.shop_in_principal;
                shopfee.shop_quantity_orders =
                        item.get("quantity_orders") != null ? Integer.parseInt(item.get("quantity_orders").toString())
                                : shopfee.shop_quantity_orders;
                shopfee.shop_sales =
                        item.get("sales") != null ? Integer.parseInt(item.get("sales").toString()) : shopfee.shop_sales;
            } else if (item.get("amount") != null) {
                String amount_description =
                        item.get("amount_description") != null ? item.get("amount_description").toString() : "#";
                ...
                } else if ("FBA Long-Term Storage".equals(amount_description) || "StorageRenewalBilling".equals(
                        amount_description)) {
                    shopfee.shop_long_storage_fee = shopfee.shop_long_storage_fee.add(amount);
                    //shopfee.addFee("shop_long_storage_fee", amount);
                } else if ("Previous Reserve Amount Balance".equals(amount_description)
                        || "Current Reserve Amount".equals(amount_description)) {
                ...
                } else if ("FBA Inventory Reimbursement".equals(amount_type)
                        || "Manual Processing Fee Reimbursement".equals(amount_type) || "Item Fee Adjustment".equals(
                        amount_type)) {
                    shopfee.shop_reimbursement_fee = shopfee.shop_reimbursement_fee.add(amount);
                    //shopfee.addFee("shop_reimbursement_fee", amount);
                } else if ("DisposalComplete".equals(amount_description) || "RemovalComplete".equals(
                        amount_description)) {
...
//	    BigDecimal itemshopother = shopfee.getFee("shop_othershare").multiply(rate).setScale(2, RoundingMode.HALF_UP);
        ...
        BigDecimal itemcoupon_redemption_fee = shopfee.shop_coupon_redemption_fee.multiply(rate)
                .setScale(2, RoundingMode.HALF_UP);
        BigDecimal itemreserve_amount = shopfee.shop_reserve_amount.multiply(rate).setScale(2, RoundingMode.HALF_UP);
        BigDecimal itemreimbursement_fee = shopfee.shop_reimbursement_fee.multiply(rate)
                .setScale(2, RoundingMode.HALF_UP);
        BigDecimal itemshopdisposal = shopfee.shop_disposal_complete_fee.multiply(rate)
                .setScale(2, RoundingMode.HALF_UP);
...
//    	item.setShareShopOtherFee( setValue(shopfeesub,itemshopother,"shop_othershare"));
    ...
    private void addLocalFeeToSku(AmzSettlementSummarySku item, Map<String, Object> localitem) {
        ...
        BigDecimal setincome = item.getPrincipal().add(item.getCommission().add(item.getFbafee().add(item.getRefund()
                .add(item.getTax().add(item.getOtherfee().add(item.getShipping().add(item.getPromotion())))))));
        ...
        if (mycountryconfig != null) {
            if (mycountryconfig.getVatRate() != null) {
                vat = setincome.multiply(mycountryconfig.getVatRate())
                        .divide(new BigDecimal("100"), 2, RoundingMode.HALF_UP);
            }
            if (mycountryconfig.getLostrate() != null) {
                exchangelost = setincome.multiply(mycountryconfig.getLostrate())
                        .divide(new BigDecimal("100"), 2, RoundingMode.HALF_UP);
            }
            ...
            if (mycountryconfig.getCostrate() != null) {
                costrate = setincome.multiply(mycountryconfig.getCostrate())
                        .divide(new BigDecimal("100"), 2, RoundingMode.HALF_UP);
            }
            if (mycountryconfig.getSellerrate() != null
                ...
                marketfee = setincome.multiply(mycountryconfig.getSellerrate())
                        .divide(new BigDecimal("100"), 2, RoundingMode.HALF_UP);
            }
            if (mycountryconfig.getCorporateInRate() != null) {
                companytax = setincome.multiply(mycountryconfig.getCorporateInRate())
                        .divide(new BigDecimal("100"), 2, RoundingMode.HALF_UP);
            }
            if (mycountryconfig.getTaxrate() != null) {
                customstax = setincome.multiply(mycountryconfig.getTaxrate())
                        .divide(new BigDecimal("100"), 2, RoundingMode.HALF_UP);
            }
            if (config != null) {
            ...
            BigDecimal weight =
                    localitem.get("weight") != null ? (BigDecimal) localitem.get("weight") : new BigDecimal(0);
        ...
        realySales = realySales.subtract(
                item.getRefundsales() == null ? new BigDecimal("0") : new BigDecimal(item.getRefundsales()));
        item.setLocalPrice(
                localitem.get("itemprice") != null ? new BigDecimal(localitem.get("itemprice").toString()).multiply(
                        realySales) : new BigDecimal(0));
        item.setLocalOtherCost(
                localitem.get("otherCost") != null ? new BigDecimal(localitem.get("otherCost").toString()).multiply(
                        realySales) : new BigDecimal(0));
        item.setLocalReturnTax(item.getLocalPrice().multiply(localitem.get("vatrate") == null ? new BigDecimal("0")
                : new BigDecimal(localitem.get("vatrate").toString())));
...
//  }
        if (maxitem != null && (shopfeesub.shop_storage_fee.compareTo(new BigDecimal("0")) != 0
            ...
            maxitem.setShareCouponRedemptionFee(
                    shopfeesub.shop_coupon_redemption_fee.add(maxitem.getShareCouponRedemptionFee()));
