...
public class AssemblyFormServiceImpl extends ServiceImpl<AssemblyFormMapper, AssemblyForm> implements
        IAssemblyFormService {
    ...
    private void saveEntryFormList(List<AssemblyFormEntry> list, List<AssemblyFormEntry> oldentrylist)
            throws BizException {
...
//			}
        ...
        if (list != null) {
            for (Map<String, Object> item : list) {
                if (item != null && item.get("auditstatus") != null) {
                    item.put("auditstatusName",
                            AssemblyForm.getAuditstatusName(Integer.parseInt(item.get("auditstatus").toString())));
                }
            }
            return list.get(0);
        }
    ...
    @Transactional
    public Map<String, Object> updateAssemblyEvent(String formid, int amount_handle, UserInfo user)
            throws BizException {
        ...
        for (int i = 0; i < sublist.size(); i++) {
            ...
            if (subnumber <= 0) {
                throw new BizException(
                        "主产品【" + material.getSku() + "】库存不足，系统自动组装子产品【" + assembly.get("sku")
                                + "】组装清单错误");
    ...
    public void createAssemblyFormByShipment(UserInfo user, String warehouseid, Material material,
        ...
        if (assamount < needassembly) {
            throw new BizException(
                    "本地SKU【" + material.getSku() + "】库存不足，尝试使用待组装库存，因子产品库存不足无法组装");
        }
        List<AssemblyForm> formlist = this.baseMapper.getCanAssemblyFormByMaterial(user.getCompanyid(), warehouseid,
                material.getId());
        ...
        if (needassembly > 0) {
            throw new BizException(
                    "本地SKU【" + material.getSku() + "】库存不足，尝试使用待组装库存，因没有找到可用组装单无法自动组装。");
