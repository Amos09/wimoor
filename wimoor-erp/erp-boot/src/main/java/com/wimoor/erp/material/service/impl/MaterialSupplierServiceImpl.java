...
public class MaterialSupplierServiceImpl extends ServiceImpl<MaterialSupplierMapper, MaterialSupplier> implements
        IMaterialSupplierService {
    ...
    public void handleDefaultSspplier(String supplierid, Material material, String oldsupid, Integer moq,
            UserInfo user) {
        boolean isnew = true;
    ...
    //供应商列表
    public void saveOrUpdateSupplier(List<MaterialSupplierVO> supplierList, UserInfo user, String id,
            Material material) {
        ...
        if (supplierList != null && supplierList.size() > 0) {
            for (MaterialSupplierVO item : supplierList) {
                ...
                if (GeneralUtil.isNotEmpty(supid)) {
                    List<MaterialSupplierStepwise> stepList = materialSupplierStepwiseMapper.selectSupplierByMainId(id,
                            supid);
