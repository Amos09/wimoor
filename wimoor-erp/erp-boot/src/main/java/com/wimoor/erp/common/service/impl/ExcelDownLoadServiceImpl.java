...
public class ExcelDownLoadServiceImpl implements IExcelDownLoadService {
    ...
    public void uploadMaterialBaseInfoFile(UserInfo user, Row info, String mtype, Map<String, String> ownermap) {
        ...
        if (StrUtil.isNotEmpty(pkglength) && StrUtil.isNotEmpty(pkgwidth) && StrUtil.isNotEmpty(pkgheight)
                && StrUtil.isNotEmpty(pkgweight)) {
        ...
        if (StrUtil.isNotEmpty(itemlength) && StrUtil.isNotEmpty(itemwidth) && StrUtil.isNotEmpty(itemheight)
                && StrUtil.isNotEmpty(itemweight)) {
        ...
        if (StrUtil.isNotEmpty(boxlength) && StrUtil.isNotEmpty(boxwidth) && StrUtil.isNotEmpty(boxheight)
                && StrUtil.isNotEmpty(boxweight)) {
    ...
    public void uploadMaterialSupplierFile(UserInfo user, Row info) {
        ...
        if (supplierlist != null && supplierlist.size() > 0) {
            ...
            if (material != null) {
                ...
                if (StrUtil.isNotEmpty(othercost)) {
                    entity.setOthercost(new BigDecimal(othercost));
                }
                entity.setOpttime(new Date());
                entity.setOperator(user.getId());
                if (StrUtil.isNotEmpty(moq)) {
                    entity.setMOQ(Integer.parseInt(moq));
                }
                entity.setMaterialid(material.getId());
                entity.setIsdefault(true);
                if (StrUtil.isNotEmpty(deliverycycle)) {
                    entity.setDeliverycycle(Integer.parseInt(deliverycycle));
                }
                entity.setCreater(user.getId());
                entity.setCreatedate(new Date());
                if (StrUtil.isNotEmpty(badrate)) {
                    entity.setBadrate(Float.parseFloat(badrate));
                }
                    ...
                    if (stepArr.length > 0) {
                        for (int i = 0; i < stepArr.length; i++) {
                            ...
                            if (StrUtil.isNotEmpty(amount)) {
                                step.setAmount(Integer.parseInt(amount));
                            }
                            if (StrUtil.isNotEmpty(price)) {
                                step.setPrice(new BigDecimal(price));
                            }
