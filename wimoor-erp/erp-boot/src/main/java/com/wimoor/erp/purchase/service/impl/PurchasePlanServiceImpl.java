...
public class PurchasePlanServiceImpl extends ServiceImpl<PurchasePlanMapper, PurchasePlan> implements
        IPurchasePlanService {
    ...
    public List<PurchasePlan> getPlanByShopid(String shopid) throws ERPBizException {
        ...
        if (list != null && list.size() > 0) {
            for (PurchasePlan item : list) {
                List<PurchasePlanWareHouse> warehouselist = purchasePlanWareHouseService.getWareHouseIdForPlanId(
                        item.getId(), shopid);
    ...
    public Map<String, Object> getLastForms(List<String> ids) {
        ...
        for (Map<String, Object> data : datas) {
            if (data != null) {
                ...
                if (auditstatus != null) {
                    lastform = lastform + " <br> " + PurchaseFormEntry.getAuditstatusName(
                            Integer.parseInt(auditstatus.toString()));
                }
