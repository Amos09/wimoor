...
public class WarehouseController {
    ...
    public Result<IPage<Warehouse>> findWarehousePageList(@RequestBody WarehouseDTO dto) {
        UserInfo userinfo = UserInfoContext.get();
        IPage<Warehouse> list = warehouseService.findByCondition(dto.getPage(), dto.getSearch(),
                userinfo.getCompanyid(), dto.getFtype(), dto.getParentid());
        return Result.success(list);
    }
    ...
    @CacheEvict(value = {"warehosueCache"}, allEntries = true)
    public Result<Warehouse> updateDataAction(@ApiParam("查询当前仓库") @PathVariable("id") String id,
            @RequestBody Warehouse model) {
        UserInfo userinfo = UserInfoContext.get();
