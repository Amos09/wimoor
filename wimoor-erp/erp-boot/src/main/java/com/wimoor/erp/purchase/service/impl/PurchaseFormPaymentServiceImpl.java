...
public class PurchaseFormPaymentServiceImpl extends
        ServiceImpl<PurchaseFormPaymentMapper, PurchaseFormPayment> implements IPurchaseFormPaymentService {
    ...
    // save一系列finance
    private void saveRecord(FinAccount account, PurchaseFormPayment payment, PurchaseFormPayment oldpayment,
            UserInfo user) {
    ...
    public void cancelPayment(PurchaseFormPayment payment, String paytype, UserInfo user) {
        ...
        BigDecimal price = payment.getPayprice().floatValue() > 0 ? payment.getPayprice()
                : payment.getPayprice().multiply(new BigDecimal("-1"));
        faccountService.updateFinCancelChange(account, payment.getProjectid(), payment.getCreatedate(), price,
                actiontype);
    }


    @Transactional
    public int updatePayment(List<PurchaseFormPayment> paymentlist, PurchaseFormEntry entry, FinAccount account,
            UserInfo user) throws ERPBizException {
        int result = 0;
        BigDecimal oldpay = entry.getTotalpay();
        if (oldpay == null) {
            oldpay = new BigDecimal("0");
    ...
    public void handleAvgPrice(PurchaseFormEntry entry) {
        ...
        if (totalprice.compareTo(new BigDecimal(0)) > 0 && amount > 0) {
            ...
            if (totalinv != 0) {
                avgprice_after = (avgprice_before.multiply(new BigDecimal(amount_before)).add(totalprice)).divide(
                        new BigDecimal(totalinv),
                        2, RoundingMode.HALF_UP);
                avgother_after = (avgother_before.multiply(new BigDecimal(amount_before)).add(othercost)).divide(
                        new BigDecimal(totalinv),
