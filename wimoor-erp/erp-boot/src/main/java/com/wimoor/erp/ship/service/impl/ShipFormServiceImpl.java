...
public class ShipFormServiceImpl implements IShipFormService {
    ...
    public void fulfillableToOutbound(UserInfo user, ShipFormDTO dto) {
        ...
        for (int i = 0; i < dto.getList().size(); i++) {
            ...
            if (materialid == null) {
                ...
                if (m == null) {
                    throw new BizException(
                            "平台SKU【" + item.getSku() + "】对应本地SKU【" + item.getMsku() + "】无法找到对应信息");
                }
                materialid = m.getId();
            ...
            if (fulfillable < shipqty) {
                ...
                assemblyFormService.createAssemblyFormByShipment(user, warehouseid, material, dto.getFormid(),
                        needassembly);
            }
    ...
    private void addMaterialFullfillableToOutbound(UserInfo user, String number, String formid, String warehouseid,
    ...
    private void subMaterialOutbound(UserInfo user, String number, String formid, String warehouseid, String material,
            Integer amount) {
        InventoryParameter para = new InventoryParameter();
        para.setAmount(amount);
        para.setFormid(formid);
    ...
    public void addFullfillableToOutboundChange(String userid, String shopid, String formid, String number,
            String warehouseid, String material, Integer amount) {
        InventoryParameter para = new InventoryParameter();
        para.setAmount(amount);
        para.setFormid(formid);
        para.setMaterial(material);
    ...
    private void subMaterialFulfillableToOutbound(UserInfo user, String formid, String number, String warehouseid,
            String material, Integer amount) {
        InventoryParameter para = new InventoryParameter();
        para.setAmount(amount);
        para.setMaterial(material);
    ...
    public void subFulfillableToOutboundChange(String userid, String shopid, String number, String formid,
            String warehouseid, String material, Integer amount) {
        InventoryParameter para = new InventoryParameter();
        para.setAmount(amount);
        para.setMaterial(material);
    ...
    public void fulfillableOut(UserInfo user, ShipFormDTO dto) throws BizException {
        ...
        for (int i = 0; i < listmap.size(); i++) {
            ...
            if (item.getMsku() != null && item.getQuantity() != null) {
                Material m = this.materialService.getBySku(user.getCompanyid(), item.getMsku());
                item.setMaterialid(m.getId());
                subFulfillableOut(user, dto.getFormid(), dto.getNumber(), dto.getWarehouseid(), item.getMaterialid(),
                        item.getQuantity());
            }
        }
    }

    private void subFulfillableOut(UserInfo user, String formid, String number, String warehouseid, String material,
            Integer amount) {
        InventoryParameter para = new InventoryParameter();
        para.setAmount(amount);
        para.setMaterial(material);
    ...
    private void cancelfulfillableToOutbound(UserInfo user, ShipFormDTO dto) throws BizException {
        ...
        for (int i = 0; i < itemlist.size(); i++) {
            ...
            subMaterialFulfillableToOutbound(user, dto.getFormid(), dto.getNumber(), dto.getWarehouseid(),
