...
public class PurchaseFinanceFormServiceImpl extends
        ServiceImpl<PurchaseFinanceFormMapper, PurchaseFinanceForm> implements IPurchaseFinanceFormService {
    ...
    private List<PurchaseFormPayment> finconvertPayment(List<PurchaseFinanceFormPayment> finpaylist, UserInfo user) {
        if (finpaylist != null && finpaylist.size() > 0) {
            ...
            for (int i = 0; i < finpaylist.size(); i++) {
                ...
                FinAccount account = faccountService.getAccByMeth(user.getCompanyid(),
                        finform.getPaymentMethod().toString());
