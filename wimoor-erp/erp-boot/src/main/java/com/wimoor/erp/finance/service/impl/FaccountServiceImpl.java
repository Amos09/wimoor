...
public class FaccountServiceImpl extends ServiceImpl<FinAccountMapper, FinAccount> implements IFaccountService {
    ...
    @Transactional
    public void updateFinCancelChange(FinAccount account, String projectid, Date createtime, BigDecimal amount,
            String ftype) {
    ...
    public void saveFinDaily(FinAccount account, Date createtime, BigDecimal amount, String ftype) {
        ...
        FinJournalDaily oldfindaily = finJournalDailyMapper.selectByday(GeneralUtil.formatDate(createtime),
                account.getId());
        if (oldfindaily != null) {
    ...
    public void cancelFinDaily(FinAccount account, Date createtime, BigDecimal amount, String ftype) {
        FinJournalDaily oldfindaily = finJournalDailyMapper.selectByday(GeneralUtil.formatDate(createtime),
                account.getId());
        List<FinJournalDaily> oldailylist = finJournalDailyMapper.selectAfterByday(GeneralUtil.formatDate(createtime),
                account.getId());
        if (oldfindaily != null) {
    ...
    public void saveFinMonthly(FinAccount account, String projectid, Date createtime, BigDecimal amount, String ftype) {
        ...
        FinTypeJournalMonthly oldfinMonthly = finTypeJournalMonthlyMapper.selectByYearMonth(year, month, projectid,
                acct);
    ...
    // 月账单类型表操作
    public void cancelFinMonthly(FinAccount account, String projectid, Date createtime, BigDecimal amount,
            String ftype) {
        String acct = account.getId();
        String[] dateStr = GeneralUtil.formatDate(createtime).toString().split("-");
        String year = dateStr[0];
