...
public class PurchaseFormEntryHistory {
    ...
    public String getAuditstatusName() {
        ...
        if (totalin == 0 && totalpay.floatValue() < 0.01 && auditstatus == 3) {
            return "已撤销";
        }
        return "";
    }
    ...
    public static String getAuditstatusName(PurchaseFormEntryHistory pojo) {
        if (pojo.getAuditstatus() == 0) {
            return "退回";
        }
        if (pojo.getAuditstatus() == 1) {
            return "待审核";
        }
        if (pojo.getAuditstatus() == 2) {
            return "审核通过";
        }
        if (pojo.getAuditstatus() == 3) {
            return "已完成";
        }
        if (pojo.getTotalin() == 0 && pojo.getTotalpay().floatValue() < 0.01 && pojo.getAuditstatus() == 3) {
            return "已撤销";
        }
        return "";
    }
