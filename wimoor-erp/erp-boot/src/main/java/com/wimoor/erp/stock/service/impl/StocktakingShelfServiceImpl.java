 ...
 */
@Service
public class StocktakingShelfServiceImpl extends ServiceImpl<StocktakingShelfMapper, StocktakingShelf> implements
        IStocktakingShelfService {
    ...
    public List<StocktakingShelf> listData(String stocktakingid) {
        List<StocktakingShelf> list = this.baseMapper.selectList(
                new LambdaQueryWrapper<StocktakingShelf>().eq(StocktakingShelf::getStocktakingid, stocktakingid));
        return list;
    }
    ...
    public void saveData(List<StocktakingShelf> list) {
        ...
        if (list != null && list.size() > 0) {
            ...
            this.baseMapper.delete(
                    new LambdaQueryWrapper<StocktakingShelf>().eq(StocktakingShelf::getStocktakingid, stockid));
        }
        for (StocktakingShelf item : list) {
    ...
    public void cancel(String id) {
        ...
        List<StocktakingShelf> list = this.baseMapper.selectList(
                new LambdaQueryWrapper<StocktakingShelf>().eq(StocktakingShelf::getStocktakingid, id));
        for (StocktakingShelf item : list) {
            WarehouseShelf shelf = iWarehouseShelfService.getById(item.getShelfid());
