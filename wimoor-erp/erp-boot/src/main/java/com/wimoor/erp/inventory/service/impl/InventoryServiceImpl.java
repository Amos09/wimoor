...
public class InventoryServiceImpl extends ServiceImpl<InventoryMapper, Inventory> implements IInventoryService {
    ...
    public int AddStockByStatus(InventoryParameter para, Status status, Operate operate) throws BizException {
        int count = 0;
        LockCheckUtils lock = new LockCheckUtils(stringRedisTemplate,
                "StockInventory" + para.getWarehouse() + para.getMaterial());
        try {
            para = para.clone();
            ...
            Inventory addinv = this.baseMapper.selectNowInv(para.getWarehouse(), para.getMaterial(), para.getShopid(),
                    para.getInvStatus(status));
            if (addinv != null) {
    ...
    public int SubStockByStatus(InventoryParameter para, Status status, Operate operate) throws BizException {
        ...
        try {
            ...
            handleStatusRecord(para, recform, "start");
            Warehouse warehouseobj = isOkForOperate(para.getWarehouse(), para.getStatus());
            Inventory subinv = this.baseMapper.selectNowInv(para.getWarehouse(), para.getMaterial(), para.getShopid(),
                    para.getInvStatus(status));
            if (subinv != null) {
    ...
    public IPage<Map<String, Object>> selectInventoryCost(Page<?> page, String warehouseid, String sku, String shopid,
            String byday) {
    ...
    public void setExcelBookInventoryReport(SXSSFWorkbook workbook, List<Map<String, Object>> warehouseList,
            ...
            for (String key : titlechange.keySet()) {
                ...
                if (key.equals("dimension")) {
                    if (skumap.get("length") == null || skumap.get("width") == null || skumap.get("height") == null) {
                        ...
                        cell.setCellValue(skumap.get("length").toString() + "*" + skumap.get("width").toString() + "*"
                                + skumap.get("height").toString());
                    }
                } else {
    ...
    public List<Map<String, Object>> selectInventoryCostAll(String warehouseid, String sku, String shopid,
            String byday) {
        if (GeneralUtil.isEmpty(sku)) {
            sku = null;
        } else {
    ...
    public Inventory selectAllInvSubWarehouse(String warehouseid, String materialid, String shopid,
            Status fulfillable) {
