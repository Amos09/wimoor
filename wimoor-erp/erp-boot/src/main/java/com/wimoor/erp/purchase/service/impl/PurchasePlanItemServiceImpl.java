...
public class PurchasePlanItemServiceImpl extends ServiceImpl<PurchasePlanItemMapper, PurchasePlanItem> implements
        IPurchasePlanItemService {
    ...
    public List<Map<String, Object>> getList(String shopid, String planid, List<String> materialidList) {
        ...
        for (Map<String, Object> item : list) {
            ...
            if (item.get("groupid") != null) {
                try {
                    Result<?> result = amazonClientOneFeignManager.getAmazonGroupByIdAction(
                            item.get("groupid").toString());
            ...
            Map<String, Object> pricemap = iStepWisePriceService.getMaterialPriceByAmount(
                    item.get("materialid").toString(), Integer.parseInt(item.get("amount").toString()));
    ...
    public List<Map<String, Object>> getList(String shopid, String planid, String batchnumber) {
        ...
        for (Map<String, Object> item : list) {
            ...
            Map<String, Object> pricemap = iStepWisePriceService.getMaterialPriceByAmount(item.get("mid").toString(),
                    amount);
            if (pricemap != null && pricemap.get("itemprice") != null) {
                item.put("price", pricemap.get("itemprice"));
            }
            ...
            if (item.get("issfg") != null && item.get("issfg").toString().equals("1")) {
                ...
                for (AssemblyVO subitem : sublist) {
                    ...
                    Map<String, Object> subpricemap = iStepWisePriceService.getMaterialPriceByAmount(
                            subitem.getSubmid(), subamount);
                ...
                if (sublist != null && sublist.size() > 0) {
                    for (int i = 0; i < sublist.size(); i++) {
                        ...
                        if (StrUtil.isNotEmpty(warehouseid)) {
                            ...
                            map = assemblyFormService.selectSubASList(null, submid == null ? null : submid.toString(),
                                    shopid);
