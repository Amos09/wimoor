...
public class WarehouseShelfInventoryServiceImpl extends
        ServiceImpl<WarehouseShelfInventoryMapper, WarehouseShelfInventory> implements IWarehouseShelfInventoryService {
    ...
    public List<MaterialVO> formInvAssemblyShelf(UserInfo user, QuotaInfoDTO dto) {
        ...
        for (MaterialVO skuvo : dto.getList()) {
            ...
            item.setShelfInvRecordList(
                    iWarehouseShelfInventoryOptRecordService.getRecordVo(shopid, dto.getFormid(), "outstockform",
                            item.getId()));
            Material m = materialService.getById(item.getId());
            item.setName(m.getName());
            item.setBoxnum(m.getBoxnum());
            item.setConList(iMaterialConsumableService.selectConsumableByMainmid(m.getId(), dto.getWarehouseid(),
                    m.getShopid()));
            if (m != null && m.getIssfg() != null && m.getIssfg().equals("1")) {
                ...
                for (AssemblyVO assvo : assvolist) {
                    ...
                    Map<String, Object> map = inventoryService.findInvDetailById(assvo.getSubmid(),
                            dto.getWarehouseid(), shopid);
                    if (map != null) {
                        assvo.setInbound(
                                map.get("inbound") != null ? Integer.parseInt(map.get("inbound").toString()) : 0);
                        assvo.setFulfillable(
                                map.get("fulfillable") != null ? Integer.parseInt(map.get("fulfillable").toString())
                                        : 0);
                        assvo.setOutbound(
                                map.get("outbound") != null ? Integer.parseInt(map.get("outbound").toString()) : 0);
                    }
                    List<WarehouseShelfInventoryVo> mshelfinvVoList = findByMaterial(shopid, warehouse,
                            assvo.getSubmid());
                    for (WarehouseShelfInventoryVo inv : mshelfinvVoList) {
                        inv.setShelfname(iWarehouseShelfService.getAllParentName(inv.getShelfid()));
                    }
                    assvo.setShelfInvList(mshelfinvVoList);
                    assvo.setShelfInvRecordList(
                            iWarehouseShelfInventoryOptRecordService.getRecordVo(shopid, dto.getFormid(),
                                    "outstockform", assvo.getSubmid()));
                }
    ...
    public ShipInboundShipmenSummarytVo formInvAssemblyShelf(ShipInboundShipmenSummarytVo itemsum) {
        ...
        for (ShipInboundItemVo item : itemvoList) {
            ...
            List<WarehouseShelfInventoryVo> shelfinvVoList = findByMaterial(itemsum.getShopid(), warehouse,
                    item.getMaterialid());
            for (WarehouseShelfInventoryVo inv : shelfinvVoList) {
                inv.setShelfname(iWarehouseShelfService.getAllParentName(inv.getShelfid()));
            }
            item.setShelfInvList(shelfinvVoList);
            item.setShelfInvRecordList(
                    iWarehouseShelfInventoryOptRecordService.getRecordVo(itemsum.getShopid(), itemsum.getCheckinv(),
            ...
            if (m != null && m.getIssfg() != null && m.getIssfg().equals("1")) {
                ...
                for (AssemblyVO assvo : assvolist) {
                    ...
                    Map<String, Object> map = inventoryService.findInvDetailById(assvo.getSubmid(),
                            itemsum.getWarehouseid(), itemsum.getShopid());
                    if (map != null) {
                        assvo.setInbound(
                                map.get("inbound") != null ? Integer.parseInt(map.get("inbound").toString()) : 0);
                        assvo.setFulfillable(
                    ...
                    List<WarehouseShelfInventoryVo> mshelfinvVoList = findByMaterial(itemsum.getShopid(), warehouse,
                            assvo.getSubmid());
                    for (WarehouseShelfInventoryVo inv : mshelfinvVoList) {
                        inv.setShelfname(iWarehouseShelfService.getAllParentName(inv.getShelfid()));
                    }
                    assvo.setShelfInvList(mshelfinvVoList);
                    assvo.setShelfInvRecordList(
                            iWarehouseShelfInventoryOptRecordService.getRecordVo(itemsum.getShopid(),
                                    itemsum.getCheckinv(), "outstockform", assvo.getSubmid()));
                }
                item.setAssemblyList(assvolist);
            }
        }
    ...
    /**
     * 刷新当前库存中的产品对应容量，之所以对所有这个库位对应的这个产品进行刷新，是考虑客户修改产品尺寸之后没有做对应的刷新库位中容量的动作。 可以最大程度的保证库位容量的准确。
     *
     * @param inv
     */
