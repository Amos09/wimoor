 ...
 */
@Service
public class StocktakingWarehouseServiceImpl extends
        ServiceImpl<StocktakingWarehouseMapper, StocktakingWarehouse> implements IStocktakingWarehouseService {
    ...
    public List<StocktakingWarehouse> listData(String stocktakingid) {
        List<StocktakingWarehouse> list = this.baseMapper.selectList(
                new LambdaQueryWrapper<StocktakingWarehouse>().eq(StocktakingWarehouse::getStocktakingid,
                        stocktakingid));
        return list;
    }
    ...
    public void saveData(List<StocktakingWarehouse> list) {
        ...
        if (list != null && list.size() > 0) {
            ...
            this.baseMapper.delete(
                    new LambdaQueryWrapper<StocktakingWarehouse>().eq(StocktakingWarehouse::getStocktakingid, stockid));
        }
        for (StocktakingWarehouse item : list) {
    ...
    public void cancel(String id) {
        ...
        List<StocktakingWarehouse> list = this.baseMapper.selectList(
                new LambdaQueryWrapper<StocktakingWarehouse>().eq(StocktakingWarehouse::getStocktakingid, id));
        for (StocktakingWarehouse item : list) {
            Warehouse warehouse = iWarehouseService.getById(item.getWarehouseid());
