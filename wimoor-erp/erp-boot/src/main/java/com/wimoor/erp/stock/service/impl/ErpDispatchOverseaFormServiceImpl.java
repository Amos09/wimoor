...
public class ErpDispatchOverseaFormServiceImpl extends
        ServiceImpl<ErpDispatchOverseaFormMapper, ErpDispatchOverseaForm> implements IErpDispatchOverseaFormService {
    ...
    public String uploadDispatchStockByExcel(Sheet sheet, UserInfo user) {
        ...
        for (int i = 4; i <= sheet.getLastRowNum(); i++) {
            ...
            if (material == null) {
                ...
                //判断库存是否充足
                Inventory inv = inventoryService.selectNowInv(wh_out.getId(), material.getId(), shopid,
                        Status.fulfillable);
    ...
    public Map<String, Object> saveForm(UserInfo user, ErpDispatchOverseaForm dispatchForm, JSONArray skuMap) {
        ...
        if (skuMap.size() > 0) {
            ...
            List<Map<String, Object>> oldentrylist = iErpDispatchOverseaFormEntryService.selectByFormid(
                    dispatchForm.getId());
    ...
    public void changeInventory(UserInfo user, ErpDispatchOverseaForm dispatchForm) {
        List<ErpDispatchOverseaFormEntry> entrylist = iErpDispatchOverseaFormEntryService.findByFormid(
                dispatchForm.getId());
        InventoryParameter parameter_out = new InventoryParameter();
        parameter_out.setShopid(dispatchForm.getShopid());
        parameter_out.setFormid(dispatchForm.getId());
    ...
    public List<Map<String, Object>> getShipArrivalTimeRecord(String shopid, String country, String sku,
            String groupid) {
