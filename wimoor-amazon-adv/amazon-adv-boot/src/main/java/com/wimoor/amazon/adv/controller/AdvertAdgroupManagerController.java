...
public class AdvertAdgroupManagerController {
    ...
    public Result<?> getAdGroupAction(String profileid, String campaignsid, String campaignType) {
        ...
        if (campaignType != null && "sd".equals(campaignType.toLowerCase())) {
            return Result.success(
                    amzAdvAdgroupsSDService.getAdGroupByCampaignsId(user, new BigInteger(profileid), campaignsid));
        } else if (campaignType != null && "sb".equals(campaignType.toLowerCase())) {
            return Result.success(
                    amzAdvAdgroupsHsaService.getAdGroupByCampaignsId(user, new BigInteger(profileid), campaignsid));
        } else {
            return Result.success(
                    amzAdvAdGroupService.getAdGroupByCampaignsId(user, new BigInteger(profileid), campaignsid));
        }
    }

    @ApiOperation("查询广告组")
    @PostMapping("/getAdGroupList")
    public Result<PageList<Map<String, Object>>> getAdGroupListAction(
            @ApiParam("查询广告组") @RequestBody QueryForList query) {
        UserInfo user = UserInfoContext.get();
    ...
    public Result<?> getAdGroupsBidAction(@PathVariable String profileid, @PathVariable String campaignType,
            String adgroupid) {
        UserInfo user = UserInfoContext.get();
        if ("sd".equals(campaignType.toLowerCase())) {
            ...
            return Result.success(
                    amzAdvAdGroupService.getAdGroupsBid(user, new BigInteger(profileid), new BigInteger(adgroupid)));
        } else {
            return Result.success();
        }
    }
    ...
    public Result<?> createAdGroupsAction(@PathVariable String profileid, @PathVariable String campaignType,
    ...
    public Result<String> updateAdGroupsAction(@PathVariable String profileid, @PathVariable String campaignType,
            @RequestBody Map<String, Object> params) {
        UserInfo user = UserInfoContext.get();
        String json = params.get("jsonstr").toString();
        JSONObject adGroup = GeneralUtil.getJsonObject(json);
    ...
    public Result<Map<String, Object>> getAdGroupChartAction(
    ...
    public Result<?> getAdgroupsAction(@PathVariable String profileid, @PathVariable String campaignType,
        ...
        if (campaignType.toLowerCase().equals("sb")) {
            for (String item : idlist) {
                ...
                param.put("includeExtendedDataFields", false);
                List<AmzAdvAdgroupsHsa> list = amzAdvAdgroupsHsaService.amzListAdGroups(new BigInteger(profileid),
                        param);
                result.addAll(list);
            }
        ...
        if (campaignType.toLowerCase().equals("sd")) {
            for (String item : idlist) {
                ...
                Map<String, Object> param = new HashMap<String, Object>();
                param.put("adGroupIdFilter", ids);
                List<AmzAdvAdgroupsSD> list = amzAdvAdgroupsSDService.amzListAdGroupsExt(user,
    ...
    public Result<?> deleteAdgroupsAction(@PathVariable String profileid, @PathVariable String campaignType,
            @RequestBody List<String> idlist) {
        if (idlist == null) {
            return Result.failed();
        }
        ...
        if (campaignType.toLowerCase().equals("sb")) {
            ...
            param.put("adGroupIdFilter", adGroupIdFilter);
            param.put("includeExtendedDataFields", true);
            List<AmzAdvAdgroupsHsa> list = amzAdvAdgroupsHsaService.amzDeleteAdGroups(user, new BigInteger(profileid),
                    param);
            result.addAll(list);
        }
        if (campaignType.toLowerCase().equals("sd")) {
    ...
    public Result<?> getAdgroupExtListAction(@PathVariable String profileid, @PathVariable String campaignType,
            @RequestBody List<String> idlist) {
        if (idlist == null) {
            return Result.failed();
        }
        ...
        if (campaignType.toLowerCase().equals("sd")) {
            UserInfo user = UserInfoContext.get();
            for (String item : idlist) {
                AmzAdvAdgroupsSD adgroup = amzAdvAdgroupsSDService.amzGetSDAdGroupExt(user, new BigInteger(profileid),
                        item);
                result.add(adgroup);
            }
        }
