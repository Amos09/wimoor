...
public class AmzAdvCampaignsSDServiceImpl extends BaseService<AmzAdvCampaignsSD> implements IAmzAdvCampaignsSDService {
    ...
    public List<AmzAdvCampaignsSD> amzListCampaignsEx(UserInfo user, BigInteger profileId,
    ...
    public AmzAdvCampaignsSD amzArchiveSpCampaigns(UserInfo user, BigInteger profileId, String campaignId) {
        ...
        if (StringUtil.isNotEmpty(response)) {
            ...
            if ("SUCCESS".equals(a.getString("code"))) {
                ...
                amzAdvOperateLogService.saveOperateLog("AmzAdvCampaignsSD", user.getId(), profileId, campaign,
                        oldCampaign);
                return campaign;
            } else {
                String errormsg = "";
    ...
    public List<AmzAdvCampaignsSD> amzCreateCampaigns(UserInfo user, BigInteger profileId,
            List<AmzAdvCampaignsSD> campaignsList) {
        AmzAdvProfile profile = amzAdvAuthService.getAmzAdvProfileByKey(profileId);
        ...
        if (StringUtil.isNotEmpty(response)) {
            ...
            for (int i = 0; i < campaignsList.size(); i++) {
                ...
                if ("SUCCESS".equals(item.getString("code"))) {
                    ...
                    if (Dbcampaigns == null) {
                        this.save(campaigns);
                        amzAdvOperateLogService.saveOperateLog("AmzAdvCampaignsSD", user.getId(), profileId, campaigns,
                                null);
                    }
                } else {
    ...
    public List<AmzAdvCampaignsSD> amzUpdateSDCampaigns(UserInfo user, BigInteger profileId,
            List<AmzAdvCampaignsSD> campaignsList) {
        if (campaignsList == null || campaignsList.size() <= 0) {
            return null;
        }
        ...
        String response = apiBuildService.amzAdvPut_V2(profile, url,
                JSONArray.toJSONString(array, SerializerFeature.WriteMapNullValue));
        if (StringUtil.isNotEmpty(response)) {
            String errormsg = "";
            JSONArray a = GeneralUtil.getJsonArray(response.toString());
