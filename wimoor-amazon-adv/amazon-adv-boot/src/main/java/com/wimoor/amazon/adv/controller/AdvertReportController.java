...
public class AdvertReportController {
    ...
    public String downExcelDateAction(@RequestBody QueryForDownload dto, HttpServletResponse response) {
        ...
        if (!StringUtil.isEmpty(groupid) || !StringUtil.isEmpty(marketplaceid) || !StringUtil.isEmpty(
                dto.getProfileid())) {
        ...
        try {
            ...
            response.addHeader("Content-Disposition",
                    "attachment;fileName=" + campaigntype + " " + reporttype + " report.xlsx");// 设置文件名
    ...
    public Result<Map<String, Object>> getMonthSumAction(@RequestBody QueryForSumProductDTO dto) {
        ...
        if (StringUtil.isNotEmpty(end)) {
            ...
            if (endarray.length >= 2) {
                String endate = GeneralUtil.getLastDayOfMonth(Integer.parseInt(endarray[0].trim()),
                        Integer.parseInt(endarray[1].trim()));
