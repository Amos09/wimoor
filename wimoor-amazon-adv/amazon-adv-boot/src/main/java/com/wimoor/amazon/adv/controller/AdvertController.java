...
public class AdvertController extends ERPBaseController<AmzAdvAuth> {
    ...
    public Result<?> getCampaignsNotArchivedAction(@RequestBody QueryForList query) {
        if (StringUtil.isNotEmpty(query.getProfileid())) {
            ...
            if (query.getCampaignType().equals("SB")) {
                PageList<AmzAdvCampaignsHsa> listsb = amzAdvCampaignHsaService.getHsaCampaignsNotArchivedByprofile(
                        query);
    ...
    public Object getKeywordsAction(HttpServletRequest request, Model model) {
        ...
        List<AmzAdvKeywords> list = amzAdvKeywordsService.getKeywordsByAdgroupId(new BigInteger(profileid), adgroupid,
                null);
        return list;
    }
    ...
    public Object getadvDataByKeywordAction(HttpServletRequest request, Model model) {
        ...
        if (StringUtil.isNotEmpty(matchType) && StringUtil.isNotEmpty(keywords) && StringUtil.isNotEmpty(adGroupId)
            ...
            String data = amzAdvBidReCommendService.bidRecommendations(new BigInteger(profileid), adGroupId, keywords,
                    matchType);
            return data;
        } else {
    ...
    public static BigDecimal operateMapsDouble(Map<String, Object> map, String name) {
        ...
        if (acos != null) {
            ...
            if (acosstr.equals("--") || StringUtil.isEmpty(acosstr) || Double.parseDouble(acosstr) > Double.parseDouble(
                    "1000000000")) {
    ...
    public static Map<String, Object> SumAdvertDate(List<Map<String, Object>> list) {
        ...
        if (list != null && list.size() > 0) {
            ...
            for (Map<String, Object> item : list) {
                ...
                totalattributedUnitsOrdered = totalattributedUnitsOrdered.add(
                        AdvertController.operateMapsDouble(item, "attributedUnitsOrdered"));
            }
            if (totalsumSales.compareTo(BigDecimal.ZERO) == 1) {
                BigDecimal totalacos = totalcost.multiply(new BigDecimal("100"))
                        .divide(totalsumSales, 2, RoundingMode.HALF_UP);
            ...
            if (totalclicks.compareTo(BigDecimal.ZERO) == 1) {
                ...
                BigDecimal totalcsrt = totalattributedUnitsOrdered.multiply(new BigDecimal("100"))
                        .divide(totalclicks, 2, RoundingMode.HALF_UP);
            ...
            if (totalimpressions.compareTo(BigDecimal.ZERO) == 1) {
                BigDecimal totalctr = totalclicks.multiply(new BigDecimal("100"))
                        .divide(totalimpressions, 2, RoundingMode.HALF_UP);
