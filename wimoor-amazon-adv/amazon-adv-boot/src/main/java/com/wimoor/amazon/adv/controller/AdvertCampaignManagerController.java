...
public class AdvertCampaignManagerController {
    ...
    public Result<PageList<Map<String, Object>>> getCampaignListAction(
            @ApiParam("查询广告活动") @RequestBody QueryForList query) {
    ...
    public Result<Object> getgetCampaignDetailAction(@PathVariable String profileid,
    ...
    public Result<?> createCampaignListAction(@RequestBody Map<String, Object> params) {
        ...
        for (int i = 0; i < campaignArray.size(); i++) {
            ...
            } else if (campaignType != null && ("HSA".equals(campaignType.toUpperCase()) || "SB".equals(
    ...
    public Result<Map<String, Object>> getCampaignChartAction(
            @ApiParam("查询广告活动") @RequestBody QueryForList query) {
        UserInfo user = UserInfoContext.get();
        Map<String, Object> map = AdvertController.amzAdvParameterMap(query);
    ...
    public Result<?> getCampaignBudgetUsageAction(@PathVariable String profileid, @PathVariable String CampaignType,
        ...
        if (CampaignType.toLowerCase().equals("sp")) {
            ...
            return Result.success(
                    amzAdvCampaignService.amzGetCampaignBudgetUsage(user, new BigInteger(profileid), param));
        }
        if (CampaignType.toLowerCase().equals("sd")) {
            JSONObject param = new JSONObject();
            param.put("campaignIds", ids);
            return Result.success(
                    amzAdvCampaignsSDService.amzGetCampaignBudgetUsage(user, new BigInteger(profileid), param));
        }
        if (CampaignType.toLowerCase().equals("sb")) {
            JSONObject param = new JSONObject();
            param.put("campaignIds", ids);
            return Result.success(
                    amzAdvCampaignHsaService.amzGetCampaignBudgetUsage(user, new BigInteger(profileid), param));
        }
        return Result.failed();
    }

    @PostMapping("/getCampaigns/{profileid}/{CampaignType}")
    public Result<?> getCampaignListAction(@PathVariable String profileid, @PathVariable String CampaignType,
            @RequestBody List<String> ids) {
        UserInfo user = UserInfoContext.get();
        ...
        if (CampaignType.toLowerCase().equals("sd")) {
            ...
            if (ids != null) {
                for (String id : ids) {
                    AmzAdvCampaignsSD sd = amzAdvCampaignsSDService.amzGetCampaigns(user, new BigInteger(profileid),
                            id);
                    if (sd != null) {
                        result.add(sd);
                    }
    ...
    public Result<?> deleteCampaignListAction(@PathVariable String profileid, @PathVariable String CampaignType,
            @RequestBody List<String> ids) {
        UserInfo user = UserInfoContext.get();
        List<Object> result = null;
        if (StringUtil.isEmpty(CampaignType)) {
        ...
        if (CampaignType.toLowerCase().equals("sd")) {
            ...
            if (ids != null) {
                for (String id : ids) {
                    AmzAdvCampaignsSD sd = amzAdvCampaignsSDService.amzArchiveSpCampaigns(user,
    ...
    public Result<?> getCampaignExtListAction(@PathVariable String profileid, @PathVariable String CampaignType,
            @RequestBody List<String> ids) {
        UserInfo user = UserInfoContext.get();
        if (StringUtil.isEmpty(CampaignType)) {
            throw new BizException("广告类型不能为空");
    ...
    public Result<List<Map<String, Object>>> getCampaignPlacementAction(String profileid, String campaignId,
            String campaignType,
    ...
    public Result<?> budgetRulesRecommendationsAction(@PathVariable String profileid, @PathVariable String CampaignType,
            @PathVariable String campaignId) {
        UserInfo user = UserInfoContext.get();
        return Result.success(
                amzAdvRecommendationsService.budgetRulesRecommendations(user, new BigInteger(profileid), campaignId));
    }

    @PostMapping("/initialBudgetRecommendation/{profileid}/{CampaignType}")
    public Result<?> initialBudgetRecommendationAction(@PathVariable String profileid,
            @PathVariable String CampaignType, @RequestBody JSONObject param) {
        UserInfo user = UserInfoContext.get();
        return Result.success(
                amzAdvRecommendationsService.initialBudgetRecommendation(user, new BigInteger(profileid), param));
    }

    @PostMapping("/budgetRecommendations/{profileid}/{CampaignType}")
    public Result<?> budgetRecommendationsAction(@PathVariable String profileid, @PathVariable String CampaignType,
            @RequestBody JSONObject param) {
        UserInfo user = UserInfoContext.get();
        return Result.success(
                amzAdvRecommendationsService.budgetRecommendations(user, new BigInteger(profileid), param));
    }

    @PostMapping("/campaignRecommendations/{profileid}/{CampaignType}")
    public Result<?> campaignRecommendationsAction(@PathVariable String profileid, @PathVariable String CampaignType,
            @RequestBody JSONObject param) {
        UserInfo user = UserInfoContext.get();
        return Result.success(
                amzAdvRecommendationsService.campaignRecommendations(user, new BigInteger(profileid), param));
    }
}
