...
/**
 * Used to create, read, update or delete ad groups. The following table describes the service behavior in terms of the
 * URL space and HTTP methods supported by the service-managed resources. Note: Ad groups are only managed for Sponsored
 * Products and do not apply to Sponsored Brands.
 *
...
public interface IAmzAdvAdGroupService extends IService<AmzAdvAdgroups> {
    ...
    /**
     * Create one or more ad groups A list of up to 100 ad groups to be created. Required fields for ad group creation
     * are: campaignId, name, state, and defaultBid
     *
     * @param profileId
     * @param adGroupArray
     * @return
    ...
    /**
     * A list of up to 100 updates containing adGroupId and the mutable fields to be modified. Mutable fields are: name,
     * and state
     *
     * @param profileId
     * @param adgroup
    ...
    /**
     * GET /v2/sp/adGroups/?startIndex={startIndex} &count={count} &stateFilter={stateFilter}
     * &campaignIdFilter={campaignIdFilter} &adGroupIdFilter={adGroupIdFilter} &name={name} startIndex起始位置 count 每页显示数量
     * stateFilter 状态【 enabled, paused, or archived】 adGroupIdFilter 活动组ID name 活动名称
     *
     * @param profileId
     * @param param
     * @return
