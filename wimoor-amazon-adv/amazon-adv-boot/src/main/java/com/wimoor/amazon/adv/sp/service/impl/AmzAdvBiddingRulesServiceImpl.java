...
public class AmzAdvBiddingRulesServiceImpl extends BaseService<AmzAdvBiddingRules> implements
        IAmzAdvBiddingRulesService {
    ...
    public List<AmzAdvBiddingRules> amzSearchBiddingRules(AmzAdvProfile profile, JSONObject param) {
        ...
        if (StringUtil.isNotEmpty(response)) {
            ...
            for (int i = 0; i < optimizationRule.size(); i++) {
                ...
                AmzAdvBiddingRules old = amzAdvBiddingRulesMapper.selectByPrimaryKey(
                        biddingrule.getOptimizationRuleId());
                if (old != null) {
