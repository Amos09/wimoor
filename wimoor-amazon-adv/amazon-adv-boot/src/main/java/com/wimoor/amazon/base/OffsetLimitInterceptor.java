        ...
        if (parameter != null && parameter instanceof Map) {
            ...
            if (param.containsKey(UnionTablesSuffix.key)) {
                ...
                if (suffixobj != null && suffixobj instanceof UnionTablesSuffix) {
                                ...
                                for (Entry<String, String> suffixentry : tablesuffixmap.entrySet()) {
                                    if (StringUtils.isNotBlank(suffixentry.getKey())) {
                                        if (!UnionTablesSuffix.self_table_suffix.equals(suffixentry.getValue())) {
                                            sql = sql.replaceAll(suffixentry.getKey().trim() + " ",
                                                    suffixentry.getKey() + suffixentry.getValue().trim() + " ");
                                        }
                                    }
                                }
                                for (ParameterMapping item : parameterMappings) {
                                    org.apache.ibatis.mapping.ParameterMapping.Builder bd = new ParameterMapping.Builder(
                                            ms.getConfiguration(), item.getProperty(), item.getJavaType());
                    ...
                    BoundSql newBoundSql = copyFromBoundSqlNewParameter(ms, boundSql, newSqlSource,
                            nweparameterMappings, boundSql.getParameterObject());
