...
public class AmzAdvCampaignHsaServiceImpl extends BaseService<AmzAdvCampaignsHsa> implements IAmzAdvCampaignHsaService {
    ...
    public List<AmzAdvCampaignsHsa> amzUpdateSBCampaigns(UserInfo user, BigInteger profileId,
    ...
    public List<AmzAdvCampaignsHsa> amzCreateSBCampaigns(UserInfo user, BigInteger profileId,
            List<AmzAdvCampaignsHsa> campaignsList) {
        if (campaignsList == null || campaignsList.size() <= 0) {
            return null;
        }
        ...
        if (StringUtil.isNotEmpty(response)) {
            ...
            for (int i = 0; i < success.size(); i++) {
                ...
                if (Dbcampaigns == null) {
                    this.save(campaigns);
                    amzAdvOperateLogService.saveOperateLog("AmzAdvCampaignsHsa", user.getId(), profileId, campaigns,
                            null);
                }
            }
            for (int i = 0; i < error.size(); i++) {
