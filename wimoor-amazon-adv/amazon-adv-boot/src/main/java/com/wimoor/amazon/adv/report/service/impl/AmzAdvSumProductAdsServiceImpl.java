...
//	AmzProductActiveDayNumMapper amzProductActiveDayNumMapper;
    ...
    public Map<String, Object> getSumProduct(Map<String, Object> param) {
        ...
        if (list != null && list.size() > 0) {
            for (int i = 0; i < list.size(); i++) {
                ...
                sumOrdered =
                        sumOrdered + (attributedConversions == null ? attributedUnitsOrdered : attributedConversions);
                attributedSales = ExchangeRateUtil.changeCurrencyByLocal(market.getCurrency(), currency,
                        attributedSales);
        ...
        for (Entry<String, ArrayList<Map<String, Object>>> itemEntry : mapList.entrySet()) {
            ...
            if (list != null && list.size() > 0) {
                for (int i = 0; i < list.size(); i++) {
                    ...
                    sumImpressions = sumImpressions + impressions;
                    sumclicks = sumclicks + clicks;
                    sumOrdered = sumOrdered + (attributedConversions == null ? attributedUnitsOrdered
                    ...
                    if (clicks != null && clicks != 0) {
                        Integer order = (attributedConversions == null ? attributedUnitsOrdered
                                : attributedConversions);
    ...
    public Map<String, Object> getMonthSumProduct(Map<String, Object> param) {
        ...
        for (Entry<String, ArrayList<Map<String, Object>>> itemEntry : mapList.entrySet()) {
            ...
            if (list != null && list.size() > 0) {
                for (int i = 0; i < list.size(); i++) {
                    ...
                    sumcost = sumcost.add(cost);
                    attributedSales = exchangeRateHandlerService.changeCurrencyByLocal(market.getCurrency(), currency,
                            attributedSales);
                    sumSales = sumSales.add(attributedSales);
                    sumcost = sumcost.add(cost);
                    sumSales = sumSales.add(attributedSales);
        ...
        try {
            ...
            attributedSalesMap = ChartPoint.generateChartDate(ChartPoint.SumType.Monthly, attributedSalesMap, begin,
                    end);
        ...
        if (sumAllClicks != 0) {
            ...
            resultMap.put("sumAllcpc", sumAllCost.multiply(new BigDecimal("100"))
                    .divide(new BigDecimal(sumAllClicks), 2, RoundingMode.HALF_UP));
        } else {
            resultMap.put("sumAllcr", 0);
        }
        if (sumAllSales.compareTo(new BigDecimal("0")) != 0) {
            resultMap.put("sumAllacos",
                    sumAllCost.multiply(new BigDecimal("100")).divide(sumAllSales, 2, RoundingMode.HALF_UP));
        } else {
        ...
        if (sumAllall.compareTo(new BigDecimal("0")) != 0) {
            resultMap.put("sumAllsalespall",
                    sumAllSales.multiply(new BigDecimal("100")).divide(sumAllall, 2, RoundingMode.HALF_UP));
            resultMap.put("sumAllacoas",
                    sumAllCost.multiply(new BigDecimal("100")).divide(sumAllall, 2, RoundingMode.HALF_UP));
        } else {
