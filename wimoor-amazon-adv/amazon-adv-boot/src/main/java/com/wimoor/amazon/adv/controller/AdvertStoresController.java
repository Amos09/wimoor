...
public class AdvertStoresController {
    ...
    public Result<Map<String, Object>> findAsinsForStorePageUrlAction(String profileId, String brandEntityId,
            String storePageUrl) {
        UserInfo user = UserInfoContext.get();
        if (profileId == null || StringUtil.isEmpty(profileId)) {
            throw new BaseException("请选择正确的店铺站点！");
        ...
        List<AmzAdvAssets> assetList = amzAdvAssetsService.getAssetsForBrandEntityId(new BigInteger(profileId),
                brandEntityId);
