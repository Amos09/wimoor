...
public class AmzAdvSnapshotHandlerServiceImpl implements IAmzAdvSnapshotHandlerService {
    ...
    public void requestSnapshot(Map<String, Object> parammap) {
        ...
        for (AmzAdvProfile profile : profiles) {
            ...
            AmzAdvSnapshot snapshot = amzAdvSnapshotMapper.selectByProfileAndType(profile.getId().toString(),
                    advResponseRecordType, campaignType);
            if (snapshot != null && (snapshot.getTreatstatus() == null || !"expired".equals(snapshot.getTreatstatus())
                    || "SUCCESS".equals(snapshot.getTreatstatus()))) {// 今天存在请求记录，则不再请求
                continue;
            }
    ...
    private boolean treatAmzAdvSnapshot(AmzAdvSnapshot record, AmzAdvProfile advProfile) {
        ...
        try {
            if (CampaignType.sd.equals(record.getCampaigntype())) {
                responseJson = apiBuildService.amzAdvDownloadGZIP(advProfile, record.getLocation(),
    ...
    public void requestSnapshotByProfile(AmzAdvProfile profile, String recordType, String campaignType,
            AmzAdvAuth advauth) {
        String url = "/" + campaignType + "/" + recordType + "/snapshot";
        JSONObject map = new JSONObject();
        map.put("stateFilter", "enabled,paused,archived");
    ...
    public void requestStoreBrand() {
        ...
        for (AmzAdvAuth advauth : advauthlist) {
            if (advauth.getDisable() || advauth.getRegion().equals("TT") || (advauth.getDisableTime() != null
                    && GeneralUtil.distanceOfHour(advauth.getDisableTime(), new Date()) < 3)) {
                continue;
            }
            List<AmzAdvProfile> profiles = amzAdvAuthService.getAmzAdvProfile(advauth);
            for (AmzAdvProfile profile : profiles) {
    ...
    public void requestStoreBrand(AmzAdvProfile amzAdvProfile) {
        ...
        if (list != null) {
            for (int j = 0; j < list.size(); j++) {
                ...
                amzAdvAssetsService.amzGetStoresAssets(null, brand.getProfileid(), brand.getBrandentityid(),
                        "brandLogo");
            }
        }
        ...
        return new Runnable() {
            public void run() {
                for (AmzAdvAuth map : runnableList) {
                    try {
                        if (map.getDisableTime() != null
                                && GeneralUtil.distanceOfHour(map.getDisableTime(), new Date()) < 1) {
                            continue;
                        }
