...
public class AmzAdvBidReCommendServiceImpl implements IAmzAdvBidReCommendService {
    ...
    public List<Map<String, Object>> amzGetTargetBidRecommendations(UserInfo user, BigInteger profileId,
            String adGroupId, JSONArray jsona) {
        if (jsona == null || jsona.size() < 1) {
            return null;
        }
    ...
    public List<Map<String, Object>> amzCreateKeywordBidRecommendations(UserInfo user, BigInteger profileId,
            String adGroupId, List<Map<String, Object>> list) {
        if (list == null || StringUtil.isEmpty(adGroupId)) {
            return null;
        }
        AmzAdvProfile profile = amzAdvAuthService.getAmzAdvProfileByKey(profileId);
    ...
    public List<Map<String, Object>> amzRecommendationsBidsForKeyword_HSA(UserInfo user, BigInteger profileId,
    ...
    public JSONArray amzRecommendationsBidsForTarget_HSA(UserInfo user, BigInteger profileId, BigInteger campaignId,
            JSONArray jsona) {
        if (jsona == null) {
            throw new BaseException("请输入正确的参数！");
        }
