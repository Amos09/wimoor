    ...
    public Map<String, Object> getFieldObjectLinkMap(Object bean) {
        ...
        for (Field field : fields) {
            try {
                ...
                if (!"opttime".equals(field.getName()) && !"campaignid".equals(field.getName()) && !"adid".equals(
                        field.getName())
                        && !"adgroupid".equals(field.getName()) && !"keywordid".equals(field.getName())
                        && !"profileid".equals(field.getName())) {
                        ...
                        if (AdvUtils.isNumeric(value.toString()) || GeneralUtil.isDouble(value.toString())) {
                            if ("Click Thru Rate (CTR)".equals(titlearray[j].toString())
                                ...
                                if (Double.parseDouble(value.toString()) == 0) {
                                    ...
                                    BigDecimal bnumber = new BigDecimal(value.toString()).multiply(
                                            new BigDecimal("100"));
