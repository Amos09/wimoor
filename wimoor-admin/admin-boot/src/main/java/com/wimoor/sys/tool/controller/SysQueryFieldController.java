...
public class SysQueryFieldController {
    ...
    public Result<?> saveMyVersionFieldAction(@PathVariable String queryname,
            @RequestBody List<SysQueryVersionFeild> fieldlist) {
        UserInfo user = UserInfoContext.get();
        if (fieldlist == null || fieldlist.size() == 0) {
            throw new BizException("选择的列表不能为空");
        }
    ...
    public Result<?> saveMyVersionFieldAction(@RequestBody SysQueryVersionDTO dto) {
        UserInfo user = UserInfoContext.get();
        if (dto.getFieldlist() == null || dto.getFieldlist().size() == 0) {
            throw new BizException("选择的列表不能为空");
        }
        SysQueryUserVersion verson = iSysQueryFieldService.saveUserVersion(user, dto.getQueryname(),
                dto.getVersionname(), dto.getFieldlist());
        return Result.success(verson.getId());
    }
