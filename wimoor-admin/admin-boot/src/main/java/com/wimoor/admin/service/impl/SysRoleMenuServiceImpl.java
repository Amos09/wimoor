...
public class SysRoleMenuServiceImpl extends ServiceImpl<SysRoleMenuMapper, SysRoleMenu> implements ISysRoleMenuService {
    ...
    public boolean update(BigInteger roleId, List<BigInteger> menuIds) {
        ...
        if (CollectionUtil.isNotEmpty(dbMenuIds)) {
            List<BigInteger> removeMenuIds = dbMenuIds.stream().filter(id -> !menuset.contains(id))
        ...
        if (CollectionUtil.isNotEmpty(menuset)) {
            List<BigInteger> insertMenuIds = menuset.stream().filter(id -> !dbMenuIds.contains(id))
                    .collect(Collectors.toList());
