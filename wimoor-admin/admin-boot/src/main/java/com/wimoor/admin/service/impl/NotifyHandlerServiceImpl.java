...
public class NotifyHandlerServiceImpl extends ServiceImpl<SystemNotifyMapper, SystemNotify> implements
        INotifyHandlerService {
     ...
     * 往Notify表中插入一条提醒记录
     */
    public Integer createRemind(NotifyTarget target, String action, String sender, String shopid, String content,
            String title) {
    ...
    /**
     * 从UserNotify中获取最近的一条公告信息的创建时间: lastTime 用lastTime作为过滤条件，查询Notify的公告信息 新建UserNotify并关联查询出来的公告信息
     *
     * @param user
     */
    ...
    /**
     * 查询用户的订阅表，得到用户的一系列订阅记录 通过每一条的订阅记录的target、targetType、action、createdAt去查询Notify表，获取订阅的Notify记录。（注意订阅时间必须早于提醒创建时间）
     * 查询用户的配置文件SubscriptionConfig，如果没有则使用默认的配置DefaultSubscriptionConfig 使用订阅配置，过滤查询出来的Notify
