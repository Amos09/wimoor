...
public class SysUserServiceImpl extends ServiceImpl<SysUserMapper, SysUser> implements ISysUserService {
    ...
    public UserInfo convertToUserInfo(SysUser user) {
        ...
        if (user == null) {
            ...
            List<String> groups = grouplist.stream().map(item -> item.getGroupid().toString())
            ...
            List<String> datalimits = datalimitList.stream().filter(item -> item.getIslimit())
                    .map(item -> item.getDatatype().toString()).collect(Collectors.toList());
    ...
    private boolean addUserWechatMPInfo(SysUser m_user, String accesstoken, String refreshtoken, String openid,
            String ftype) {
        // TODO Auto-generated method stub
    ...
    public IPage<UserVO> listQuery(Page<?> page, UserDTO dto) {
        ...
        for (int i = 0; i < result.getRecords().size(); i++) {
            ...
            List<String> roles = iSysUserRoleService.list(
                            new LambdaQueryWrapper<SysUserRole>().eq(SysUserRole::getUserId, uv.getId()))
                    .stream().map(item -> {
            ...
            List<SysUserGroup> groups = sysUserGroupMapper.selectList(
                    new LambdaQueryWrapper<SysUserGroup>().eq(SysUserGroup::getUserid, uv.getId()));
            if (groups != null & groups.size() > 0) {
                ...
                for (SysUserGroup item : groups) {
                    try {
                        Result<?> resultv = restTemplate.getForObject(
                                "http://wimoor-amazon/amazon/api/v1/amzgroup/id/" + item.getGroupid(), Result.class);
            ...
            List<SysUserDatalimit> limits = iSysUserDatalimitService.list(
                    new LambdaQueryWrapper<SysUserDatalimit>().eq(SysUserDatalimit::getUserid, uv.getId()));
            if (limits != null && limits.size() > 0) {
                List<String> limit = new LinkedList<String>();
                for (SysUserDatalimit item : limits) {
                    if (item.getIslimit()) {
                        SysDictItem dict = iSysDictItemService.getOne(
                                new LambdaQueryWrapper<SysDictItem>().eq(SysDictItem::getDictCode, "limit_data_type")
    ...
    public boolean updateUser(UserInsertDTO userDTO, UserInfo operatorUserInfo) {
        ...
        if (StrUtil.isNotBlank(userDTO.getPassword())) {
            user.setSalt(PasswordHelper.initRoundomSalt());
            String password = PasswordHelper.encryptPassword(userDTO.getAccount(), userDTO.getPassword(),
                    user.getSalt());
            user.setPassword(password);
        }
    ...
    public SysUser changePassword(UserRegisterInfoDTO dto) {
        ...
        if (existUser != null) {
            ...
            existUser.setPassword(
                    PasswordHelper.encryptPassword(dto.getAccount(), dto.getPassword(), existUser.getSalt()));
